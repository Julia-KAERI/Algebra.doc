...
이다. @def-LA_multiplication_of_matrix 에 의해 선형사상의 합성은 각 선형사상의 행렬표현의 곱임을 알 수 있다. $\boldsymbol{K}$ 가 $K\in \mathcal{L}(W,\,V)$ 의 행렬표현이며, $\boldsymbol{L}$ 가 $L\in \mathcal{L}(U,\,W)$ 의 행렬표현이므로 $\boldsymbol{K}$ 의 열의 갯수와 $\boldsymbol{L}$ 의 행의 갯수는 $\text{dim}(W)$ 와 같아야 한다. 
...
이제 연산자 $L\in \mathcal{L}(U)$ 이 $L(u_i) = \sum_j A_{ij}u_j$ 로 정의되었으며 이 연산자의 $\mathcal{B}_1$ 기저에서의 행렬표현을 $\boldsymbol{A}$ 라 하자. 즉 $\boldsymbol{A} = [L]_{\mathcal{B}_1}$ 이다. 이 때,
...
이다. 이로부터 $[L]_{\mathcal{B}_2} = \boldsymbol{P}^{-1}\boldsymbol{AP}$ 임을 알 수 있다. 즉 두 기저 $\mathcal{B}_1 =\{u_i\}$, $\mathcal{B}_2 = \{P(u_i)\}$ 사이에,

$$
[L]_{\mathcal{B_1}} = \boldsymbol{A} \iff [L]_{\mathcal{B}_2} = \boldsymbol{P}^{-1}\boldsymbol{AP}
$$
...
&emsp;($1$) $\boldsymbol{A} \sim \boldsymbol{A}$ 이다.

&emsp;($2$) $\boldsymbol{A} \sim \boldsymbol{B}$ 이면 $\boldsymbol{B} \sim \boldsymbol{A}$ 이다.

&emsp;($3$) $\boldsymbol{A} \sim \boldsymbol{B}$ 이고 $\boldsymbol{B} \sim \boldsymbol{C}$ 이면 $\boldsymbol{C} \sim \boldsymbol{A}$ 이다.
