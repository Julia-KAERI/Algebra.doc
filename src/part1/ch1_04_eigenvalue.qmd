...
$L$ 을 대각화 하는 기저 $\mathcal{B}=(v_1,\ldots,v_n)$ 을 생각하자. 우선 모든 $v_i$ 에 대한 고유값이 다르다면 @prp-LA_invariant_subspace_and_eigenvector 에 의해 $U$ 는 $U$ 에 속하는 $\mathcal{B}$ 의 부분집합을 기저로 가지며 이 기저에서 $L|_U$ 는 대각화된다. $U$ 의 기저 $\mathcal{B}_U =(u_1,\ldots,\,u_m)$ 을 생각하자. $u_i = \sum_i a_{ji}v_j$ 라면 $a_{ji}\ne 0$ 이 아닌 모든 $v_i$ 는
...
두 연산자가 교환가능함을 가정하자. $K$ 가 대각화 가능 연산자이므로 $K$ 에 대한 고유값이 $k_1,\ldots,\,k_m$ 일 때 $V=\bigoplus_{k=1}^m E(k_i,\, K)$ 이다. @prp-LA_matrix_representation_of_compatible_matrix 에 의해 $E(k_i,\,K)$ 는 $L$ 에 대한 불변부분공간이며 
