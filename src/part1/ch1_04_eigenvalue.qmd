...
### 고유값, 고유벡터, 고유공간 {#sec-part1_eigens}
...
즉 $v$ 가 $\lambda$ 의 고유값을 갖는 $L$ 의 고유벡터라면 $(L-\lambda I)v=0_V$ 이다. @thm-part1_isomorphic_linear_map 로부터 다음과 같은 결론을 얻게 된다.

</br>
<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part1_finite_dimensional_vector_space_and_eigenvector}
...
::: {#exm-diagonal_matrix_and_eigenvalue}
...
### 고유값과 행렬식 {#sec-part1_eigenvalue_and_determinant}
...
@exr-part1_axler4_5_A_25 에 따라 모든 $V$ 의 벡터는 같은 고유값을 가지는 벡터이다. 이 고유값을 $c$ 라고 하자. 모든 기저에 대해 $L$ 의 행렬형식은 $cI_V$ 이므로 $L$ 은 $cI_V$ 이다. .
...
$v\in V$ 에 대해 $\text{span}\{v_i\}$ 가 $L$ 에 대한 불변부분공간이므로 모든 $v$ 는 $L$ 의 고유벡터이다. @exr-part1_axler4_5_A_26 에 따라 $L$ 은 항등연산자의 스칼라곱이다.
...
::: {#prp-degree_of_characteristic_polynomial}
...
::: {#prp-part1_invariance_of_characteristic_polynomial}
...
@prp-part1_invariance_of_characteristic_polynomial 는 유한차원 벡터공간 $V$ 에서 정의된 $L\in \mathcal{L}(V)$ 에 대한 특성다항식을 정의 할 수 있게 해 준다. 어떤 기저에 대해서도 $[L]$ 의 특성다항식은 같기 때문에 이것은 연산자 $L$ 에 대한 특성다항식이라고 할 수 있다. 

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-part1_eigenvalue_characteristic_polynomial}
...
::: {.proof}
대수학의 기본정리에 의해 복소수 영역에서는 항상 특성다항식의 해가 존재하며 @prp-part1_eigenvalue_characteristic_polynomial 에 의해 고유벡터가 존재한다. $\square$
:::
...
::: {.proof}
$\bf{A}_n \in \mathcal{M}_{n \times n}(\mathbb{C})$ 이라 하고 $n$ 에 대한 귀납법으로 증명한다. $n=1$ 일 때는 자명하다. $k<n$ 에 대해 위의 명제가 성립한다고 가정하자. 우리는 @thm-existance_of_eigenvector_for_complex_field 를 통해 행렬 $\bf{A}_n$ 에 대한 고유값 $\lambda$ 와 고유벡터 $\bf{v}_1$ 이 존재한다는 것을 안다. $\bf{v}_1$ 을 확장하여 $\mathcal{M}_n(\mathbb{F})$ 의 기저를 구성 할 수 있으며, 이 기저로 변환된  블럭 상삼각행렬을 이룬다 (@prp-invariant_subspace_block_upperdiagonal_matrix). 이 블럭 상삼각 행렬의 1 행과 1 열을 제거한 $(n-1)\times (n-1)$ 행렬은 가정에 의해 어떤 상 삼각 행렬과 닮았다. 따라서 $\bf{A}_n$ 은 어떤 상 삼각행렬과 닮았다. $\square$ 
:::
...
@prp-eigenvector_linear_independence 에서 보였듯이 서로 다른 고유값을 갖는 고유벡터는 선형독립이므로 이 $n$ 개의 고유벡터는 $\mathcal{M}_n(\mathbb{F})$ 의 기저이다. 따라서 $\bf{A}$ 는 대각화 가능 행렬이다. $\square$
:::
...
서로 다른 고유값을 갖는 고유벡터의 집합은 선형독립이라는 것을 안다(@prp-eigenvector_linear_independence). 따라서 $\{\bf{v}_1,\ldots,\,\bf{v}_m \}$ 가 선형독립이면 이 벡터들이 각각 다른 고유값을 갖는 고유벡터가 되는 행렬 $\bf{A}$ 가 존재함을 보이면 된다. $\bf{v}_1,\ldots,\,\bf{v}_m$ 을 확장하여 기저 $\{\bf{v}_1,\ldots,\,\bf{v}_n\}$ 을 구성하고 $\bf{P}= \begin{bmatrix} \bf{v}_1 & \cdots & \bf{v}_n\end{bmatrix}$ 라 하자. $\bf{P}^{-1}$ 은 $\bf{v}_i$ 를 표준기저 $\hat{\bf{e}}_i$ 로 변환해주는 연산자이다. 
...
$\bf{u}\in \ker (\bf{P})\cap \text{im}(\bf{P})$ 라 하자. $\bf{Pu}=\bf{0}$ 이며 $\bf{Pv}=\bf{u}$ 인 $\bf{u}$ 가 존재해야 한다. 그런데 $\bf{P}^2\bf{v} = \bf{Pu}=\bf{0}$ 이며 $\bf{P}^2\bf{v} = \bf{Pv}$ 이므로 $\bf{u}=\bf{0}$ 이다. 즉 $\ker (\bf{P})\cap \text{im}(\bf{P}) = \{\bf{0}\}$ 이다. @thm-rank_nullity 에 의해 $\mathcal{M}_n (\mathbb{F})=(\ker (\bf{P})) \oplus (\text{im}(\bf{P}))$ 이다.
...
@thm-rank_nullity 에 의해 $n=\text{nullity}(\bf{A})+\text{rank}(\bf{A})$ 이며 $\ker (\bf{A}) \cap \text{im}(\bf{A})=\{\bf{0}\}$ 이므로 $\mathcal{M}_n(\mathbb{F})=\ker (\bf{A})\oplus \text{im}(\bf{A})$ 이다.
...
($1\implies 2$) 가정에 의해 $\ker (\bf{A})\cap \text{im}(\bf{A})= \{\bf{0}\}$ 이다. @prp-condition_for_direct_sum 의해 $\ker (\bf{A}) + \text{im}(\bf{A}) = \ker (\bf{A})\oplus \text{im}(\bf{A}) = \mathcal{M}_{n}(\mathbb{F})$ 이다.

($2\implies 3$) @thm-rank_nullity 에 의해 $\ker (\bf{A}) \cap \text{im}(\bf{A}) = \{\bf{0}\}$ 이다. 

($3 \implies 1$) $\ker (\bf{A})$ 와 $\text{im}(\bf{A})$ 는 $\mathcal{M}_n(\mathbb{F})$ 의 부분공간이며 가정에 의해 $\ker (\bf{A}) + \text{im}(\bf{A}) = \ker (\bf{A}) \oplus \text{im}(\bf{A})$ 도 부분공간이다. @thm-rank_nullity 에 의해 $\mathcal{M}_{n}(\mathbb{F}) = \ker (\bf{A})\oplus \text{im}(\bf{A})$ 이다.
