...
@prp-linearmap_basis 를 보라. $\square$
...
@prp-linear_functional_4 로부터 $\varphi(v_i) \in V^\ast$ 이다. 이제 $\dim V^\ast = \dim V = n$ 이므로 $\{\varphi_1,\ldots,\varphi_n\}$ 이 선형독립임을 보이면 된다.
...
@prp-dual_basis 은 $n$ 차원 $\mathbb{F}$-벡터공간 $V$ 의 모든 선형 범함수는 $(\varphi_i,\ldots,\,\varphi_n)$ 의 선형결합으로 표현할 수 있다는 것을 의미한다.
