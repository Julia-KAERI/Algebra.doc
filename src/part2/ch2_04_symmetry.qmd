...
유클리드 공간 $\mathbb{R}^n$ 에서 어떤 고정된 $v\in \mathbb{R}^n$ 에 대해 다음과 같이 정의된 함수 $t_v :\mathbb{R}^n \to \mathbb{R}^n$ 를 $v$ 만큼의 **평행 이동(translation)** 이라고 한다.
...
$v \in \mathbb{R}^n,\, v\ne 0$ 에 대한 평행 이동 $t_v$ 는 isometry 이지만 linear isometry 는 아니다.
:::
...
$\mathbb{R}^n \mapsto\mathbb{R}^n$ isometry 는 평행이동과 linear isometry 의 합성으로 표현 할 수 있다. 
...
Isometry $f:\mathbb{R}^n \to \mathbb{R}^n$ 를 생각하자. $f(0)=a$ 일 때 $t_{-a}\circ f$ 를 생각하자. @prp-MA_composition_of_isometries 에 의해 $t_{-a}\circ f$ 도 isometry 이며 원점을 보존하므로 $t_{-a}\circ f$ 는 linear isometry 이다. $\varphi = t_{-a}\circ f$ 라 하면 $f = t_{a}\circ \varphi$ 이므로 임의의 isometry 는 평행이동과 linear isometry 의 합성이다. $\square$
...
$t_a$ 가 $a$ 만큼의 평행이동 이고 $\varphi,\, \psi$ 가 linear isometry 일 때 다음이 성립함을 보여라.
...
$M_n$ 을 $\mathbb{R}^n$ 에서의 isometry 의 집합이라고 하자. $\mathbb{R}^n$ 에서의 평행이동과 linear isometry 가 전단사임을 안다. 또한 합성도 isometry 임을 안다. $f=(t_a\circ \varphi)$ 일 때 $f^{-1} = \varphi^{-1}\circ t_{-a} \in M_n$ 이다. $\square$
...
Isometry $f$ 는 평행이동 $t_a$ 와 linear isomeetry $\varphi$ 의 합성으로 $f=t_a \circ \varphi$ 로 표현 할 수 있다. 이 때 $a=f(0)$ 으로 정해진다. $\mathbb{R}^n$ 에서의 linear isomeetry 의 집합을 $O_n$ 이라 하자. ($O_n$ 은 $n \times n$ 직교행렬의 집합과 동형이라는 것을 안다!!!). 선형대수학에서 배운 지식으로 우리는 이것을 쉽게 보일 수 있다.
...
&emsp; ($2$) $\ker (\pi)$ 는 $\mathbb{R}^n$ 에서의 평행이동의 집합 $T_n$이다.
