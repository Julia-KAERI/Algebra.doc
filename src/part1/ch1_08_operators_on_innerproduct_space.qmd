...
우선 고정된 $v$ 에 대해 $\phi (u) = \langle Lu,\,v\rangle$ 는 선형 범함수(@exr-adjoint_operator_1) 이며 @thm-part1_reisz_representation_theorem_1 에 의해 $\phi(u) = \langle u, w\rangle$ 인 $w\in U$ 가 유일하게 존재한다. 이제 함수 $L^\ast : V \to U$ 를 $L^\ast( v)= w$ 인 함수로 생각 할 수 있다. (뭔가 이상한가? 이상해야 하는데..) 이 $L^\ast$ 를 $L$ 에 대한 adjoint function 이라고 한다.
...
여기서 한가지 짚어야 할 것이 있다. 우리는 리츠의 표현정리(@thm-part1_reisz_representation_theorem_1) 를 이용하였는데 이 정리는 유한차원 벡터공간에서만 성립한다. 실제로 adjoint 는 유한차원 벡터공간에서만 유일하게 정해지며, 무한차원벡터공간에서는 유일하지 않을 수 있다(@Hoffman1971 . pp 295)
...
@prp-properties_of_isometry 에서 보였듯이 $S$ 가 linear isometry 이면 $S^\ast =S^{-1}$ 이다. 따라서
$$
...
@prp-properties_of_isometry 에 의해 $S^{-1}=S^\ast$ 이면 $S$ 가 linear isometry 인 것을 이용한다.
$$
...
### 유니타리 행렬과 직교 행렬 {#sec-part1_unitary_matrix_and_orthogonal_matrix}
