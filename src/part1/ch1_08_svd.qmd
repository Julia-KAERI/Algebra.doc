...
($3$) @thm-kernel_image_of_adjoint_operator 과 ($2$) 로부터 $\text{im}(\bf{A}^\ast \bf{A}) = (\ker(\bf{A}^\ast \bf{A}))^\perp = (\ker (\bf{A}))^\perp = \text{im}(\bf{A}^\ast)$ 임을 안다. 
...
($2$) $\bf{A}$ 가 에르미트 이므로 $\dim (\bf{A})$ 는 $\bf{A}$ 의 $0$ 이 아닌 고유값의 갯수(반복되는 것도 포함하여)와 같다. @prp-properties_of_AAast 의 ($4$) 에 따라 $\bf{A}$ 의 특이값의 갯수는 $\text{rank}(\bf{A})$ 와 같다.
...
이다. 마지막 부등호는 베셀 부등식(@prp-part1_properties_of_orthonormal_vectors ($3$)) 으로부터 성립한다. 따라서 $\|\boldsymbol{Av}\| \le s_M \|\boldsymbol{v}\|$ 이다. $\square$
:::
