...
$(\boldsymbol{u}_1,\ldots,\boldsymbol{u}_n)$ 을 각각 $\lambda_i$ 의 고유값을 갖는 $\boldsymbol{A}$ 의 고유벡터라고 하고 $\boldsymbol{Bu}_i = \eta_i \boldsymbol{u}_i$ 라고 하자. $\boldsymbol{U}=\begin{bmatrix} \boldsymbol{u}_1 & \cdots & \boldsymbol{u}_n \end{bmatrix}$ 에 대해 $\boldsymbol{U}^{-1}\boldsymbol{AU}$ 는 대각행렬이며 이를 $\boldsymbol{D}$ 라고 하자. $\boldsymbol{U}^{-1}\boldsymbol{BU}=\boldsymbol{C}$ 라고 하자. 

$$
\boldsymbol{U}^{-1}\boldsymbol{ABU}= \boldsymbol{DC}, \qquad \boldsymbol{U}^{-1}\boldsymbol{BAU}= \boldsymbol{CD}
$$
...
$$
\begin{aligned}
(\boldsymbol{DC})_{ij} &= \sum_k D_{ik}C_{kj} = \sum_k D_{ii}\delta{ik}c_{kj}= D_{ii}C_{ij}, \\
(\boldsymbol{CD})_{ij} &= \sum_k C_{ik}D_{kj} = \sum_k D_{jj}\delta{ik}c_{ik}= D_{jj}C_{ij}
\end{aligned}
$$
