...
::: {#prp-dual_map}
@def-dual_map 에서 정의된 $L^T$ 은 선형사상이다.
:::
...
@prp-transpose_of_matrix 의 표기법을 그대로 사용한다. $\boldsymbol{A}$ 의 열공간의 차원은 $\text{rank}(L)$ 와 같다. @lem-rank_Tprime 에 따라 $\text{rank}(L^T)$ 과도 같다. 이것은 $\boldsymbol{A}^T$ 의 열공간의 차원과 같고 또한 $\boldsymbol{A}$ 의 행공간의 차원과 같다. $\square$ 
:::
