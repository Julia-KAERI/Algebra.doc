...
\implies & \langle \boldsymbol{u}_1,\,\boldsymbol{u}_2 \rangle = 0 \qquad \square
\end{aligned}
$$
...
임을 안다. @exr-operator_orthogonal_to_every_vector 로 부터 $\boldsymbol{A}=\boldsymbol{A}^\ast$ 임을 알 수 있다. 즉 $\boldsymbol{A}$ 는 에르미트 행렬이다.
 
($2 \implies 1$) $\boldsymbol{A}$ 가 에르미트 행렬이라 하자. 
...
$\mathbb{C}$-벡터공간에서 정의된 isometry 를 **유니타리 연산자/행렬 (unitary operator/matrix)** 라 하고 $\mathbb{R}$-벡터공간에서 정의된 isometry 를 **직교 연산자/행렬 (orthogonal operator/matrix)** 라 한다. 유니타리 행렬에 의한 행렬/벡터의 변환을 **유니타리 변환 (unitary transformation)**, 직교행렬에 의핸 행렬/벡터의 변환을 **직교 변환 (orthogonal transformation)**이라고 한다.
