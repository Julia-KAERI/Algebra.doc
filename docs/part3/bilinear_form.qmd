...
$T\in \mathcal{S}^2(V)$ 일 때 $\Psi_T(v) = T(v, v)$ 는 quadratic form 이다.
...
&= T(v+v', v+v') -T(v, v)- T(v', v') = T(v, v') + T(v', v) = 2T(v, v') 
\end{aligned}
$$
...
$$
\Psi_T(cv) = T(cv, cv) =  cT(v, cv)= c^2T(v) = c^2 \Psi_T(v)
$$ 
...
$0_Q \in \mathcal{Q}_V$ 임은 쉽게 보일 수 있다. 
$$
\begin{aligned}
...
이므로 $\Psi + a\Phi \in \mathcal{Q}_V$ 이다. 이로부터 $\mathcal{Q}_V$ 가 벡터공간임은 쉽게 증명 할 수 있다. $\square$

:::

</br>

::: {#thm-isomorphism_between_S2V_QV}
