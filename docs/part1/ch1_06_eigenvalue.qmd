...
이다. 즉 $\{\boldsymbol{u}_1,\ldots,\,\boldsymbol{u}_m\}$ 를 확장하여 $\mathbb{F}^{n \times 1}$ 기저에서는 $\boldsymbol{A}$ 로 표현된 변환이 블럭 상삼각행렬이 된다. 즉 $\boldsymbol{P} = \begin{bmatrix} \boldsymbol{u}_1 & \cdots &\boldsymbol{u}_n \end{bmatrix}$ 에 대해 $\boldsymbol{P}^{-1}\boldsymbol{AP}$ 는 블록 상삼각행렬이다. $\square$
