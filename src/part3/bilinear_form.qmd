...
우선 $0_M: \mathfrak{M}_k \to \mathbb{F}$ 를 다음과 같이 정의한다.

$$
0_M(v_1,\ldots,\,v_k) := 0
$$ {#eq-zero_of_k_linear_form}

이렇게 정의된 $0_M$ 은 $\mathfrak{M}_k$ 에 포함된다. $T_1,\, T_2\in \mathfrak{M}_k,\, c\in \mathbb{F}$ 에 대해 합과 스칼라곱을 다음과 같이 정의한다.
...
벡터공간 $V$ 에서 정의된 쌍선형 형식의 집합은 벡터공간이다. 벡터공간 $V$ 에서 정의된 $k$-텐서의 집합 $\mathcal{T}^k(V)$ 는 벡터공간이다.
...
유한차원 $\mathbb{F}$-벡터공간 $V$ 에서의 쌍선형 형식 $T\in \mathcal{T}^2(V)$ 을 생각하자. $V$ 의 기저 $\mathcal{B}=(v_1,\ldots,\,v_n)$ 에 대해 $T$ 의 $\mathcal{B}$ 에 대한 행렬표현 $[T]_{\mathcal{B}}$ 를 다음과 같이 정의한다.
...
$\bf{A}\in \F^{n \times n}$ 에 대해 아래와 같이 정의된 $B: \F^n \times \F^n \to \mathbb{F}$ 는 $\F^n$ 에 대한 쌍선형 형식이다.

$$
B(\bf{u}, \bf{v}) = \bf{u}^T\bf{A}\bf{v}.
$$
...
$n$ 차원 $\mathbb{F}$-벡터공간 $V$ 의 기저 $\mathcal{B}_V = (v_1,\ldots,\,v_n)$ 을 생각하자. 아래에 정의된 $\Phi_\mathcal{B} : \mathcal{T}^2(V) \to \F^{n\times n}$ 는 동형사상이다.
...
임의의 $\bf{A}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 와 $v_1,\,v_2 \in V$ 에 대해 $([v_1]_\mathcal{B})^T \bf{A} [v_2]_\mathcal{B}$ 는 $V$ 에서의 쌍선형 형식이다. 따라서 $\Phi_\mathcal{B}$ 는 전사이다. 또한 @prp-ALA_matrix_representation_of_blinear_form 에 따라 $[T]_\mathcal{B}$ 는 유일하게 정해진다. 따라서 $\Phi_\mathcal{B}$ 는 단사이다. 즉 $\Phi_\mathcal{B}$ 는 동형사상이다.
...
$$ 
\begin{aligned}
0 &= T(v_i + v_j, v_i + v_j) \\[0.3em]
&= T(v_i, v_i) + T(v_i, v_j) + T(v_j, v_i) + T(v_j, v_j) = T(v_i, v_j)+ T(v_j, v_i)
\end{aligned}
$$
...
$\bf{A}\in \mathbb{F}^{n\times n}$ 가 모든 $1\le i,\,j\ne n$ 에 대해 $A_{ij}= A_{ji}$ 일 때 $\bf{A}$ 를 **대칭행렬(symmectric matrix)** 라고 한다. 모든 $1 \le i \le j \le n$ 에 대해 $A_{ij} = -A_{ji}$ 일 때 $\bf{A}$ 를 **교대행렬(alternaing matrix)** 혹은 **반대칭 행렬(skew-symmetric matrix)** 이라고 한다.
...
$\bf{A}$ 가 교대행렬이라면 $A_{ii} = - A_{ii}$ 이므로 모든 대각성분은 $0$ 이다.
:::
...
($2$) $\bf{E}^{(i, j)} \in \mathbb{F}^{n\times n}$ 를 $i$ 행 $j$ 열 성분은 $1$, 나머지 $n^2-1$ 개의 성분은 $0$ 인 행렬이라고 하자. $\{\bf{E}^{(i, j)}:  1 \le i,\,j\le n\}$ 은 $\mathbb{F}^{n\times n}$ 의 기저임은 쉽게 알 수 있다. 이 때, 

$$
\{\bf{E}^{(i, i)}: 1 \le i \le n \} \cup \{ \bf{E}^{(i, j)}+ \bf{E}^{(j, i)} : 1 \le i < j \le n\}
$$
...
$$
\{ \bf{E}^{(i, j)}- \bf{E}^{(j, i)} : 1 \le i < j \le n\}
$$

는 선형독립이며 $n\times n$ alternating 행렬을 span 한다. 따라서 $\dim(\mathcal{A}^2(V)) = \dfrac{n(n-1)}{2}$ 이다. 


($3$) $\bf{A} = \dfrac{1}{2}(\bf{A}+\bf{A}^T) + \dfrac{1}{2} (\bf{A}-\bf{A}^T)$ 이다. 여기서 $\dfrac{1}{2}(\bf{A}+\bf{A}^T)$ 는 대칭행렬이며 $\dfrac{1}{2}(\bf{A}-\bf{A}^T)$ 는 alternating 행렬이므로 $\mathcal{T}^2(V) = \mathcal{S}^2(V) + \mathcal{A}^2(V)$ 이다. 또한 $\mathcal{S}^2(V) \cap \mathcal{A}^2(V)=\{0\}$ 이므로 $\mathcal{T}^2(V) = \mathcal{S}^2(V) \oplus \mathcal{A}^2(V)$ 이다. $\square$
...
우리가 [대칭군을 이용한 행렬식](../part1/ch1_03_determinant.qmd#sec-LA_determinant_definition_using_symmetric_group) 에서 행렬식을 도입할 때 우선 행렬의 행벡터에 대한 $\det$ 함수로 도입하였다. 행벡터로 도입한것은 우리가 바로 전에 행 기본연산에 대해 논의했기 때문에 그 연속선상에서 그렇게 했다. 우리는 이미 열벡터에 대해서도 정의할 수 있다는 것을 안다. $\bf{A}_i$ 를 $\bf{A}$ 의 $i$ 번째 열벡터라고 하자.

$$
\phi (\bf{A}_{1},\ldots,\,\bf{A}_{k}) := \det (\bf{A})
$$

라고 하하면 $\phi$ 함수는 다음과 같은 성질을 가진다는 것을 안다. 

&emsp;($1$) $\phi (\bf{e}_1,\ldots,\,\bf{e}_k)=1$

&emsp;($2$) $\phi \in \mathcal{A}^k(\mathbb{F}^k)$


이제 우리는 반대로 위의 두 조건을 만족하는 $k$-교대형식은 유일하다는 것을 보일 것이다.
...
우선 $\dim (\mathcal{A}^k (\F^k))\ne 0$ 임을 보이자. 대칭군을 사용하여, 

$$
\phi(\bf{A}_1,\ldots,\,\bf{A}_k) := \sum_{\sigma \in S_k} \text{sgn}(\sigma) \prod_{i=1}^k A_{\sigma(i),i}
$$ {#eq-first_def_of_det}

라고 정의하자. 우리는 $\phi \in \mathcal{T}^k(\mathbb{F}^k)$ 임을 쉽게 보일 수 있으며 @exr-determinant_change_of_order 에서 보인대로 $\phi \in \mathcal{A}^k (\F^k)$ 이다. 이 때 $\phi (\bf{e}_1,\ldots,\,\bf{e}_k)=1$ 이므로 $\dim (\mathcal{A}^k (\F^k)) > 0$ 이다.

이제 $\psi \in \mathcal{A}^k(\F^k))$ 이며 $\psi (\bf{e}_1,\ldots,\,\bf{e}_k) = c$ 라고 하자. 여기에 대해 $\chi := c\phi -\psi$ 라고 정의하면 $\chi \in \mathcal{A}^k(\F^k)$ 이며 $\chi(\bf{e}_1,\ldots,\bf{e}_k)=0$ 이다.

그런데 모든 $\bf{v}\in \F^k$ 는 $\{\bf{e}_1,\ldots,\,\bf{e}_k\}$ 의 선형결합이다. $i=1,\ldots,\,k$ 에 대해 $\bf{v}_i = \sum_j A_{ij}\bf{e}_j$ 라고 하자. 

$$
\begin{aligned}
\chi(\bf{v}_1,\ldots,\,\bf{v}_k) &= \sum_{j_1=1}^k A_{1, j_1}\chi (\bf{e}_{j_1},\bf{v}_2,\ldots,\,\bf{v}_k) \\
& \qquad \vdots \\
&= \sum_{j_1=1}^k \cdots \sum_{j_k=1}^k A_{1, j_1}\cdots A_{k,\,j_k} \chi(\bf{e}_{j_1},\ldots,\,\bf{e}_{j_k})
\end{aligned}
$$

이다. 그런데 $\chi$ 안의 표준기저의 인덱스 가운데 어떤 두개가 같으면 교대 $k$-형식 의 정의에 의해 $\chi$ 함수값이 $0$ 이다. 모두 다르면 $\chi$ 함수의 정의에 의해 $0$ 이다. 따라서 임의의 $\bf{v}_1,\ldots,\,\bf{v}_n$ 에 대해 $\chi(\bf{v}_1,\ldots,\,\bf{v}_n)=0$ 이다. 


이것은 $\mathcal{A}^k(\F^k)$ 의 모든 원소는 @eq-first_def_of_det 에서 정의된 $\phi$ 의 상수배라는 의미이다. 즉 $\dim (\mathcal{A}^k (\F^k)) = 1$ 이다. $\square$
...
$\phi \in \mathcal{A}^k(\F^k)$ 이며 $\phi (\bf{e}_1,\ldots,\,\bf{e}_k)=1$ 인 $\phi$ 는 유일하다. 
...
$x\in V$ 에 대해 $[x]_\mathcal{B} = \bf{x}=\begin{bmatrix}x_1 & \cdots &x_n\end{bmatrix}^T,\, [B_\Psi]_\mathcal{B}=\bf{A}$ 라고 하자. 그렇다면, 

$$
\Psi(x) = \dfrac{1}{2}\bf{x}^T\bf{Ax} = \dfrac{1}{2} \sum_{i=1}^n A_{ii}{x_i}^2 + \sum_{i<j} A_{ij}x_i x_j
$$
...
::: {.proof}
$v,\,v'\in V$ 에 대해 $[v]_\mathcal{B}=\bf{v},\, [v']_\mathcal{B}=\bf{v}'$ 이라 하고 $[L]_\mathcal{B}=\bf{L},\, [B_\Psi]_\mathcal{B}= \bf{B}$ 라고 하자.
우선 $\Psi,\,\Psi\in \mathcal{Q}_V$ 일 때 $B_\Psi (v, v) = B_\Phi (v, v) \iff \Psi = \Phi$ 임은 당연하다. 따라서 $\bf{v}^T[B_\Psi]\bf{v} = \bf{v}^T[B_\Phi]\bf{v} \iff [B_\Psi]=[B_\Phi]$ 를 의미한다.

($1 \implies 2$)  @prp-ALA_matrix_representation_of_blinear_form 에 따라 $B_{\Psi}(Lv, Lv') = \bf{v}^T\bf{L}^T\bf{BLv}$, $B_\Psi (v,v) = \bf{v}^T\bf{Bv}$ 이다. 따라서 $\bf{L}^T\bf{BL}= \bf{B}$ 이다. 

($2 \implies 1$)  $\bf{L}^T\bf{BL}= \bf{B}$ 이면 $\bf{v}^T\bf{L}^T\bf{BLv}$, $B_\Psi (v,v) = \bf{v}^T\bf{Bv}$ 이므로 $L\in O(V,B_\Psi)$ 이다. $\square$
...
$\bf{J}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 가 대칭행렬 일 때 

$$
O_n^\bf{J} (\mathbb{F}) = \{ \bf{A} \in GL_n(\mathbb{F}) : \bf{A}^T\bf{JA} = \bf{J}\}
$$

를 $\bf{J}$ 에 대한 대칭군이라고 한다.
...
대칭행렬 $\bf{J}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 가 가역일 때 $\bf{A}\in O_n^\bf{J}(\mathbb{F})$ 의 행렬식은 $\det(\bf{A}^T\bf{JA}) = \bf{J}$ 이며 $\det (\bf{A}^T) = \det (\bf{A})$ 이므로 $\det (\bf{A})= \pm 1$ 이다.
...
::: {.proof}
$v,\,v'\in V$ 에 대해 $[v]_\mathcal{B}=\bf{v},\, [v']_\mathcal{B}=\bf{v}'$ 이라 하고 $[L]_\mathcal{B}=\bf{L},\, [B]_\mathcal{B}= \bf{B}$ 라고 하자.
우선 $B,\,B'\in \mathcal{Q}_V$ 일 때 $B (v, v') = B' (v, v') \iff B = B'$ 임은 당연하다. 따라서 $\bf{v}^T[B]\bf{v} = \bf{v}^T[B']\bf{v} \iff [B]=[B']$ 를 의미한다.

($1 \implies 2$)  $B(Lv, Lv') = \bf{v}^T\bf{L}^T\bf{BLv}$, $B (v,v) = \bf{v}^T\bf{Bv}$ 이다. 따라서 $\bf{L}^T\bf{BL}= \bf{B}$ 이다. 

($2 \implies 1$)  $\bf{L}^T\bf{BL}= \bf{B}$ 이면 $\bf{v}^T\bf{L}^T\bf{BLv}$, $B (v,v) = \bf{v}^T\bf{Bv}$ 이므로 $L\in Sp(V,B_\Psi)$ 이다. $\square$
...
$\mathcal{M}_{2 \times 2}(\mathbb{R})$ 에서 정의된 $\bf{J} = \begin {bmatrix} 1 & 0 \\ 0 & -1\end{bmatrix}$ 에 대한 직교군 $O_2^\bf{J}$ 을 $O(1, 1)$ 이라고 한다.
...
&emsp;($1$) $\bf{A} = \begin{bmatrix} x& z \\ y & w\end{bmatrix} \in O(1, 1)$.
