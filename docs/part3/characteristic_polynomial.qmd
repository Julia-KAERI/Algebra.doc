...
::: {#thm-degree_one_factor_from_zeros}
...
@thm-degree_one_factor_from_zeros 에 따라 $m>n$ 개의 근을 가지면 $\deg(p) \ge m > n$ 이다. $\square$
...
대수학의 기본정리는 다항식의 영점을 어떻게 발견할 것인가에 대해서는 말하지 않는다. 단지 복소계수 다항식의 경우 최소한 하나의 영점을 가진다는 것을 보여준다. @thm-degree_one_factor_from_zeros 와 [대수학의 기본정리](#thm-fundamental_theorem_of_algebra) 를 이용하면 다음을 알 수 있다. 


</br>

::: {#thm-fundamental_theorem_of_algebra_2}
...
우선 $p\in \mathbb{C}_n[t]$ 일 때 일차식의 곱으로 분해됨을 induction 을 사용하여 보일 수 있다 $n=1$ 일 경우는 자명하다. $n-1$ 에 대해 성립하을 가정하자. 대수학의 기본정리에 의해 $p(t)$ 는 영점 $c_1$ 을 가지며 @thm-degree_one_factor_from_zeros 에 의해 $p(t) = (t-c)q(t)$ 인 $q(t)$ 가 존재한다. 또한 $\deg (q)= \deg (p)-1$ 이므로 induction 의 가정에 의해 $q(t)$ 는 $n-1$ 개의 일차식의 곱이므로 $p(t)$ 는 $n$ 개의 일차식의 곱이 된다는 것을 알 수 있다.
...
::: {#exr-pair_zeros_in_real_polynomial}
...
::: {#prp-factorization_of_real_polynomial}
...
$$ {#eq-factorization_of_real_polynomial}

:::

::: {.proof}

$p(t)\in \mathbb{R}_n[t]$ 와 같은 계수를 갖는 복소계수 다항식 $q(t)$ 을 생각하자. @thm-fundamental_theorem_of_algebra_2 에 의해 $q(t)$ 는 일차식의 곱으로 분해된다. $q(t)$ 의 영점 가운데 실수인 것을 $c_1,\ldots,\,c_n$ 이라고 하고 복소수인 것들을 $\lambda_1,\ldots,\,\lambda_l$ 이라고 하자. @exr-pair_zeros_in_real_polynomial 에 의해 $q(\lambda_1)= 0$ 이므로 $q(\overline{\lambda_1})=0$ 이어야 하며 $r_1(t) = x^2 - (\lambda_1 + \overline{\lambda_1}) + |\lambda_1|^2$ 이 $q(t)$ 를 분해해야 하고 $b_1 = (\lambda_1) + \overline{\lambda_1},\, c_1 =|\lambda_1|^2$ 에 대해 $b_1^2<4c_1$ 이어야 한다. $q(t)/r_1(t)$ 에 대해서도 이 방법을 계속 사용 할 수 있다. 따라서 $q(t)$ 는 실수근과 복소수근에 대해 @eq-factorization_of_real_polynomial 와 같이 분해 할 수 있다. $\square$
...
<!-- 어떤 $p(t_j) = 0$ 이라면 $c\not\in \{p(t_1),\ldots,\,p(t_n)\},\, c\in \mathbb{R}$ 을 선택한다. 모든 $p(t_j)\ne 0$ 이라면 $c=0$ 으로 놓는다. 그리고 $k\in \{0,\ldots,\,n\}$ 에 대해 $p_k(t) \in \mathbb{C}_{n-1}[t]$ 를 다음과 같이 정의한다.

$$
p_k (t) =(p(t_k)-c)\prod_{j=0,\,j\ne k}^n\dfrac{(t-t_j)}{(t_k -t_j)}
$$

그렇다면 $p_k(t_k)= p(t_k)-c$ 이며 $j\ne k$ 에 대해 $p_k (t_j)= 0$ 이다. $q(t) = \displaystyle \left(\sum_{k=0}^n p_k(t) \right)$ 라고 정의하면 모든 $t_j$ 에 대해 $q(t_j)= p(t_j)-c$ 이며 $q(t)\in \mathbb{R}_n[t]$ 이다.  -->
...
[케일리-해밀턴 정리](#thm-cayley_hamilton) 로부터 우리는 다음을 알 수 있다.

::: {#prp-degree_limit_of_minimal_polynonmial}
...
::: {#thm-minimal_polynomial_and_eigenvalues}
...
::: {#prp-minimal_polynomial_and_subspace}
...
:::{#prp-constat_of_minimal_polynomal}
...
::: {#thm-odd_dimensional_vector_space_and_eigenvalue}
...
@prp-degree_limit_of_minimal_polynonmial 로 부터 우리는 최소다항식의 차수가 $\dim(V)$ 보다 작거나 같음을 안다. 즉 $n \le \dim (V)$ 을 최소다항식의 차수일 때
...
::: {#lem-order_of_minimal_polynomial_of_inverse}
...
@lem-order_of_minimal_polynomial_of_inverse 에 따라 $L^{-1}$ 의 최소다항식은 $m_{L^{-1}} = \frac{1}{4}q(t)$ 이다.
...
$m_L$ 이 $L$ 의 최소다항식이며 $n=\dim (V)$ 라고 하자. @prp-constat_of_minimal_polynomal 에 따라 $L$ 이 가역인 것의 필요충분조건은 $m_L$ 의 상수항이 $0$ 이 아닌것이다. 
