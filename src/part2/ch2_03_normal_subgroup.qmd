...
$a,\,g$ 가 군 $G$ 의 원소일 때 $gag^{-1}$ 을 $g$ 에 의한 $a$ 의 **켤레(conjugate)** 라고 한다. $N\le G$ 가 모든 $a\in N$ 과 $g\in G$ 에 대해 $gag^{-1}\in N$ 일 때 $N$ 을 $G$ 의 **정규부분군(normal subgroup)** 이라고 하며 $N \trianglelefteq G$ 라고 표기한다.
...
$H \le G$ 일 때 $g\in G$ 에 대해 $gHg^{-1}= \{ ghg^{-1}: h\in H\}$ 는 $G$ 의 부분군이다. 
...
::: {#def-MA_conjugate_subgroup_subgroups}

@prp-MA_conjugate_subgroup 에서와 같이 정의된 $gHg^{-1}$ 을 **켤레부분군(conjugate subgroup)** 이라고한다.
...
군 $G,\,G'$ 의 데카르트곱 $G\times G'$ 에 아래와 같은 곱연산을 부여한 것을 $G$ 와 $G'$ 의 **외곱 (external product)** 라고 한다.
...
$r$ 과 $s$ 가 서로소인 두 정수일 때 계수가 $rs$ 인 순환군은 계수가 $r$, $s$ 인 두 순환군의 외곱과 동형이다.
...
두 무한 순환군의 곱군은 은 무한순환군이 아님을 보여라. 
...
이로부터 $N \trianglelefteq G$ 일 때 $N$ 에 의한 잉여류의 집합에 @prp-MA_product_of_cosets_of_normal_subgroup 에서와 같이 $(aN)(bN)=abN$ 이라는 연산을 부여할 수 있다는 것을 즉, 이 연산이 잘 정의된다는 것을 알 수 있다. 이 때 $N$ 에 의한 $G$ 의 왼쪽 잉여류 의 집합을 $G/N$ 이라고 하자.
...
$N \trianglelefteq G$ 일 때 함수 $\pi : G\to G/N$ 을 $\pi (g) = gN$ 으로 정의하면 $\pi\in \text{Hom}(G,\, G/N)$ 이다. 군과 몫군 사이에 정의된 이 준동형사상을 **정준사상(canonical map)** 혹은 **자연 사상(natural map)** 이라고 한다.
...
::: {.solution}
*일반적으로 반군(semigroup) 은 이항연산에 결합법칙이 성립하는 집합이며 항등원을 포함하는 반군을 monoid 라고 한다.*
