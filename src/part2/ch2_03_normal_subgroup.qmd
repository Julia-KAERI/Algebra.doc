...
$H$ 에 대한 왼쪽 잉여군의 집합과 $H$ 에 대한 오른쪽 잉여군의 집합이 같은 것의 필요충분조건은 @prp-normal_subgroup_condition 의 ($3$) 인 것을 보이자. $L_H$ 를 왼쪽 잉여군의 집합, $R_H$ 를 오른쪽 잉여군의 집합이라고 하자.
...
::: {#exm-multiplicaiton_map_and_homomorphism}

@prp-inner_product_group 의 $f$ 는 동형사상이 아니더라도 전단사 일 수 있다. 예를 들어 $S_3 = \{1,\, x,\,x^2,\,y,\,xy,\,x^2y\}$ 를 생각하자. 여기서 $x^3=y^2=1$ 이며 $x^2y=yx$ 이다. $H=\langle x\rangle,\, K = \langle y \rangle$ 에 대해 $f:H \times K \to S_3$ 를 $f(h, k) = hk$ 로 정의하자. $f$ 는 전단사이지만 $xy\ne yx$ 이므로 $f \notin \text{Hom}(H \times K,\, G)$ 이다.
...
::: {.solution}
@prp-inner_product_group 에 따라 $H\cap K = \{1_G\}$, $HK=G$, $H \trianglelefteq G,\, K \trianglelefteq G$ 임을 보이면 된다.
...
order 3 부분군과 order 5 부분군은 cyclic group 이다. $A=\langle a \rangle$ 를 order 3 부분군, $B=\langle b \rangle$ 를 order 5 부분군이라고 하자. $bab^{-1}$ 은 $a$ 이거나 $a^2$ 이다. $bab^{-1}=a$ 이면 $ab=ba$ 이므로 $AB=BA$ 이며, 따라서 @prp-herstein_lem_2_5_1 에 따라 $AB\le G$ 이다. $bab^{-1}=a^2$ 이면  $a^2b=a^{-1}b=ba$ 이다. 따라서 $AB=BA$ 이며 $AB\le G$ 이다. $AB = \langle ab\rangle$ 이며 $o(ab)=15$ 이다.
