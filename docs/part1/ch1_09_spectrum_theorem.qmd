...
이므로 $\boldsymbol{Pe}_i$ 는 $\boldsymbol{A}$ 의 고유벡터이다. $\boldsymbol{P}$ 가 isometry 이므로 $\{\boldsymbol{Pe}_1,\ldots,\,\boldsymbol{Pe}_n\}$ 은 $\boldsymbol{A}$ 의 고유벡터로 이루어진 $\mathcal{M}_n(\mathbb{C})$ 의 정규직교기저이다.
...
$\boldsymbol{A}\in \mathcal{M}_{n\times n}(\mathbb{F})$ 가 어떤 정규직교기저 $(\boldsymbol{u}_1, \ldots,\,\boldsymbol{u}_n)$ 에 대해 $\|\boldsymbol{Au}_i\| = \|\boldsymbol{A}^\ast \boldsymbol{u}_i\|$ 이면 $\boldsymbol{A}$ 가 정규행렬인가? 
