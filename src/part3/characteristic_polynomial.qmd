...
### 다항식의 기본적인 성질 {#sec-basic_properties_of_polynomial}

@thm-existance_of_eigenvector_for_complex_field 에서 정의한 바와 같다.

::: {.callout-note appearance="minimal"}

::: {#def-part3_polynomials}
#### 다항식의 차수와 표기

다항식 $p(t) = a_0 + a_1 t + \cdots + a_n t^n$ 에서 $a_n\ne 0$ 일 때 $p(t)$ 를 $n$ 차 다항식이라고 하며 이때의 $n$ 값을 다항식 $p(t)$ 의 **차수 (degree)** 라고 하며 $\deg (p)$ 혹은 $\deg (p(t))$ 라고 표기한다. $\mathbb{F}$ 값을 계수로 갖는 변수 $t$ 에 대한 다항식의 집합을 $\mathbb{F}[t]$ 라고 표기하며, $\mathbb{F}_n[t]$ 는 $n$ 차를 포함하여 그 이하의 차수를 갖는 다항식의 집합을 의미한다. 
...
### 다항식의 영점 {#sec-zeros_of_polynomials}

::: {.callout-note appearance="minimal"}

::: {#def-part3_zeros_of_polynomials}
...
::: {#thm-part3_degree_one_factor_from_zeros}
...
@thm-part3_degree_one_factor_from_zeros 에 따라 $m>n$ 개의 근을 가지면 $\deg(p) \ge m > n$ 이다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-part3_division_of_polynomials}
...
은 $\mathbb{F}_n[t]$ 에서 선형독립이며(@exm-linear_dependency_of_polynomials) 또한 $\mathbb{F}_n[t]$ 에 포함된다. $X$ 에 $n+1$ 개의 원소가 포함되므로 $X$ 는 $\mathbb{F}_n[t]$ 의 기저이다. $p(t) \in \mathbb{F}_n[t]$ 이므로, 
...
::: {#thm-part3_fundamental_theorem_of_algebra}
...
대수학의 기본정리는 다항식의 영점을 어떻게 발견할 것인가에 대해서는 말하지 않는다. 단지 복소계수 다항식의 경우 최소한 하나의 영점을 가진다는 것을 보여준다. @thm-part3_degree_one_factor_from_zeros 와 대수학의 기본정리(@thm-part3_fundamental_theorem_of_algebra) 를 이용하면 다음을 알 수 있다. 


</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part3_fundamental_theorem_of_algebra_2}
...
우선 $p\in \mathbb{C}_n[t]$ 일 때 일차식의 곱으로 분해됨을 induction 을 사용하여 보일 수 있다 $n=1$ 일 경우는 자명하다. $n-1$ 에 대해 성립하을 가정하자. 대수학의 기본정리에 의해 $p(t)$ 는 영점 $c_1$ 을 가지며 @thm-part3_degree_one_factor_from_zeros 에 의해 $p(t) = (t-c)q(t)$ 인 $q(t)$ 가 존재한다. 또한 $\deg (q)= \deg (p)-1$ 이므로 induction 의 가정에 의해 $q(t)$ 는 $n-1$ 개의 일차식의 곱이므로 $p(t)$ 는 $n$ 개의 일차식의 곱이 된다는 것을 알 수 있다.
...
::: {#exr-part3_pair_zeros_in_real_polynomial}
...
::: {#prp-part3_factorization_of_real_polynomial}
...
$$ {#eq-part3_factorization_of_real_polynomial}
...
$p(t)\in \mathbb{R}_n[t]$ 와 같은 계수를 갖는 복소계수 다항식 $q(t)$ 을 생각하자. @thm-part3_fundamental_theorem_of_algebra_2 에 의해 $q(t)$ 는 일차식의 곱으로 분해된다. $q(t)$ 의 영점 가운데 실수인 것을 $c_1,\ldots,\,c_n$ 이라고 하고 복소수인 것들을 $\lambda_1,\ldots,\,\lambda_l$ 이라고 하자. @exr-part3_pair_zeros_in_real_polynomial 에 의해 $q(\lambda_1)= 0$ 이므로 $q(\overline{\lambda_1})=0$ 이어야 하며 $r_1(t) = x^2 - (\lambda_1 + \overline{\lambda_1}) + |\lambda_1|^2$ 이 $q(t)$ 를 분해해야 하고 $b_1 = (\lambda_1) + \overline{\lambda_1},\, c_1 =|\lambda_1|^2$ 에 대해 $b_1^2<4c_1$ 이어야 한다. $q(t)/r_1(t)$ 에 대해서도 이 방법을 계속 사용 할 수 있다. 따라서 $q(t)$ 는 실수근과 복소수근에 대해 @eq-part3_factorization_of_real_polynomial 와 같이 분해 할 수 있다. $\square$
...
정사각 행렬에 대한 특성다항식은 이미 [특성다항식](../part1/ch1_04_eigenvalue.qmd#sec-characteristic_polynomial) 에서 정의하였지만 환기를 위해 다시 한번 정의한다.

::: {.callout-note appearance="simple" icon="false"}
::: {#def-part3_characteristic_polynomial_re}
...
- @prp-part1_invariance_of_characteristic_polynomial : 특성 다항식은 닮음 변환에 대해 불변이다. 즉 행렬 $\boldsymbol{A}\in \mathcal{M}_{n\times n}(\mathbb{F})$ 과 가역행렬 $\boldsymbol{P}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 에 대해 $p_{\boldsymbol{A}}(\lambda) = p_{\boldsymbol{P^{-1}AP}}(\lambda)$ 이다.

- @prp-part1_eigenvalue_characteristic_polynomial : $\boldsymbol{A}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 에 대해 $\lambda\in \mathbb{F}$ 가 $\boldsymbol{A}$ 의 고유값이기 위한 필요충분조건은 $p_{\boldsymbol{A}}(\lambda)=0$ 인 것이다.

</br>

::: {.callout-note appearance="simple" icon="false"}
::: {#def-part3_trace_of_matrix}
...
::: {#prp-part3_trace_is_invariant_under_similar_transformation}
...
::: {#prp-part3_similarity_relation_in_annihilator_ideal}
...
::: {#prp-part3_annihilator_ideal_is_nontrivial}
...
::: {#exr-part3_before_cayley_hamilton}
...
::: {#thm-part3_cayley_hamilton_theorem}
...
임을 보이면 된다. $\boldsymbol{A}$ 가 상삼각행렬이므로 $(\boldsymbol{A}- A_{ii}\boldsymbol{I}_n)_{ii}  = 0$ 이다. 위의 @exr-part3_before_cayley_hamilton 에서 보였듯이 $p_\boldsymbol{A}(\boldsymbol{A}) =0$ 이므로 $p_T(L) \in \mathcal{I}_L$ 이다.
...
::: {#thm-part3_existance_and_uniqueness_of_minimal_polynomial}
...
@prp-part3_annihilator_ideal_is_nontrivial 에 의해 $L$ 에 대한 최소다항식은 존재한다. 이제 $p,\,q$ 가 $L$ 에 대한 최소다항식이라고 하자. 당연히 두 다항식은 같은 차수 $m$ 를 가진다. $p \ne q$ 라면 $r=p-q$ 는 $m$ 보다 작은 차수를 가지며 $r(L)=0$ 이다. $p,\,q$ 가 최소다항식이므로 남은 유일한 가능성은 $r=0$ 일 때 뿐이며 따라서 $p=q$ 이다. $\square$
:::

</br>

위의 정리와 @prp-part3_similarity_relation_in_annihilator_ideal 로부터 다음을 알 수 있다.
...
[케일리-해밀턴 정리](#thm-part3_cayley_hamilton_theorem) 로부터 우리는 다음을 알 수 있다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-part3_degree_limit_of_minimal_polynonmial}
...
::: {#prp-part3_divisability_of_annihilator_ideal}
...
우리는 @prp-part1_eigenvalue_characteristic_polynomial 에서 연산자에 대한 특성다항식의 해가 그 연산자의 고유값임을 보였다. 실제로는 특성다항식이 아니라 최소다항식에 대해서도 성립한다. 

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part3_minimal_polynomial_and_eigenvalues}
...
::: {#prp-part3_minimal_polynomial_and_subspace}
...
:::{#prp-part3_constant_of_minimal_polynomal}
...
::: {#prp-part3_even_dimensiona_kernel}
...
$p(t) = t^2+bt+c,\, U = \ker (L^2+bL+cI_V)= \ker p(L)$ 라고 하자. @prp-kernal_and_images_of_operator_under_polynomial 에 따라 $U$ 는 $L$-불변부분공간이다. $K=L|_U$ 라고 하면 $p(K)= p(L|_U)$ 이며 $K^2+bK+cI_U=0$ 이다. $u\in U$ 가 $\lambda$ 의 고유값을 갖는 $K$ 의 고유벡터라고 가정하자. 그렇다면
...
::: {#thm-part3_odd_dimensional_vector_space_and_eigenvalue}
...
@thm-existance_of_eigenvector_for_complex_field 에 의해 유한차원 $\mathbb{C}$-공간에서 정의된 모든 연산자는 고유값을 가진다는 것을 안다. 따라서 $\mathbb{R}$-벡터공간에서 생각한다. $n=\dim (V)$ 라고 하자. $n=1$ 이면 당연하게도 성립한다. 이제 $n$ 보다 작은 홀수차원 $\mathbb{R}$-벡터공간 $V$ 에서 성립한다는 것을 가정하자. $L\in \mathcal{L}(V)$ 에 대한 최소다항식 $m(t)\in \mathbb{R}[t]$ 를 생각하자. 
...
@prp-part3_degree_limit_of_minimal_polynonmial 로 부터 우리는 최소다항식의 차수가 $\dim(V)$ 보다 작거나 같음을 안다. 즉 $n \le \dim (V)$ 을 최소다항식의 차수일 때
...
::: {#lem-part3_order_of_minimal_polynomial_of_inverse}
...
@lem-part3_order_of_minimal_polynomial_of_inverse 에 따라 $L^{-1}$ 의 최소다항식은 $m_{L^{-1}} = \frac{1}{4}q(t)$ 이다.

:::

</br><div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-part3_axler4_5b_15}
...
$m_L$ 이 $L$ 의 최소다항식이며 $n=\dim (V)$ 라고 하자. @prp-part3_constant_of_minimal_polynomal 에 따라 $L$ 이 가역인 것의 필요충분조건은 $m_L$ 의 상수항이 $0$ 이 아닌것이다. 
