...
$$
\bf{L} = \begin{bmatrix} 1 & t_0 & t_0^2 & \cdots & t_0^n \\ 1 & t_1 & t_1^2 & \cdots & t_1^n \\ & & \vdots & & \\ 1 & t_n^1 & t_n^2 & \cdots & t_n^n\end{bmatrix},\qquad \bf{a} = \begin{bmatrix} a_0 \\ a_1 \\ \vdots \\ a_n\end{bmatrix},\qquad \bf{p}= \begin{bmatrix} p(t_0) \\ p(t_1) \\ \vdots \\ p(t_n)\end{bmatrix}
$$

를 생각하자. $\bf{La}$ 의 $i$ 번째 성분은 $p(t_i)$ 와 같으므로 $\bf{La}=\bf{p}$ 를 만족하는 $\bf{a}$ 가 $\mathbb{R}$-벡터로서 존재한다면 우리는 원하는 답을 얻는 것이다. 여기서 $\bf{L}$ 은 Vandermond 행렬로 @exr-determinant_of_vandermond_matrix 에서 보았듯이 서로 다른 $t_0,\ldots,t_n$ 에 대해 $\det(\bf{L}) \ne 0$ 이다. 즉 $\bf{L}$ 은 가역행렬이며 따라서 $\bf{La}=\bf{p}$ 에 대한 nontrivial solution $\bf{a}$ 가 존재한다. $\bf{L}$ 과 $\bf{p}$ 가 실수에서의 행렬과 벡터이므로 $\bf{a}$ 도 모든 성분이 실수이다. 
...
### 특성다항식 {#sec-AZA_characteristic_polynomial}
...
유한차원 벡터공간 $V$ 에서 정의된 연산자 $L$ 에 대해 $p_{L}(t) = \det (t I_V-L)$ 를 **특성 다항식(characteristic polynomial)** 이라 한다.

:::
:::


행렬식은 닮음 변환에 대해 불변이므로 어떤 기저 $\mathcal{B}$ 에 대해서도 특성다항식은 불변이다. 또한 우리는 다음을 보였다. 또한 @prp-LA_eigenvalue_characteristic_polynomial 에서 $L\in \mathcal{L}(V)$ 에 대해 $\lambda\in \mathbb{F}$ 가 $L$ 의 고유값이기 위한 필요충분조건은 $p_{L}(\lambda)=0$ 인 것임을 안다.
...
다음과 같이 정의된 $\text{tr} : \F^{n\times n} \to \mathbb{F}$ 을 행렬의 **trace** 라고 한다.

$$
\text{tr}(\bf{\bf{A}}) := \sum_{i=1}^n A_{ii}.
$$ {#eq-ALA_definition_of_trace_for_matrix}

유한차원 벡터공간 $V$ 에서 정의된 연산자 $L\in \mathcal{L}(V)$ 과 $V$ 의 기저 $\mathcal{B}$ 에 대해 연산자 $L$ 의 **trace** 는 다음과 같이 정의된다.
$$
\text{tr}(L) := \sum_{i=1}^{\dim (V)}([L])_{ii}.
$$ {#eq-ALA_definition_of_trace_for_operator}

:::
:::

행렬에 대한 trace 는 기저와 무관하게 정해진다는 것을 보일 수 있다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-ALA_trace_is_invariant_under_similar_transformation}

정사각 행렬에 대한 trace 는 닮음 변환에 대해 불변이다.
...
$\bf{A},\,\bf{P}\in \mathbb{F}^{n\times n}$ 이며 $\bf{P}$ 가 가역이라고 하자. 

$$
\begin{aligned}
\text{tr}(\bf{P}^{-1}\bf{AP}) &= \sum_{i=1}^n\left(\bf{P}^{-1}\bf{AP}\right)_{ii} \\[0.3em]
&= \sum_{i, j, k=1}^n (P^{-1})_{ij}A_{jk}P_{ki} = \sum_{j, k=1}^n A_{jk} (\bf{PP}^{-1})_{kj} = \sum_{j=1}^n A_{jj} \\
&= \text{tr}(\bf{A}). \qquad \square
\end{aligned}
$$
...
::: {#prp-ALA_trace_and_determinant_in_characteristic_polynomial}

유한차원 벡터공간 $V$ 에 대해 $L\in \mathcal{L}(V)$ 의 특성다항식 

$$
p_L(t) = t^n + a_{n-1}t^{n-1}+ \cdots + a_1 t + a_0
$$

이 

$$
p_L(T) = (t-\lambda_1)\cdots (t-\lambda_n)
$$

으로 인수분해 되면 (이 $\lambda_i$ 가운데 동일한 값이 있어도 상관 없다.) 이 행렬의 trace 와 행렬식은 아래와 같다.

$$
\begin{aligned}
\operatorname{tr}(L) &= \sum_{i=1}^n \lambda_i = a_n,\\[0.3em]
\det (L) &= (-1)^n\prod_{i=1}^n \lambda_i .
\end{aligned}
$$
...
trace 와 행렬식은 기저에 무관히므로 일단 아무 기저라도 잡을 수 있으며 이 기저에 대한 행렬로 생각하자. $V$ 가 실벡터 공간이라도 일단 복소벡터공간이라고 생각하고 상삼각행렬로 닮음변환하자. @prp-ALA_trace_is_invariant_under_similar_transformation 에 따라 상삼각행렬로 변환해도 trace 와 행렬식은 불변이다. $\square$

:::

</br>

### 케일리 해밀턴 정리 {#sec-ALA_cayley_hamilton_theorem}
...
$\mathbb{F}$-벡터공간 $V$ 에 대해 $L\in \mathcal{L}(V)$ 일 때 아래와 같이 정의된 $\mathcal{I}_L$ 를 **annihilator ideal** 이라고 한다.
...
$n=\dim (V)$ 이라고 하자. $\dim(\mathcal{L}(V)) = n^2$ 이므로, $\{I_V,\, L, \ldots,\, L^{n^2}\}$ 는 선형종속이다. $\square$
...
$\bf{B}_i\in \F^{n\times n}$ 는 상삼각행렬이며 $i$ 번째 대각성분이 $0$ 이라고 하자. 이 때 $\bf{B}_1 \bf{B}_2 \cdots \bf{B}_n = \bf{0}$ 임을 보여라.
...
$\bf{v}=\begin{bmatrix} v_1 &\ldots & v_n\end{bmatrix}^T$ 라고 하자. $1\le i \le n$ 인 $k$ 에 대해 $v_{i+1} = \cdots =v_{n}=0$ 이면 $\bf{B}_i \bf{v}$ 의 $i$ 번째 성분부터 $n$ 번째 성분까지 $0$ 임을 보이자. 

$$
(\bf{B}_i\bf{v})_{j\ge i} = \sum_{k=1}^n (\bf{B}_i)_{jk}(v_k) =\sum_{k>j\ge i}^n(\bf{B}_i)_{jk}(\bf{v}_k) = (\bf{B}_i)_{ii}v_i = 0
$$

이다. $\bf{B}_1 \bf{B}_2 \cdots \bf{B}_n \bf{v}$ 를 생각하면 $\bf{B}_n\bf{v}$ 는 마지막 성분이 $0$ 이며 차례로 아래부터 성분이 $0$ 이 되므로 $\bf{B}_1 \bf{B}_2 \cdots \bf{B}_n \bf{v} =\bf{0}$ 이다. 
...
$n=\dim (V)$ 라고 하자. 우선 $\mathbb{F}=\mathbb{C}$ 에 대해 증명한다. @thm-LA_uppertriangular_matrix_complex_field 에서 우리는 복소수체에서 정의된 모든 정사각행렬은 상삼각행렬과 닮았다는 것을 보였다. 즉 모든 연산자는 어떤 기저에서 상삼각행렬로 표현된다. 특성다항식은 닮음변환에 대해 불변이므로 우리는 상삼각행렬에 대한 특성다항식에 대해서만 증명하면 된다. $\bf{A}$ 가 $L$ 의 상삼각행렬 표현이라고 하면, $p_\bf{A}(t) = \prod_{i=1}^n (t-A_{ii})$ 이다. 이제 

$$
p_\bf{A}(\bf{A}) = \prod_{i=1}^n (\bf{A}-A_{ii}\bf{I}_n)=0
$$ 

임을 보이면 된다. $\bf{A}$ 가 상삼각행렬이므로 $(\bf{A}- A_{ii}\bf{I}_n)_{ii}  = 0$ 이다. 위의 @exr-ALA_before_cayley_hamilton 에서 보였듯이 $p_\bf{A}(\bf{A}) =0$ 이므로 $p_T(L) \in \mathcal{I}_L$ 이다.
...
### 최소다항식 {#sec-ALA_minimal_polynomial}
...
$L\in \mathcal{L}(V)$ 에 대한 annihilator ideal $\mathcal{I}_L$ 가운데 1 차 이상의 다항식 중 가장 작은 차수를 갖는 monic 다항식을 $L$ 에 대한 **최소다항식(minimal polynomnial)** 이라고 하고 $m_L$ 로 표기한다.
...
::: {#exm-ALA_get_minimal_polynomial}

다음 세 행렬의 최소다항식을 생각하자.

$$
A=\begin{bmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{bmatrix},\quad B=\begin{bmatrix} 1 & 1 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{bmatrix}, \quad C=\begin{bmatrix} 1 & 1 & 0 \\ 0 & 1 & 1\\ 0 & 0 & 1 \end{bmatrix}
$$

세 행렬의 특성다항식은 모두 $(t-1)^3$ 으로 같다. 그러나 $A-I=0$ 이므로 $m_A(t) = t-1$ 이며 $(B-I)\ne 0,\, (B-I)^2=0$ 이므로 $m_B(t) = (t-1)^2$, 같은 방법으로 $m_C(t) = (t-1)^3$ 임을 알 수 있다. 따라서 이 세 행렬은 서로 닮지 않았다.
...
@prp-ALA_division_of_polynomials 로 부터 다음을 쉽게 증명 할 수 있다.
...
유한차원 벡터공간 $V$ 에 대해 $U\le V$ 이며 $L\in \mathcal{L}(V)$ 에 대해 $L|_U$ 가 $L$ 의 $U$ 로의 제한일 때 $L$ 의 최소다항식은 $L|_U$ 의 최소다항식으로 나누어진다.
...
$p_L$ 이 $L$ 의 최소다항식이라고 하자. $u\in U$ 이면 $u\in V$ 이므로 $p_L(T)u=0$ 이다. 따라서 $p_L$ 는 $L|_U$ 에 대한 annihilator ideal 에 포함된다. $\square$
...
### 최소다항식과 연산자의 대각화 {#sec-ALA_minimal_polynomial_and_diagonalization}

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-ALA_ALA_minimal_polynomial_and_diagonalization}

유한차원 $\F$-벡터공간 $V$ 에서의 연산자 $L$ 이 대각화 가능할 필요충분조건은 어떤 서로 다른 $c_1,\ldots,\,c_m\in \F$ 에 대해 $p(t)=\prod_{i=1}^m (t-c_i)$ 가 $L$ 의 최소다항식인 것이다.
...
$L$ 이 대각화 가능하면 서로 다른 $L$ 의 고유값 $c_1,\ldots,\,c_m$ 에 대해 $V=\bigoplus_{i=1}^m E(c_i,\,L)$ 이다. 따라서 $p(L)=0$ 이며 $p(t)$ 는 $L$ 의 최소다항식이다. 

이제 서로 다른 $c_1,\ldots,\,c_m\in \F$ 에 대해 $p(t)=\prod_{i=1}^m (t-c_i)$ 가 $L$ 의 최소다항식이면 $L$ 은 대각화 가능함을 보이자. $m$ 에 대한 induction 을 사용한다. $m=1$ 일 때는 자명하다. 이제 $m>1$ 일 때 $1,\ldots,\,m-1$ 까지 성립함을 가정하자. $U=\im (L-c_mI)$ 는 $L$-불변 부분공간임을 안다(@prp-LA_kernal_and_images_of_operator_under_polynomial). $u\in U$ 이면 어떤 $v\in V$ 에 대해 $(L-c_mI)(v)$ 이며 최소다항식 $p$ 를 생각하면

$$
(L-c_1I)\cdots (L-c_{m-1}I)u = (L-c_1I)\cdots (L-c_m I) v =0
$$ {#eq-thm-ALA_ALA_minimal_polynomial_and_diagonalization_1}

이다. 따라서 $(L-c_1I)\cdots (L-c_{m-1}I)$ 는 $\im(L-c_{m-1}I)$ 은 $U$ 의 annihilator ideal 에 속하며 induction 의 가정에 의해 $U$ 는 $L$ 의 고유벡터로 이루어진 기저를 가진다. 이제 $\text{im}(I-c_mI)\cap \ker (I-c_mI)=\{0_V\}$ 임을 보이자. $u\in \text{im}(I-c_mI)\cap \ker (I-c_mI)$ 라면 $(L-c_m I)u=0_V$ 이며 $u=(I-c_m I)v$ 인 $v\in V$ 가 존재해야 한다. $Lu=c_mu$ 이므로 @eq-thm-ALA_ALA_minimal_polynomial_and_diagonalization_1 로 부터

$$
0 = (L-c_1I)\cdots (L-c_{m-1} I)u = (c_m -c_1)\cdots (c_m -c_{m-1}) u
$$

이다. $c_1,\ldots,\,c_m$ 이 모두 다르다는 조건으로 부터 $u=0_V$ 임을 알 수 있다. 따라서 $\text{im}(I-c_mI)\cap \ker (I-c_mI)=\{0_V\}$ 이며 @thm-LA_rank_nullity 로부터 $V=\text{im}(L-c_m I) \oplus \ker (L-c_m I)$ 임을 안다. $\im (L-c_m I)$ 가 $L$ 의 고유벡터로 이루어진 기저를 가지며 $\ker (L-c_m I)$ 는 고유값 $c_m$ 을 가지는 $L$ 의 고유벡터로 된 기저를 가지므로 $V$ 는 $L$ 의 고유벡터로 이루어진 기저를 가진다. $\square$
...
$B\in \F^{m \times m},\,C\in \F^{m \times n},\, D\in \F^{n\times n}$ 에 대해 

$$
A = \begin{bmatrix} B & C \\ 0 & D\end{bmatrix}
$$

이고 $A,\,B,\,D$ 의 특성다항식과 최소다항식을 각각 $p_X,\, m_X,\, (X=A,\,B,\,D)$ 라고 하자. 이 때 다음이 성립함을 보여라.

&emsp;($1$) $p_A(t) =p_B(t) p_D(t)$, 

&emsp;($2$) $m_A(t)$ 는 $\text{lcm} (m_B(t),\, m_D(t))$ 의 배수,

&emsp;($3$) $D=0$ 이면 $m_A(t) = \text{lcm} (m_B(t),\, m_D(t))$.
...
($1$) $B,\,D$ 를 상삼각 행렬로 변환시키는 변환행렬을 각각 $U_B,\, U_D$ 라고 하면 

$$
\begin{bmatrix} U_B & 0 \\ 0 & U_D\end{bmatrix}
$$

는 $A$ 를 상삼각 행렬로 변환시키며 이 상삼각 행렬의 대각성분은 각각 $B,\,D$ 의 상삼각행렬의 대각성분과 같다. 따라서 $p_A(t) = p_B(t)p_D(t)$ 이다.

($2$) 다항식을 다루므로 $B$, $D$ 를 상삼각행렬이라고 간주할 수 있다. 다항식 $p$ 에 대해 

$$
p(A) = \begin{bmatrix} p(B) & Y \\ 0 & p(D)\end{bmatrix}
$$

이며 여기서 $Y$ 는 $p,\,B,\,C,\,D$ 에 의해 결정되는 $m \times n$ 행렬이다. 이로부터 $m_B(t) \mid m_A(t)$ 이며 $m_D(t) \mid m_A(t)$ 이어야 하므로 $m_A(t)$ 는 $\text{lcm} (m_B(t),\, m_D(t))$ 의 배수이어야 한다.
...
$L\in \mathcal{L}(V)$ 일 때 최소다항식 $p_L$ 이 1차식의 곱으로 인수분해된다면 어떤 기저 $\mathcal{B}$ 에서 $[L]_\mathcal{B}$ 가 상삼각행렬이 됨을 보여라.
...
$n=\dim V$ 라고 하고 induction 을 통해 증명한다. $n=1$ 일 때는 자명하다. $n-1$ 에 대해 성립함을 가정한다. 

$$
p_L(t) = (t-\lambda_1)\cdots (t-\lambda_n)
$$

이라고 하자. $p_L(\lambda_1)=0$ 이므로 $\lambda_1$ 은 $L$ 의 고유값이며 이 $L$ 에 대한 고유벡터 $e_1$ 이 존재한다. $e_1$ 과 선형 독립인 $f_2,\ldots,\,f_n$ 을 찾을 수 있으며 이 기저에서 $L$ 의 표현은

$$
\left[ \begin{array}{c|c}\lambda_1 & *\\  \hline 0 & L_1 \end{array}\right]
$$

이다. Induction 의 가정에 의해 $L_1$ 을 상삼각 행렬로 만드는 기저 $e_2,\ldots,\,e_n$ 이 존재하며 기저 $\mathcal{B}=(e_1,\ldots,\,e_n)$ 에 대해 $[L]_\mathcal{B}$ 는 상삼각행렬이다. 
...
상삼각 행렬 $L\in \mathcal{L}(V)$ 을 생각하자. 상삼각 행렬의 대각성분이 모두 1 일 때 $L$ 을 **unipotent** 라고 한다. $L$ 이 unipotent 이고 $L^m=I$ 인 자연수 $m$ 이 존재하면 $L=I$ 임을 보여라.
...
$n=\dim (V)$ 라고 하면 $p_L(t) = (t-1)^n$ 이다. 또한 $q(t) = t^m-1$ 에 대해 $q(L)=0$ 이므로 $p_L \in \mathcal{I}_L,\, q\in \mathcal{I}_L$ 이다. @prp-ALA_divisability_of_annihilator_ideal 에 의해 $m_T \mid p_L,\, m_T \mid q$ 이어야 한데 $p_L$ 과 $q$ 의 공약수는 $t-1$ 뿐이므로 $m_L(t) = t-1$ 이다. 즉 $L=I$ 이다.
...
#### Axer4 5B.16, 동반 행렬 (companion matrix)
...
#### Axler4 5B.23
유한체원 벡터공간 $V$ 와 $L\in \mathcal{L}(V)$ 를 생각하자. $n=\dim (V)$ 라고 하자. $v\in V$ 에 대해 $\text{span}(v,\, Lv,\ldots,\,L^{n-1}v)$ 는 $L$-불변부분공간임을 보여라.
