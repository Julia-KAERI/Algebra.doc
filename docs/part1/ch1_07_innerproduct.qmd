...
벡터공간 $V$ 와 유한차원 부분공간 $U$ 에 대해 다음이 성립한다.

&emsp; ($1$) $U \cap U^\perp = \{0_V\}$, 

&emsp; ($2$) $V = U \oplus U^\perp$,

&emsp; ($3$) $\dim (V) = \dim (U) + \dim (U^\perp)$

:::


::: {.proof}
$U$ 의 정규직교기저를 찾아 $\{e_1,\ldots,\,e_n\}$ 이라 하고 이 기저를 확장하여 나머지 정규직교기저를 $\{f_1,\ldots\}$ 이라 하자. $\{f_1,\ldots\}$ 의 선형결합은 $U$ 와 직교하므로 $U^\perp$ 에 포함된다. $U^\perp$ 의 어떤 벡터가 $\{e_i\}$ 기저 성분을 포함한다면 $U$ 와 직교할수 없다. 따라서 $U^{\perp} = \text{span} (f_1,\ldots,f_m)$ 이다. 이를 이용하면 세가지를 모두 보일 수 있다. $\square$
:::


::: {.callout-warning appearance="minimal" icon="false"}

위의 @prp-generated_subspace_and_orthogonal_compliment 는 무한차원 벡터공간에서는 성립하지 않는다. @exr-axler_6_C_14 를 참고하라.
...
$u\in \text{im}(P)$ 이면 $P(v)=u$ 인 $v\in V$ 가 존재하며, $P^2v = Pu = u$ 이므로 $\text{im}(P)$ 는 $P$ 불변 부분 공간이다. $\square$
...
유한차원 벡터공간 $V$ 와 projection $P\in \mathcal{L}(V)$ 에 대해 $U = \text{im}(P)$ 라 하자. $U \le V$ 이므로 $V=U \oplus U^\perp$ 이다. 또한 @prp-uniqueness_of_orthogonal_decomposition 에서 보인 것처럼 $v\in V$ 에 대해 $v=u+u',\, u\in U,\, u'\in U^{\perp}$ 가 되도록 하는 $u,\,u'$ 은 유일하다. 이제 반대로 $U \le V$ 에 대해 $v\in V$ 는 어떤 $u\in U$ 와 $u'\in U^\perp$ 의 합으로 유일하게 표현 될 수 있다는 것을 안다. 이렇게 어떤 부분공간 $U$ 를 치역으로 갖는 orthogonal projection 이라고 하며 $P_U$ 라고 쓴다.
...
벡터공간 $V$ 의 부분공간 $U$ 를 치역으로 갖는 projeciton 을 $V$ 에 대한 $U$ 로의 **orthogonal projection** 이라고 하며 $P_U$ 라고 쓴다.
...
$U$ 가 $V$ 의 유한차원 부분공간일 때 $U^\perp = \{0_V\} \iff U=V$ 임을 보여라. [$U$ 가 $V$ 의 무한차원 부분공간일때는 성립하지 않는다는 것을 @exr-axler_6_C_14 에서 보인다]{.aside}
...
유한차원 벡터공간 $V$ 에서 정의된 projection $P$ 가 모든 $v\in V$ 에 대해 $\|Pv\| \le \|v\|$ 라고 하자. $P=P_U$ 인 어떤 $U\le V$ 가 존재함을 보여라.
...
로 정의되었다. $U\le C_{[-1, 1]}$ 이 다음과 같이 정의되어다고 하자.
...
$g\in U^\perp$ 라고 하자. $f_n(x)$ 를 $g(x)$ 와 $n\in \mathbb{Z}_+$ 에 대해 다음과 같이 정의한다.
...
$f_n(0)=0$ 이며 $f_n(x)$ 는 $[-1,\,1]$ 에서 연속인 함수이다. 즉 $f_n \in U$ 이다.

$$
\int_{-1}^1 f_n(x)\, g(x)\, dx = \int_{-1}^{-\frac{1}{n}} g^2(x)\, dx +\int_{\frac{1}{n}}^1 g^2(x)\, dx + \dfrac{g\left(\frac{1}{n}\right)}{\frac{1}{n}} \int_{0}^{\frac{1}{n}}xg(x)\, dx - \dfrac{g\left(-\frac{1}{n}\right)}{\frac{1}{n}}\int_{-\frac{1}{n}}^0 xg(x)\, dx 
$$

이다. 적분에 대한 평균값 정리에 의해 $\int_{0}^{1/n}xg(x) \, dx = n x_1 g(x_1)$ 을 만족하는 $x_1\in (0, \frac{1}{n})$ 이 존재하며, $\int_{-1/n}^0 xg(x)\,dx = -nx_2 g(x_2)$ 를 만족하는 $x_2\in (-\frac{1}{n}, 0)$ 이 존재한다. 따라서, 
