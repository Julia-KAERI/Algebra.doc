...
이다. 또한 선형 범함수를 내적으로 표현할 수 있도록 해 주는 벡터 $u$ 는 유일하다. 
...
이므로, $u = \sum_i \overline{\varphi (e_i)}e_i$ 로 잡으면 된다. 이제 유일성을 보이자. $\phi (v) = \langle v,\, u\rangle = \langle v,\,u'\rangle$ 이라 하자. 모든 $v\in V$ 에 대해 $\langle v, u-u'\rangle =0$ 이므로 $v=u-u'$ 을 생각하면 $u=u'$ 이어야 한다. $\square$
...
$L\in \mathcal{M}_{m \times n}$ 에 대해 다음과 같이 정의된 $L^+\in \mathcal{M}_{n \times n}$ 을 $L$ 의 **무어 펜로즈 역행렬 (Moorre-Penrose inverse matrix)** 혹은 **의사역행렬 (pseudo inverse matrix)** 라고 한다.
...
$V$ 는 유한차원 내적벡터공간이고 $U\le V$ 라고 하자. $P_{U^\perp} =  I - P_U$ 임을 보여라.
...
$U, W\le V$ 이고 $V$ 가 유한차원 벡터공간일 때 $P_U P_W=0$ 일 필요충분조건은 모든 $u\in U,\, w\in W$ 에 대해 $\langle u,\,w\rangle = 0$ 인 것임임을 보여라.
...
$P_UP_W=0$ 이면 $W \le U^\perp$ 이므로 모든 $u\in U,\, w\in W$ 에 대해 $\langle u,\,w \rangle = 0$ 이다. 
모든 $u\in U,\, w\in W$ 에 대해 $\langle u,\, w\rangle =0$ 이면 $P_UP_W=0$ 임은 자명하다. 
:::
...
유한차원 내적 벡터공간 $V$ 에서 정의된 projection $P$ 가 모든 $v\in V$ 에 대해 $\|P(v)\| \le \|v\|$ 라고 하자. $P=P_U$ 인 어떤 $U\le V$ 가 존재함을 보여라.
...
$U$ 가 $V$ 의 유한차원 부분공간이며 $T\in \mathcal{L}(V)$ 일 때 다음이 동치임을 보여라.

&emsp; ($1$) $U$ 는 $T$-불변 부분공간이다.

&emsp; ($2$) $P_U T P_U = T P_U$ 이다.
...
$U$ 가 $T$ 불변부분공간이라고 하자. $v \in V$ 에 대해 $P_Uv = u \in U$ 라 하자. $P_UTP_U(v) = P_UT(u)=T(u)$ 이며 $TP_U(v)= T(u)$ 이므로 $P_UTP_U = TP_U$ 이다.

$U$ 가 $T$ 불변부분공간이 아니라고 하자. $u\in U$ 에 대해 $Tu\not\in U$ 라 하자. 어떤 $w'\in U^\perp$ 에 대해 $v=u+w$ 라면 $P_UTP_U(v) = P_UT(u) \in U$ 이지만 $TP_U(v)= T(u)\not\in U$ 이므로 $P_U TP_U \ne TP_U$ 이다.
