...
이것을 만족하는 함수는 어떤 것이든 내적이 될 수 있으며, 내적이 정의된 벡터공간을 **내적 벡터 공간(Inner product vector space)** 혹은 **내적 공간(Inner product space)** 이라 한다. 필드를 명시할 경우 $\mathbb{F}$-**내적 벡터 공간** 이라고 한다.흔히 두 백터의 내적을 $\langle u,\,v\rangle$ 로 쓰며 그 정의는 위의 $\phi$ 함수에 대해 $\langle u,\, v\rangle = \phi (u,\,v)$ 이다.$^\ast$ 
...
$\mathbb{R}^n$ 에서 $x = (x_1,\ldots,\,x_n)$, $y=(y_1,\ldots,\,y_n)\in \mathbb{R}^n$ 의 내적을 $\langle x,\, y \rangle =  \sum_{i=1}^n x_i y_i$ 로 정의할 수 있으며 이렇게 정의된 내적을 **스칼라곱(scalar product)** 혹은 **dot product** 라고 하고, 이렇게 스칼라곱으로 내적이 정의된 $\R^n$ 을 **유클리드 공간 (Euclidean sapce)** 이라고 한다. 
:::
...
두 벡터의 내적이 $0$ 일 때 두 벡터는 서로 **직교한다(be orthogonal, be perpendicular)** 고 한다. $\{v_1,\ldots,\,v_n\}$ 의 서로 다른 두 벡터가 모두 직교한다면 $\{v_1,\ldots,\,v_n\}$ 을 직교하는 벡터의 집합이라고 한다. 한 벡터가 다른 벡터의 $0$ 이 아닌스칼라의 곱일 때 두 백텨는 **평행하다(be parallel)** 라고 한다.
:::
:::
...
$L\in \mathcal{L}(V)$ 라 하자. 정규직교기저 $\mathcal{B}=(e_i)$ 에 대해
...
### 벡터에 대한 정사영 {#sec-LA_projection_on_vector}

두 벡터의 내적이 $0$ 일 때 두 벡터는 직교한다(be perpendicular)고 하며, 한 벡터가 다른 벡터의 스칼라곱일 때 두 벡터가 **평행하다(be parallel)** 고 한다. 내적벡터공간 $V$ 의 두 벡터 $u,\,v$ 가 주어졌다고 하자. 이 때 $v$ 의 $v$ 에 대한 정사영 $\text{Proj}_u v$ 를 아래와 같이 정의하자.
...
그람-슈미트 과정을 통해 얻은 $\{e_1,\ldots,\,e_m\}$ 은 정규직교벡터이다. 즉, $\langle e_i,\, e_j\rangle = \delta_{ij}$ 이다.
...
이다. 따라서 $\{e_1,\ldots,\,e_m\}$ 은 정규직교벡터이다. $\square$
...
::: {.proof}
$V$ 를 $n$ 차원 $\mathbb{C}$-내적벡터공간이라 하자. @thm-LA_uppertriangular_matrix_complex_field 에서 $L\in \mathcal{L}(V)$ 는 어떤 기저 에서 상삼각행렬로 표현된다는 것을 보였다. 이 기저를 $\mathcal{B}_V = (v_1,\ldots,\,v_n )$ 라고 하고, 그람-슈미트 과정을 통해 얻은 정규직교기저를 $(e_1,\ldots,\,e_n)$ 라고 하자. 즉 $e_i \in \text{span}(v_1,\ldots,\,v_i)$ 이다. 그렇다면, 그람 슈미트 과정으로부터
...
유한차원 내적벡터공간에서 항상 정규직교기저를 구할 수 있으며, 정규직교기저에서 여러 계산이 편해지기 때문에 정규직교기저를 가정하고 문제를 푸는 경우가 많다.
...
$V$ 의 정규직교기저 $(e_1,\ldots,\,e_n)$ 에 대해 $u=\sum a_i e_i$ 라고 하면 

$$
\begin{aligned}
\varphi (u) &= \varphi \left(\sum_i a_i e_i\right) = \sum_i a_i \varphi(e_i) = \sum_i \langle u,\,e_i\rangle  \varphi(e_i) \\ 
&= \sum_i \langle u,\, \overline{\varphi (e_i)}e_i \rangle = \left\langle u,\, \sum_i \overline{\varphi(e_i)} \, e_i \right\rangle
\end{aligned}
$$
...
Reitz 의 표현정리는 유한차원 내적벡터공간에서만 성립하며 무한차원에서는 성립하지 않는다. @exr-LA_axler_6_B_15 를 보라.
:::
...
### 직교 여공간 {#sec-LA_orthogonal_complement}
...
### 사영연산자 {#sec-LA_orthogonal_projection}
...
연산자 $P\in \mathcal{L}(V)$ 가 $P^2=P$ 일 때 $P$ 를 **사영 연산자(projection or projection operator)** 라고 한다.

:::
:::

예를 들어 $T\in \mathcal{L}(\mathbb{R}^2)$ 이 $T(x,\,y) = (x, 0)$ 으로 정의되었다고 하자. $T$ 는 선형사상이며 $T^2(x,\,y)$$= (x,\,0)$$= T(x,\,y)$ 이므로 $T$ 는 사영연산자이다. 이처럼 projectiion 은 우리가 기하학적으로 생각하는 그 사영을 추상화 한 것이다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_range_of_projection}

$P$ 가 사영연산자이면 $\text{im}(P)$ 는 $P$-불변 부분 공간이다.
...
벡터공간 $V$ 와 사영연산자 $P\in \mathcal{L}(V)$ 에 대해 $U = \text{im}(P)$ 라 하자. $U$ 가 유한차원 부분공간이라면 $V=U \oplus U^\perp$ 이라는 것을 안다. 또한 @prp-LA_uniqueness_of_orthogonal_decomposition 에서 보인 것처럼 $V$ 가 유한차원이라면 $v\in V$ 에 대해 $v=u+u',\, u\in U,\, u'\in U^{\perp}$ 가 되도록 하는 $u,\,u'$ 은 유일하다. 이제 반대로 $U \le V$ 에 대해 $v\in V$ 는 어떤 $u\in U$ 와 $u'\in U^\perp$ 의 합으로 유일하게 표현 될 수 있다는 것을 안다. 이렇게 어떤 부분공간 $U$ 를 치역으로 갖는 사영연산자를 다음과 같이 따로 정의할 수 있다.
...
벡터공간 $V$ 의 유한차원 부분공간 $U$ 를 치역으로 갖는 사영 연산자를 $V$ 에 대한 $U$ 로의 **직교사영연산자 (orthogonal projection)** 이라고 하며 $P_U$ 라고 쓴다.

:::
:::


아래 직교 사영 연산자에 대한 명제들은 쉽게 증명 할 수 있다.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_properties_of_orghogonal_projection}
$P_U$ 가 $U$ 로의 직교 사영 연산자이고 $U$ 가 $V$ 의 유한차원 부분벡터공간 일 때 다음이 성립한다. 
...
$\F=\R$ 일 경우는 $\Phi$ 가 선형사상이 되어 쉽게 풀 수 있다. @exr-LA_axler4_6_C_13 을 보라.
...
또한 $\text{im}\left( L|_{(\ker (L))^\perp}\right) \subset \text{im}(L)$ 임은 자명하다. $w\in \text{im}(L)$ 이면 $w = Lv$ 인 $v\in \mathbb{F}^n$ 가 존재하며 @prp-LA_generated_subspace_and_orthogonal_compliment 에 따라 어떤 $x\in\ker L,\, (\ker L)^\perp$ 에 대해 $v=x+y$ 이다. 이 때 $L|_{(\ker (L))^\perp} y = w$ 이므로 $w\in \text{im}\left( L|_{(\ker (L))^\perp}\right)$ 이며 따라서  $\text{im}\left( L|_{(\ker (L))^\perp}\right) = \text{im}(L)$ 이다. 즉 $L\in \mathbb{F}^{m\times n}$ 전사이다. $\square$
...
이로부터 우리는 다음을 쉽게 보일 수 있다. 그리고 왜 $L^+$ 가 $L$ 의 의사 역사상인지 알 수 있다.
...
유한차원 내적 벡터공간 $V$ 에서 정의된 사영연산자 $P$ 가 모든 $v\in V$ 에 대해 $\|Pv\| \le \|v\|$ 라고 하자. $P=P_U$ 인 어떤 $U\le V$ 가 존재함을 보여라.
...
::: {#exr-LA_axler4_6_C_19}

#### Axler4th 6.C.10

$V$ 는 유한차원 벡터공간이며 $L\in \mathcal{L}(V)$ 가 사영연산자 이면 $L^+=L$ 임을 보여라.
...
$L$ 이 사영연산자이므로 $L^2=L$ 이며 $L = P_{\text{im}(L)}$ 이다. 또한 $(\ker (L))^\perp = \text{im}(L)$ 이므로 $L|_{(\ker L)^\perp} = I|_{\text{im}(L)}$ 이다. @def-LA_pseudo_inverse 로 부터 $L^+w = L$ 이다.
...
@exr-LA_axler4_6_C_20 에서 보였듯이 $\text{im}(M) = \text{im}(L^+) = (\ker L)^\perp$ 이므로 
...
이제 $v\in (\ker L)^\perp$ 인 경우를 생각해보자. 앞서 보았듯이 $P_{\text{im}(M)}=P_{(\ker L)^\perp}$ 이므로 $M^+v = \left(M|_{(\ker M)^\perp} \right)^{-1}v$ 이다. 역시 @exr-LA_axler4_6_C_20 에서 보였듯이 $(\ker L^+)=(\text{im}(L))^\perp$ 이며 @cor-LA_dimension_relation_of_orthogonal_complement ($2$) 로부터
...
::: {#exr-LA_operator_orthogonal_to_every_vector}
