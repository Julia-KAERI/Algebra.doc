...
일반적으로 $\{y_1,\,y_2,\ldots\}$ 처럼 기술되는 집합은 집합 내의 원소를 나열하는 순서가 중요하지 않다. 하지만 그 순서가 중요한 경우가 있는데 이 경우 집합에 순서를 부여하여 $(x_1,\,x_2,\,x_3,\ldots)$ 와 같이 $(\qquad)$ 안에 나열하여 표현한다. 즉, $\{a,\,b\} = \{b,\,a\}$ 이지만 $(a,\,b) \ne (b,\,a)$ 이다. 
...
다항식 $p(x) = a_0 + a_1 x + \cdots + a_n x^n$ 을 생각하자. $a_n\ne 0$ 일 때 $p(x)$ 를 $n$ 차 다항식이라고 하며 이때의 $n$ 값을 다항식 $p(x)$ 의 **차수 (degree)** 라 한다. 굳이 0차 다항식이라고 이름을 붙이지는 않지만 상수함수도 다항식에 포함된다. $P_n(\mathbb{R})$ 을 실계수를 갖는 $n$ 차 이하의 다항식의 집합이라고 하면 $P_n(\mathbb{R})$ 이 $\mathbb{R}$-벡터공간임을 쉽게 알 수 있다. $P_n (\mathbb{C})$ 또한 $\mathbb{C}$ -벡터공간이다.
:::
...
::: {#exm-subspace_2}
@exm-vectorspace_polynomial 에서 정의한 $P_n(\mathbb{R})$ 을 생각하자. $m \le n$ 일 때, $P_m(\mathbb{R})$ 은 $P_n (\mathbb{R})$ 의 부분공간이다.
:::
...
::: {#prp-sum_of_subspace}
$V_1 \le V,\, V_2 \le V$ 일 때 $V_1+ V_2 \le V$ 이다. 따라서 $V$ 의 부분공간 $V_1, \ldots,\,V_m$ 의 합도 $V$ 의 부분공간이다.
:::
...
&emsp; ($1$) $V_1,\ldots,\,V_m$ 이 $V$ 의 부분공간이고 $V_1+ \cdots + V_m$ 이 직합이다.  
  
&emsp; ($2$) $0_V$ 를 각각의 $v_i\in V_i$ 의 합으로 나타내는 방법은 모든 $v_1 = \cdots = v_m = 0_V$ 뿐이다.
:::
...
::: {#cor-subspace}
$V_1,\,V_2$ 가 $V$ 의 부분공간일 때 $V_1+ V_2 = U_1\oplus U_2$ 일 필요충분조건은 $V_1 \cap V_2 = \{0\}$ 이다.
:::
...
우리는 앞서 어떤 집합을 확장하는 데카르트곱에 대해 알아보았다. 이제 벡터공간의 데카르트곱이 벡터공간이 되도록 확장하는 것을 알아보도록 하자.
...
::: {#exm-cartesian_product_space}

우리는 $\mathbb{F}$ 가 벡터공간임을 안다. 위의 @def-product_space 의 정의에 따라 곱공간 $\mathbb{F}^n$ 을 벡터공간으로 
...
우리는 앞에서 실수의 집합 $\mathbb{R}$ 이나 복소수의 집합 $\mathbb{C}$ 도 벡터공간임을 보였으며, 은근슬쩍 $\mathbb{R}^n$ 과 $\mathbb{C}^n$ 을 도입했다. $\mathbb{R}^n$ 은 $\mathbb{R}$ 의 곱공간으로 볼 수 있다는 것을 이해 할 수 있을 것이다. 곱공간을 통해 우리는 어떤 벡터공간들을 모아 더 큰 벡터공간을 만들 수 있다.

</br>

::: {#prp-map_product_space}

$U_1,\ldots,\,U_m$ 이 $\mathbb{F}$-벡터공간 $V$ 의 부분공간일 때 다음과 같이 정의된 함수 $\phi :  U_1 \times \cdots \times U_m  \to  U_1 + \cdots + U_m$ 는 선형사상이다.
...
::: {.proof}
$U = U_1 \times \cdots \times U_m$ 에 대해 $u,\,u'\in U,\, \lambda \in \mathbb{F}$ 일 때 $\phi (u+\lambda u') = \phi(u)+\lambda \phi (u')$ 인 것을 쉽게 보일 수 있다. $\square$
:::
...
유한차원 벡터공간 $V$ 의 부분공간 $U_1,\ldots,\,U_m$ 에 대해 $U_1 + \cdots + U_m$ 이 직합일 필요충분조건은
$$
...
$V$ 가 $\mathbb{F}$-벡터공간이고, $a_1,\ldots,\,a_m \in \mathbb{F}$, $\{v_1,\ldots,\,v_m\}\subset V$ 일 때

$$
v = a_1 v_1 + \cdots + a_m v_m
$$

이라면, $v$ 는 $\{v_1,\ldots,v_m\}$ 의 **선형결합** 이라고 한다. $v$ 가 $\{v_1,\ldots,\,v_m\}$ 의 선형결합일 때 $\{v_1,\ldots,\,v_m\}$ 가 $v$ 를 **span** 한다라고 한다. 또한 $\{v_1,\ldots,\,v_m\}$ 으로 span 할 수 있는 모든 벡터의 집합을 $\text{span} (v_1,\ldots,\,v_m)$ 이라 표현한다. 즉,
...
### 생성된 부분공간 {#sec-generated_subspace}
...
벡터공간 $V$ 의 부분집합 $\{v_1,\ldots,\,v_m\}$ 에 대해 $a_1 v_1 + \cdots + a_m v_m=0_V$ 을 만족하는 $a_1,\ldots,\,a_m$ 은  $a_1 = \cdots = a_m=0$ 뿐일 때, 즉
...
뒤에 나오겠지만 선형 독립인 벡터의 성질은 벡터공간을 이해하는데 매우 중요하다. 우리는 선형독립인 벡터들에 대해 다음을 증명 할 수 있다.

::: {#prp-zero_vector_dependent}
...
::: {.proof}
우선 $0_V$ 을 $\{v_1,\ldots,\,v_n\}$ 의 선형결합으로 표현하는 방법이 유일하다는 것을 안다. $v\in \text{span}(v_1,\ldots,\,v_n)$ 일 때,
...
### 벡터공간의 기저 (basis) 와 순서 기저 (ordered basis)
...
::: {#thm-finitebasis}
유한차원 벡터공간은 유한개의 벡터로 이루어진 기저를 갖는다. [무한차원 벡터공간이 기저를 갖는다는 것을 선택공리를 통해서 증명할 수 있지만 여기서는 다루지 않는다.]{.aside}
:::
...
우리는 이제 기저(basis) 라는 개념을 통해 어떤 벡터공간은 유한개의 벡터의 선형결합으로 모두 표현할 수 있다는 것을 알게되었다. 기저는 벡터공간을 설명하는데 매우 중요하며 앞으로 벡터와 벡터공간의 성질은 대부분 기저를 이용하여 보이게 된다. 또한 기저를 통해 차원(dimension) 이라는, 벡터공간의 아주 중요한 성질을 말할 수 있게 된다.
...
체 $\mathbb{F}$ 의 원소를 성분으로 갖는 $m \times n$ 행렬의 집합을 $\mathcal{M}_{m \times n}(\mathbb{F})$ 로 표기하기로 하였다. 이와 유사하게 $\mathbb{F}$ 를 성분으로 갖는 $3 \times 1$ 행렬, 혹은 행벡터의 집합을 $\mathcal{M}_m(\mathbb{F})$ 으로 표기하도록 한다. 
...
$\mathbb{F}^{n}$ 에서의 **표준기저벡터** $\hat{e}_i$ 는 $i$ 번째 성분은 1이고 나머지 성분은 0 인 벡터이다. 즉  $\hat{e}_i$ 의 $j$-번째 성분 $(\hat{e}_i)_j$ 는
...
$n$ 차원 벡터공간 $V$ 에 대해 $u,\,v\in V,\,\lambda \in \mathbb{F}$ 라 하자. $V$ 의 한 순서기저 $\mathcal{B} = (v_1,\ldots,v_n)$ 에 대해 $v=\sum_i a_i v_i$, $u=\sum_i b_i v_i$ 일 때, 

$$
[u+\lambda v]_{\mathcal{B}} = \left[\sum_{i} (a_i + \lambda b_i) v_i \right]_{\mathcal{B}} = \begin{bmatrix} a_1 + \lambda b_i \\ \vdots \\ a_n + \lambda b_n\end{bmatrix}
$$
