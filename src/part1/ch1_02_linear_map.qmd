...
$\mathbb{F}$-벡터공간 $U,\,V$ 에 대해 정의된 함수 $L: U \to V$ 이 모든 $u_1,\,u_2\in U,\, c\in \mathbb{F}$ 에 대해 

$$
L(u_1+ c u_2) = L(u_1) + c L(u_2).
$$ {#eq-def_linear_map}

이면  $L$ 을 $U$ 에서 $V$ 로의 **선형 사상(linear map)** 혹은 **선형 변환 (linear transformation)** 이라 한다. 그리고 $\mathbb{F}$-벡터공간 $U,\,V$ 에 대해 $U\mapsto V$ 인 선형사상의 집합을 $\mathcal{L}(U,\,V)$ 라고 쓴다. 자기 자신으로의 선형변환은 특별히 **선형 연산자(Linear operator)** 혹은 **연산자(Operator)** 라 하며 벡터공간 $V$ 에서의 선형 연산자의 집합을 $\mathcal{L}(V)$ 라 쓴다.
:::
:::
...
함수 $L:U \to V$ 가 모든 $u\in U$ 에 대해 $L(u)=0_V$ 일 때 이 $L$ 를 **영사상(zero map)** 이라 부르며 $0(u)$ 와 같이 표기한다. 그리고 영사상은 선형사상이다.
...
::: {#exm-LA_T_zero}
$L\in \mathcal{L}(U,\,V)$ 일 때 $L(0_U) = L(0_U+0_U)= L(0_U)+ L(0_U)$ 이므로 $L(0_U)=0_V$ 이다.
:::
...
$L\in \mathcal{L}(U,\,V)$ 에 대해 $L(u)=v$ 이면 $L(-u)=-v$ 이다.
...
::: {#exm-LA_derivative_integral_as_operator}
...
::: {#exm-LA_identity_operator}

#### 항등연산자
$I_U\in \mathcal{L}(U)$ 일 때 모든 $u\in u$ 에 대해 $I_U(u) = u$ 이면 $I_U$ 를 벡터공간 $U$ 에서의 **항등 연산자 (identity operator)** 라고 한다. 항등사상은 선형사상임은 자명하다. 또한 $L\in \mathcal{L}(U)$ 일 때 다음 두 명제가 동치이다.
...
::: {#prp-LA_restriction_is_linear}

선형 사상에 대한 한정 사상 역시 선형 사상이다.

:::

</div></br>

### 선형사상의 합과 스칼라곱 {#sec-LA_sum_and_scalar_product_of_linear_map}
...
$$
\begin{aligned}
(L+K)(u_1 + b u_2) &= L(u_1 + b u_2) + K(u_1+ b u_2) \\[0.3em]
&= v_1+ b v_2 +v'_1+ b v'_2  \\[0.3em]
& = (L+K)(u_1) + b  (L+K)(u_2) \\[0.3em]
(c L)( u_1 + b u_2) &= c (L(u_1+ b u_2))= c (v_1 + b v_2) \\[0.3em]
&= c v_1 + cb v_2  \\[0.3em]
...
### 선형사상의 합성 {#sec-LA_composition_of_linear-maps}
...
$$
\begin{aligned}
(L\circ K)(u_1 + c u_2) &= L(K(u_1+ c u_2)) = L(K(u_1)+ K(c u_2)) \\[0.3em]
...
$$
\begin{aligned}
(L_1 \circ L_2) \circ L_3 &= L_1 \circ  (L_2 \circ L_3) \\[0.3em]
(L_1+L_2) \circ K &= L_1 \circ K + L_2  \circ K \\[0.3em]
...
우리는 $\mathbb{F}$ 가 1차원 벡터공간임을 안다. 따라서 $\mathbb{F}$-벡터공간 $V$ 에 대해 $V\mapsto \mathbb{F}$ 인 선형사상을 생각 할 수 있다.  우리는 @prp-LA_linearmap_basis 에서 $V$ 의 기저에 대해서만 값을 알면 선형사상이 유일하게 정해진다는 것을 보였다. 이제 $V$ 의 기저 $(v_i)$ 에 대해 다음과 같이 정의된 $\varphi_j \in \mathcal{L}(V, \mathbb{F})$ 를 생각 할 수 있다.
...
### 선형사상의 행렬표현 {#sec-LA_matrix_representation_of_linear_map}
...
$$
\begin{aligned}
L(u) &= L \left( \sum_{j=1}^n a_j u_j \right) = \sum_{j=1}^n a_j L(u_j) = \sum_{j=1}^n \sum_{i=1}^m a_j A_{ij} v_i = \sum_{i=1}^m \left(\sum_{j=1}^n A_{ij}a_j\right)v_i
\end{aligned}
$$
...
우리는 $[u]_{\mathcal{B}_U} = \begin{bmatrix} a_1 \\ \vdots \\a_m \end{bmatrix}$ 임을 안다. 만약 우리가 $m \times n$ 행렬 $\bf{A}$ 와 $n \times l$ 행렬 $\bf{B}$ 의 행렬의 곱을 다음과 같이 정의한다고 하자.

$$
(\bf{AB})_{ij} := \sum_{k=1}^n A_{ik}B_{kj}
$$
...
::: {#def-LA_multiplication_of_matrix}

$m \times n$ 행렬 $\bf{A}$ 와 $n \times l$ 행렬 $\bf{B}$ 의 곱 $\bf{AB}$ 는 다음과 같이 정의된다.
...
### 선형사상의 합의 행렬표현 {#sec-LA_matrix_representation_of_sum_of_linear_map}

$L,\,K \in \mathcal{L}(U,\,V)$ 이고 $U,\,V$ 의 기저 $\mathcal{B}_U,\,\mathcal{B}_V$ 에 대해 $\bf{L}=[L]_{\mathcal{B}_U,\mathcal{B}_V}$, $\bf{K} =[K]_{\mathcal{B}_U,\mathcal{B}_V}$ 라 하자. $(L+K)(u)= L(u)+ K(u)$  이므로,
...
이다. 이로부터 $(\bf{L}+\bf{K})_{ij} = L_{ij} + K_{ij}$ 로 정의하는 것이 자연스럽다는 것을 알 수 있다.

</br>

### 선형사상의 합성의 행렬표현 {#sec-LA_matrix_representation_of_compostion_of_linear_map}

$L\in \mathcal{L}(U,\, W),\, K \in \mathcal{L}(W, V)$ 라 하자. $U,\,V,\,W$ 의 한 기저를 $\mathcal{B}_U =(u_j),\,\mathcal{B}_V = (v_i),\,\mathcal{B}_W = (w_k)$ 라 하자. 이 기저에서의 $L$ 와 $K$ 의 행렬표현을 $\bf{L},\,\bf{K}$ 라 하고, $(L\circ K)\in \mathcal{L}(U, V)$ 에 대한 행렬표현 $\bf{M}$ 를 알아보자.
...
이다. @def-LA_multiplication_of_matrix 에 의해 선형사상의 합성은 각 선형사상의 행렬표현의 곱임을 알 수 있다. $\bf{K}$ 가 $K\in \mathcal{L}(W,\,V)$ 의 행렬표현이며, $\bf{L}$ 가 $L\in \mathcal{L}(U,\,W)$ 의 행렬표현이므로 $\bf{K}$ 의 열의 갯수와 $\bf{L}$ 의 행의 갯수는 $\text{dim}(W)$ 와 같아야 한다. 

</br>

### 선형 사상의 스칼라곱  {#sec-LA_scalar_multiplication_of_linear_map}

$\mathbb{F}$-벡터공간 $U,\,V$ 에 대해 $L \in \mathcal{L}(U,\,V)$ 일 때 $[L]_{\mathcal{B}_U,\mathcal{B}_V} = \bf{L}$ 이며 $c\in \mathbb{F}$ 라 하자.

$$
(c L)(u_i) = c \left( L(u_i)\right) = \sum_j c L_{ji}v_j
$$

이므로 $(c \bf{L})_{ij} = c L_{ij}$ 임을 알 수 있다.

</br>

### 항등사상과 항등행렬 {#sec-LA_identity_matrix}

::: {.callout-note icon="false"}

#### **항등 행렬**

::: {#def-LA_identity_matrix}

벡터공간 $V$ 에 대해 $I\in \mathcal{L}(V)$ 가 $I(v)=v$ 일 때, 즉 $I$ 가 항등사상일 때 이에 대한 행렬표현 $\bf{I}=[I]$ 를 항등행렬이라 한다. 어떤 기저 $\mathcal{B}_V = \{v_i\}$ 에 대해서도 $I(v_i) = v_i$ 이므로 $(\bf{I})_{ij} = \delta_{ij}$ 이다. 차원을 같이 표기할때는 $\bf{I}_n$ 과 같이 표기하기로 한다.
:::
:::

</br>

### 행렬의 행벡터와 열벡터 {#sec-LA_column_vector_and_row_vector}

$\mathbb{F}^{1\times n}$ 과 $\mathbb{F}^n$ 이 벡터공간임을 안다. 행렬 $\bf{A}$ 의 $i,\,j$ 성분을 $A_{ij}$ 혹은 $A_{i,j}$  로 표현할 수 있듯이 $\bf{A}$ 의 $i$ 번째 행을 $\bf{A}_{i,:}$ 혹은 $\bf{A}_{i:}$ 라 쓰고 $j$ 번째 열을 $\bf{A}_{:,j}$ 혹은 $\bf{A}_{:j}$ 라 쓰기로 하자. 각각의 $\bf{A}_{:i}$ 와 $\bf{A}_{j:}$ 를 행벡터와 열벡터로 간주한다. 즉 $m\times n$ 행렬의 경우 $\bf{A}_{:i}\in \mathbb{F}^m$ 이며 $\bf{A}_{i:}\in \mathbb{F}^{1\times m}$ 이다. 많은 경우 계산을 하는데 도움이 된다.

**1.** $\bf{A}\in \mathbb{F}^{m\times n}$ 과 $\bf{B}\in \mathbb{F}^{n \times p}$ 를 생각하자. 이 때,

$$
(\bf{AB})_{ij} = \sum_k A_{ik}B_{kj} = \bf{A}_{i:}\bf{B}_{:j}
$$ 
...
$$
\begin{aligned}
(\bf{AB})_{:i} &= \bf{A}(\bf{B})_{:i} \\
(\bf{AB})_{j:} & = (\bf{A})_{j:}\bf{B}
\end{aligned}
$$
...
$$
\bf{AB} =  \begin{bmatrix} \bf{AB}_{:1} & \cdots & \bf{AB}_{:n} \end{bmatrix} =  \begin{bmatrix} \bf{A}_{1:}\bf{B} \\ \vdots \\ \bf{A}_{m:} \bf{B} \end{bmatrix}
$$

이다. 위의 개념은 많은 경우 편리하게 사용된다.

**2.** $\bf{A} \in \mathbb{F}^{m \times n}$ 와 $\bf{c}\in \mathbb{F}^{n}$ 를 생각하자. $c_i$ 를 $\bf{c}$ 의 $i$ 번째 성분이라 하면,

$$
\bf{Ac} = c_1 \bf{A}_{:1} + \cdots + c_n\bf{A}_{:n}
$$

이다. 즉 $\bf{Ac}$ 는 $\bf{A}$ 의 열벡터들의 선형결합이다.

**3.** $\bf{A} \in \mathbb{F}^{m \times n}$ 와 $\bf{d}\in \mathbb{F}^{1\times m}$ 를 생각하자. $d_i$ 를 $\bf{d}$ 의 $i$ 번째 성분이라 하면,

$$
\bf{dA} = d_1 \bf{A}_{1:} + \cdots + d_m\bf{A}_{m:}
$$

이다. 즉 $\bf{dA}$ 는 $\bf{A}$ 의 행벡터들의 선형결합이다.

</br>

## 선형사상의 kernel, image, rank, nullity

### 선형사상의 kernel 과 image {#sec-LA_kernel_and_image_of_linear_map}
...
$L\in \mathcal{L}(U,\,V)$ 에 대해 $L$ 의 kernel $\ker (L)$ 과 image $\text{im}(L)$ 는 다음과 같이 정의된다.

$$
\begin{aligned}
\ker (L) &:= \{ u\in U : L(u) = 0_V\} ,\\[0.3em]
\text{im}(L) &:= \{L(u) : u \in U\}.
\end{aligned}
$$
...
::: {#prp-LA_ker_range_subspace}

$L\in \mathcal{L}(U,\,V)$ 일 때, $\ker (L) \le U$ 이며 $\text{im} (L) \le V$ 의 부분공간이다.
...
$L\in \mathcal{L}(U,\,V)$ 에서 $\ker (L)$ 의 차원을 $\text{nullity}(L)$ 라 하고 $\text{im}(L)$ 의 차원을 $\text{rank}(L)$ 라 한다. 

$$
\begin{aligned}
\text{nullity}(L) &:= \dim (\ker (L)), \\[0.3em]
...
### Rank-nullity 정리 {#sec-LA_rank_nullity_theorem}
...
::: {.proof}
$\ker (L)$ 는 $U$ 의 부분공간이다(@prp-LA_ker_range_subspace). $\text{nullity}(L)=m$ 이라 하고 $\ker (L)$ 의 기저를 $(u_1,\ldots,u_m )$ 이라 하자. 이 기저를 확장하여 $U$ 의 기저를 이루는 벡터 $(u_1,\ldots,\,u_m,\,w_1,\ldots,w_n)$ 를 구할 수 있다(@lem-LA_basis_expansion). 그렇다면 $\dim (U) = m + n$ 이다. 이제 $\text{rank}(L)=n$ 임을 보이면 된다.
...
### 동형사상과 동형 {#sec-LA_isomorphism_of_linear_map}
...
::: {#def-LA_isomorphism_in_vector_space}

$L\in \mathcal{L}(U,\,V)$ 에 대한 역사상은 $T^{-1}$ 로 표기하며, 역변환이 존재하는 선형 사상을 **가역 사상(invertible map)** 이라고 한다. $L$ 이 가역사상이며 모든 $u_1,\,u_2\in U$ 에 대해

$$
L(u_1) + L(u_2) = L(u_1+u_2)
$$ {#eq-LA_condition_for_isomorphism}

일 때 이 $L$ 을 **동형사상(isomorphism)** 이라 한다. 그리고 두 벡터공간 사이에 가역인 사상이 존재하면 두 벡터공간이 **동형(isomorphic)** 이라고 한다. 두 벡터공간 $U,\,V$ 가 동형일 때 $U \cong V$ 라고 쓴다.

:::
:::

@eq-LA_condition_for_isomorphism 은 선형사상에 대해 당연히 성립하므로 아래의 정리를 얻을 수 있다.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-LA_equivalence_of_invertibility_and_isomorphism}

선형사상 $L$ 이 가역사상인것과 동형사상인것은 동치이다.
:::
...
::: {#prp-LA_condition_of_injectivity}

선형사상 $L\in \mathcal{L}(U,\,V)$ 가 단사일 필요충분조건은 $\text{nullity} (L) = 0$ 이다.
...
(2 $\implies$ 1) $L\in \mathcal{L}(U,\,V)$ 가 동형사상이면 $L$ 가 전사이므로 $\text{im}(L) = V$ 이어야 하며, $L$ 가 단사이므로 $\text{nullity} (L) = 0$ 이어야 한다. 선형사상의 기본정리(@thm-LA_rank_nullity) 에 의해
...
::: {#cor-LA_isomorphism_U_F}
$n$ 차원 $\mathbb{F}$-벡터공간 $U$ 와 $\mathbb{F}^n$, $\mathbb{F}^{1\times n}$ 은 동형이다.

:::

</div></br>

동형사상의 역사상에 대해 다음이 성립한다.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#lem-LA_linearity_of_inverse_map}
...
$L\in \mathcal{L}(U,\,V)$ 가 동형사상, 즉 가역사상이라고 하자. 이 때 임의의 $u\in U,\, v\in V$ 에 대해 $(L^{-1}\circ L )(u)=u$ 이고 $(L \circ L^{-1}) (v)= v$ 이다. $\mathcal{B}_U,\,\mathcal{B}_V$ 가 각각 $U,\,V$ 의 기저라고 하자. $\bf{L}=[L]_{\mathcal{B}_U, \mathcal{B}_V}$ 에 대해 $[L^{-1}]_{\mathcal{B}_V,\mathcal{B}_U} =\bf{L}^{-1}$ 이라고 쓰는 것은 자연스럽다. 또한 $\bf{LL}^{-1} = \bf{L}^{-1}\bf{L} = \bf{I}$ 라는 것을 알 수 있다. 이 때 $\bf{L}^{-1}$ 을 $\bf{L}$ 의 **역행렬 (inverse matrix)** 라 한다.

</br>

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-LA_basis-tranformations}

#### 가역사상과 기저의 변환

$L\in \mathcal{L}(V,\,W)$ 에 대해 다음 두 명제는 동치이다. 
...
(1 $\implies$ 2) $V$ 의 기저 $\{v_1,\ldots,\,v_m\}$ 에 대해 $\text{nullity} (L) = 0$ 이므로 @cor-LA_range_independence 에 따라 $\{L(v_1),\ldots,\, L(v_m)\}$ 은 선형독립이다. $L$ 이 전사이므로  $\{L(v_1),\ldots,\, L(v_m)\}$ 는 역시 $V$ 의 기저이다.
...
### 선형사상공간의 차원 {#sec-LA_dimension_of_linear_map_space}

우리는 $\mathbb{F}$-벡터공간 $U,\,V$ 에 대해 $\mathcal{L}(U,\,V)$ 가 벡터공간이라는 것을 안다(@prp-LA_LUV_vector_space). 또한 $\mathbb{F}$ 의 원소를 성분으로 갖는 $m \times n$ 행렬의 집합 $\mathbb{F}^{m \times n}$ 도 벡터공간이라는 것을 안다 (@exm-LA_vectorspace_matrix). $n = \dim (U),\, m = \dim (V)$ 이고 $\mathcal{B}_U = (u_i)$, $\mathcal{B}_V = (v_j)$ 가 각각 $U,\,V$ 의 기저라고 하자. $L\in \mathcal{L}(U,\,V)$ 의 행렬표현 $[L]_{\mathcal{B}_U,\,\mathcal{B}_V}$ 를 생각하면 $[-]_{\mathcal{B}_U,\,\mathcal{B}_V}$ 는 $\mathcal{L}(U,\,V) \mapsto \mathbb{F}^{\dim (V) \times \dim(U)}$ 함수이다. $L,\, K\in \mathcal{L}(U,\,V)$, $c \in \mathbb{F}$ 에 대해
...
$\mathbb{F}$ 에서 정의된 유한차원 벡터공간 $U,\,V$ 와 그 순서기저 $\mathcal{B}_U,\, \mathcal{B}_V$ 에 대해 $[L] = [L]_{\mathcal{B}_U,\,\mathcal{B}_V}$ 는 $\mathcal{L}(U,\,V) \to\mathbb{F}^{\dim (V) \times \dim(U)}$ 인 선형사상이다.
...
::: {#thm-LA_isomorphic}
$n$ 차원 벡터공간 $U$ 와 $m$ 차원 벡터공간 $V$ 에 대해 $\mathcal{L}(U,\,V) \cong \mathbb{F}^{m\times n}$ 와 동형이다.
:::

</div>

::: {.proof}
$U,\,V$ 의 기저를 각각 $\mathcal{B}_U = (u_i),\,\mathcal{B}_V = (v_j)$ 라 할 때 $\Phi: \mathcal{L}(U,\,V) \to \mathcal{M}_{m \times n}(\mathbb{F})$ 를 $\Phi(L) = [L]_{\mathcal{B}_U, \mathcal{B}_V}$ 로 정의한다. @lem-LA_MM_linearmap 으로부터 $\Phi$ 는 선형사상임을 안다. $\ker (\Phi) = \{0_U\}$ 이므로 단사이다. 모든 $\mathcal{M}_{m \times n}(\mathbb{F})$ 에 대해 상응하는 $L$ 가 존재하므로 전사이다. 즉 $\Phi$ 는 동형사상이다. $\square$

:::

</br>

위의 정리와 @prp-LA_same_dim_vs 로부터 다음을 알 수 있다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-LA_dimension_of_LUV}

유한차원 벡터공간 $U,\,V$ 에 대해 다음이 성립한다.

$$
\dim (\mathcal{L}(U, V)) = \dim \left(\mathbb{F}^{\dim (V) \times \dim(U)}\right) = \dim (V)\times \dim (U).
$$
...
&emsp; $1$. $U \cong \mathbb{F}^n$, $V \cong \mathbb{F}^m$

&emsp; $2$. $\mathcal{L}(U,\,V) \cong  \mathbb{F}^{m \times n}$
...
### 동형사상인 연산자의 성질 {#sec-LA_properties_of_isomorphism}

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-LA_isomorphic_linear_map}
...
::: {.proof}
함수가 가역이면 전단사이고, 전단사 이면 가역이다. 따라서 $L$ 가 전사라는 것과 단사라는 것이 동치임을 보이면 된다. Rank-nullity 정리(@thm-LA_rank_nullity) 로부터
...
우리는 $\mathbb{F}$ 가 벡터공간임을 안다. 위의 @def-LA_product_space 의 정의에 따라 곱공간 $\mathbb{F}^n$ 을 벡터공간으로 볼 수 있다. 실제로 우리가 수학 및 여러 다양한 분야에서 사용하는 $\mathbb{F}^n$ 은 벡터공간으로서의 $\mathbb{F}^n$ 이며 실제로는 앞으로 배울 내적이 정의된 내적벡터공간으로서의 $\mathbb{F}^n$ 이다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_dimension_of_product_space}

$V_1,\ldots,\,V_m$ 이 유한차원 벡터공간일 때 곱공간의 차원은 아래와 같다.
$$
...
$\left(v^{(i)}_1,\ldots,\,v^{(i)}_{m_i}\right)$ 를 $V_i$ 의 기저라고 하면 $\dim (V_i)=m_i$ 이다. $v\in V_1 \times \cdots \times V_m$ 의 $i$ 번째 성분을 기술하는데 $m_i$ 개의 기저벡터가 필요하므로 $V_1 \times \cdots \times V_m$ 를 기술하는 데는 $\dim V_1 + \cdots + \dim V_m$ 개의 벡터가 필요하다. $\square$ 

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_map_product_space}
...
::: {#prp-LA_product_space_and_direct_sum}
...
@thm-LA_directsum 에 의해 $U_1 + \cdots + U_m$ 이 직합일 필요충분조건은 $0$ 을 각각의 $u_i\in U_i$ 의 합으로 나타내는 방법이 모든 $u_i=0$ 인 것 뿐이다. 따라서, 
...
::: {.proof}
@prp-LA_map_product_space 에서 정의된 $\Phi$ 를 생각한다. @prp-LA_product_space_and_direct_sum 에 의해 $U_1+\cdots + U_m$ 이 직합일 필요충분조건이 $\Phi$ 가 단사인 것이며, $U_1 \times \cdots \times U_m$ 과 $U_1 + \cdots + U_m$ 이 유한차원 벡터공간이므로 $\Phi$ 가 단사인 것의 필요충분조건은 
$$
\dim (U_1 + \cdots + U_m) = \dim U_1 + \cdots + \dim U_m
$$
이다. $\square$
...
### 기저의 변화에 따른 행렬의 변화 {#sec-LA_matrix_transform_by_basis_transform}

$U$ 가 $n$ 차원 $\mathbb{F}$-벡터공간이며 $\mathcal{B}_1 = (u_1,\ldots,u_n)$ 이 $U$ 의 기저라고 하자. $P\in \mathcal{L}(U)$ 가 가역이면 @cor-LA_basis_transformation 에서 보았듯이 $\mathcal{B}_2= (P(u_1),\ldots, P(u_n) )$ 도 $U$ 의 기저이다. $v_i = P(u_i)$ 라면 $u_i = P^{-1}(P(u_i)) = P^{-1}(v_i)$ 이다. $\bf{u}_i=[u_i]_{\mathcal{B}_1}$, $\bf{v}_i = [v_i]_{\mathcal{B}_1}$, $\bf{P}=[P]_{\mathcal{B}_1}$ 라면 $\bf{v}_i = \bf{Pu}_i$ 이다. 그런데 $\bf{u}_i =[u_i]_{\mathcal{B}_1}$ 은 표준기저 $\bf{e}_i$ 이므로 $\bf{v}_i$ 는 $\bf{P}$ 의 $i$ 번 째 열 $\bf{P}_{:i}$ 이다. 이것은 앞으로도 매우 중요하므로 뒤에 별도로 다시 언급하겠다.

이제 $\mathcal{B}_2$ 기저에서 $u_i$ 와 $v_i$ 를 표현해보자. $[v_i]_{\mathcal{B}_2} = \bf{e}_i$ 임은 자명하다. 

$$
\begin{aligned}
u_i &= \sum_j \delta_{ji}u_j = \sum_j (\bf{P}\bf{P}^{-1})_{ji}u_j \\
&= \sum_j \sum_k P_{jk}P^{-1}_{ki}u_j = \sum_k P^{-1}_{ki} \left(\sum_{j} P_{jk}u_j\right) \\
&= \sum_k P_{ki}^{-1} v_k 
\end{aligned}
$$

이므로 $[u_i]_{\mathcal{B}_2} = \bf{P}^{-1}[v_i]_{\mathcal{B}_2}$ 이다. 즉 $[u_i]_{\mathcal{B}_2}$ 는 $\bf{P}^{-1}$ 의 $i$ 번째 열이다.


이제 연산자 $L\in \mathcal{L}(U)$ 이 $L(u_i) = \sum_j A_{ij}u_j$ 로 정의되었으며 이 연산자의 $\mathcal{B}_1$ 기저에서의 행렬표현을 $\bf{L}$ 라 하자. 즉 $\bf{L} = [L]_{\mathcal{B}_1}$ 이다. 이 때,

$$
\begin{aligned}
L(v_i) &= L(P(u_i)) = TL\left( \sum_{k=1}^n  P_{ki}u_k\right) = \sum_{k=1}^n P_{ki} L(u_k) \\
&= \sum_k P_{ki} \left(  \sum_l L_{lk}u_l \right)= \sum_k P_{ki}\left(\sum_l L_{lk} \sum_{j} (P^{-1})_{jl} v_j\right) \\
&= \sum_j \left( \sum_l \sum_k (P^{-1})_{jl} L_{lk} P_{ki}\right) v_j
\end{aligned}
$$

이다. 이로부터 $[L]_{\mathcal{B}_2} = \bf{P}^{-1}\bf{LP}$ 임을 알 수 있다. 즉 두 기저 $\mathcal{B}_1 =\{u_i\}$, $\mathcal{B}_2 = \{P(u_i)\}$ 사이에,

$$
[L]_{\mathcal{B_1}} = \bf{L} \iff [L]_{\mathcal{B}_2} = \bf{P}^{-1}\bf{LP}
$$
...
::: {#def-LA_similarity_transformation_of_linear_operator}
...
::: {#def-LA_similarity_transformation}

행렬 $\bf{A},\,\bf{B}\in \mathcal{M}_{n\times n}(\mathbb{F})$ 에 대해 어떤 가역행렬 $\bf{P}\in \mathcal{M}_{n\times n}(\mathbb{F})$ 이 존재하여 $\bf{A}=\bf{P}^{-1}\bf{BP}$ 가 성립할 때 두 행렬 $\bf{A}$ 와 $\bf{B}$ 는 닮았다고 하며 $\bf{P}^{-1}\bf{AP}$ 를 $\bf{A}$ 의 **닮음변환(similarity transformation)** 이라고 한다. 
...
우리가 이미 보였지만 다음 명제는 별도로 언급할 가치가 있다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_basis_transoframtion_by_similarity_transformation}
$\bf{P}$ 가 가역행렬일 때 $\bf{P}$ 의 각 열은 표준기저가 이 변환에 의해 변환된 새로운 기저이다.
:::

</div>

::: {.proof}
표준기저 $(\bf{e}_i)$ 에 대해 $\bf{Pe}_i = \bf{P}_{:i}$ 이다. $\bf{P}$ 가 가역행렬이므로 열벡터의 집합은 선형독립이다. $\square$ 
:::

</br><div class="border" style="background-color:#F2F4F4  ;padding:5px;">

::: {#exm-LA_similarity_equivalence}

두 연산자 $L,\, K$ 가 닮았을 때 $L \sim K$ 라 표현한다고 하자. 이 때 다음이 성립함을 보여라.

&emsp;($1$) $L \sim L$ 이다.

&emsp;($2$) $L \sim K$ 이면 $K \sim L$ 이다.

&emsp;($3$) $L \sim K$ 이고 $K \sim M$ 이면 $M \sim L$ 이다.
...
$V$ 가 유한차원 벡터공간이고 $L\in \mathcal{L}(V, W)$ 이다. $V$ 의 어떤 부분공간 $U$ 는 $U \cap \ker (L)=\{0\}$ 이며 $\text{im}\,(L)=\{L(u) : u \in U\}$ 임을 보여라.
:::
...
$\ker (L)$ 의 기저 $(v_1,\ldots,\,v_m)$ 을 확장하여 $V$ 의 기저 $(v_1,\ldots,\,v_m,\,u_1,\ldots,\,u_n)$ 을 구성 할 수 있다(@lem-LA_basis_expansion). $U=\text{span}(u_1,\ldots,\,u_n)$ 이라 놓으면 된다. 

:::

</br><div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-LA_dimension_and_injectivity_surjectivity}
...
::: {#exr-LA_inverse_image_of_linear_map}
...
($1 \implies 2$) $U$ 의 기저 $(u_1,\ldots,\,u_m )$ 에 대해 $v_i=S(u_i)$ 라고 하자. $S$ 가 동형이므로 $\{v_1,\ldots,\,v_m \}$ 도 선형 독립이다. $\{u_1,\ldots,\,u_m\}$ 을 확장하여 $V$ 의 기저 $\{u_1,\ldots,\,u_m,\,u_{m+1}, \ldots, v_n\}$ 을 구성 할 수 있으며, $\{v_1,\ldots,\,v_m\}$ 을 확장하여 $V$ 의 기저 $\{v_1,\ldots,\,v_m,\,v_{m+1}, \ldots, v_m\}$ 을 구성 할 수 있다. 이 때 $T\in \mathcal{L}(V)$ 를 $T(u_i) = v_i$ $(i=1,\ldots,\,m)$ 으로 정의하면 된다.
...
$L$ 이 $cI$ 이면 $SL = LS$ 이다. 이제 $L$ 이 항등사상의 스칼라곱 아니라고 가정하자. $V$ 의 기저 $\mathcal{B}=(u_i)$ 에 대해 $Lu_i \notin \text{span}(u_i)$ 인 $u_i\in \mathcal{B}$ 가 존재한다. 일반성을 잃지 않고 $Lu_1\notin \text{span}(u_1)$ 이라고 할 수 있다. $S(u_1) = u_1$ 이며 $L(u_1)\in \ker S$ 인 $S$ 가 존재하며 $LS(u_i)\ne 0_V$ 이지만 $SL(u_i) = 0_V$ 이므로 $SL \ne LS$ 이다. 
