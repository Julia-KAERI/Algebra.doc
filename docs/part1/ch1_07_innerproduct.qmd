...
우리는 앞서 @prp-norm_of_innerproduct_space 에서 내적벡터공간에는 자연스러운 거리함수가 존재한다는 것을 보였다. 거리공간과 내적공간의 관계에 대해 말해주는 정리가 조르당 폰 노이만 정리이다.
...
($2 \implies 1$) 우선 $\mathbb{R}$-거리공간에서 보이자. 함수 $\phi : V \times V \to \mathbb{F}$ 를 
...
임을 안다. 즉 $c\in \mathbb{Z}_+$ 일 때 $\phi (cu, v)=c\phi (u, v)$ 이다. $c=0$ 이면 $\phi (0, v)=0 = 0\cdot \phi(u, v)$ 이다. $c\in \mathbb{Z}_-$ (음의 정수의 집합) 라고 하자. $c<0$ 일 때 성립함을 가정했을 때 $c-1$ 에서 성립한다면 모든 음의 정수에 대해 성립한다는 것을 안다. 

$$
\phi ((c-1)u, v)= \phi (cu-u, v)= \phi (cu, v) + \phi (-u, v)= (c-1)\phi (u, v)
$$

이므로 음의 정수에 대해서 성립한다. 즉 모든 정수 $c$ 에 대해 $\phi (cu, v)= c\phi (u, v)$ 이다.

이제 유리수에 대해 성립한다는 것을 보이자. $m, n\in \mathbb{Z}$ 에 대해 $c=m/n$ 이라 하자. 

$$
n\phi (cu, v) = \phi (ncu, v)= \phi (mu, v)= m \phi (u, v)= cn \phi(u, v)
$$

이므로 $\phi (cu, v) = c\phi (u, v)$ 이다.
