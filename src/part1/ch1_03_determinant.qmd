...
::: {#def-part1_elementary_row_operation}
...
### 기본 행 연산의 행렬 표현 {#sec-part1_matrix_representatin_of_elementary_row_operation}
...
### 순열의 표기 {#sec-part1_notation_of_permutations}
...
### 순열의 분해 {#sec-part1_decomposition_of_permutation}
...
### 순열에 대한 $\text{sgn}$ 함수 {#sec-part1_sgn_for_permutation}
...
::: {#thm-part1_permutation_even_odd}
...
$\boldsymbol{A}\in \mathcal{M}_{n\times n}(\mathbb{F})$ 라고 하자. @def-part1_elementary_row_operation 에서 정의된 행 기본연산 $E^{(1)},\, E^{(2)},\, E^{(3)}$ 에 대해 다음이 성립한다.
...
$\boldsymbol{S}$ 가 가역행렬이므로 $\boldsymbol{Av}=\boldsymbol{0} \iff \boldsymbol{SAv}=\boldsymbol{0}$ 이다. 즉 $\ker \boldsymbol{A} = \ker (\boldsymbol{SA})$ 이며 rank-nullity 정리(@thm-rank_nullity) 에 의해 $\text{rank} (\boldsymbol{A}) = \text{rank} (\boldsymbol{SA})$ 이다. $\square$ 
