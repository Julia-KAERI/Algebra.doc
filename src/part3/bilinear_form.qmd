...
대칭행렬 $\boldsymbol{J}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 가 가역일 때 $\boldsymbol{A}\in O_n^\boldsymbol{J}(\mathbb{F})$ 의 행렬식은 $\pm 1$ 이다.

:::

::: {.solution}

$\det(\boldsymbol{A}^T\boldsymbol{JA}) = \boldsymbol{J}$ 이며 $\det (\boldsymbol{A}^T) = \det (\boldsymbol{A})$ 이므로 $\det (\boldsymbol{A})= \pm 1$ 이다.
