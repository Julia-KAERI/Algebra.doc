...
이면 $U$ 를 $L$ 에 대한 $V$ 의 **불변부분공간(invariant subspace)**, 혹은 $L$-**불변부분공간** 이라고 한다.
:::
:::
...
::: {#prp-LA_invariant_subspace_and_eigenvector}

$L\in \mathcal{L}(V)$ 이고 $U$ 가 $L$-불변 부분공간이라고 하자. 서로 다른 고유값을 가지는 $L$ 의 고유벡터 $v_1,\ldots,\,v_k$ 에 대해 $v_1+\cdots + v_k\in U$ 이면 각각의 $v_1,\ldots,\,v_k \in U$ 이다.
...
$Lv_i = c_iv_i$ 라고 하자. $k$ 에 대한 induction 으로 증명한다. $k=1$ 일 경우는 자명하다. 이제 $k>1$ 에 대해 생각하자. $v=v_1+\cdots + v_k$ 에 대해 $Lv\in U$ 라고 하자. 그렇다면 
$$
Lv-c_kv=(c_1−c_k)v_1 + \cdots + (c_{k-1}−c_k)v_{k-1}
$$

이다. $v\in U,\, Lv\in U$ 이므로 $(c_1−c_k)v_1 + \cdots + (c_{k-1}−c_k)v_{k-1}\in U$ 이 induction 에 대한 가정으로 $v_1,\ldots,\,v_{k-1}\in U$ 이다. $\square$
...
$\bf{D}\in \F^{n\times n}$ 가 대각행렬, 즉 대각성분을 제외한 모든 성분이 $0$ 인 행렬이라고 하자. 표준기저 $\hat{\bf{e}}_i$ 에 대해 $\bf{D}\hat{\bf{e}}_i = D_{ii} \hat{\bf{e}}_i$ 이므로 $\hat{\bf{e}}_i$ 는 $D_{ii}$ 를 고유값으로 갖는 $\bf{D}$ 의 고유벡터이다. 
...
유한차원 $\F$-벡터공간에서 정의된 $L\in \mathcal{L}(V)$ 에 대해 $\det (\lambda I - L) = 0$ 을 만족하는 $\lambda\in \F$ 는 $L$ 의 고유값이며 이 $\lambda$ 를 고유값으로 갖는 고유벡터는 항상 존재한다.
...
$\det (\lambda I-L)  = 0$ 이면 $\text{nullity} (\lambda I-L)  \ne 0$ 이므로 $Lv=\lambda v$ 를 만족하는 영벡터가 아닌 벡터 $v$ 가 항상 존재한다. $\square$
...
$\{v_1,\ldots,\,v_m \} \subset V$ 이 선형독립일 필요충분 조건은 위 벡터들이 서로 다른 고유값을 갖는 고유벡터가 되도록 하는 $L\in \mathcal{L}(V)$ 가 존재하는 것임을 보여라.
...
서로 다른 고유값을 갖는 고유벡터의 집합은 선형독립이라는 것을 안다(@prp-LA_eigenvector_linear_independence). 따라서 $\{v_1,\ldots,\,v_m \}$ 가 선형독립이면 이 벡터들이 각각 다른 고유값을 갖는 고유벡터가 되는 연산자 $L$ 이 존재함을 보이면 된다. $v_1,\ldots,\,v_m$ 을 확장하여 기저 $\mathcal{B}=\{v_1,\ldots,\,v_n\}$ 을 구성할 수 있다. $Lv_i = iv_i (i=1,\ldots,\,n)$ 인 선형연산자가 그것이다.
...
$\bf{A}\in \F^{n\times n}$ 이고 $T\in \mathcal{L}(\F^n)$ 가 $T(\bf{x}) = \bf{Ax}$ 라고 하자.
...
정사각행렬 $\bf{A}\in \mathbb{F}^{n\times n}$ 에 대해 $p_{\bf{A}}(\lambda) := \det (\lambda \bf{I}-\bf{A})$ 를 $\bf{A}$ 의 **특성 다항식(characteristic polynomial)** 이라 한다.
...
$\bf{A}\in \mathbb{F}^{n\times n}$ 에 대한 특성다항식은 $n$ 차 다항식이다.
:::
...
특성 다항식은 닮음 변환에 대해 불변이다. 즉 행렬 $\bf{A}\in \mathbb{F}^{n\times n}$ 과 가역행렬 $\bf{P}\in \F^{n\times n}$ 에 대해 $p_{\bf{A}}(\lambda) = p_{\bf{P^{-1}AP}}(\lambda)$ 이다.
:::
...
[블록행렬](ch1_03_determinant.qmd#def-LA_block_matrix)의 대각 부분 행렬이 정사각행렬이라면, 행렬의 모든 대각성분은 부분행렬의 대각성분이 된다. 이를 대각 부분행렬이라 하자. 대각 부분행렬을 제외한 부분행렬이 모두 $0$ 행렬인 경우 **블록 대각 행렬 (block diagonal matrix)** 라 한다. 상삼각 행렬/하삼각 행렬처럼 **블록 상삼각행렬(block upper triangular matrix)** 와 **블록 하삼각행렬(block lower triangular matrix)** 를 정의 할 수 있다.

즉 $\bf{A}=\F^{m\times m}, \bf{B} = \F^{n\times n},\, \bf{C}=\F^{m\times n},\, \bf{D}= \F^{n\times m}$ 에 대해

$$
\begin{bmatrix} \bf{A} & \bf{C} \\ \bf{0} & \bf{B}\end{bmatrix}
$$

형태인 행렬을 블럭 상삼각행렬 이라고 하고

$$
\begin{bmatrix} \bf{A} & \bf{0} \\ \bf{D} & \bf{B}\end{bmatrix}
$$

형태인 행렬을 블럭 하삼각행렬 이라고 한다.

</br>

### 불변 부분공간과 블럭 상삼각행렬 {#sec-LA_invariant_subspace_and_block_upper_triangular_matrix}

우선 @sec-LA_matrix_transform_by_basis_transform 에서 본 행렬의 닮음 변환을 다시 한번 상기하자. $\F^n$ 공간에서 정의된 선형독립인 벡터 $\{\bf{u}_1,\ldots,\, \bf{u}_n\}$ 와 $\bf{A}\in \mathbb{F}^{n\times n}$ 을 생각하자. $\bf{u}_i$ 가 $i$ 번째 열이 된 행렬 $\bf{P} = \begin{bmatrix} \bf{u}_1 & \cdots & \bf{u}_n \end{bmatrix}$ 는 기저를 변환하는 행렬, 즉 닮음변환을 정의하는 행렬이 되며 $\bf{P}^{-1}\bf{AP}$ 는 변환된 기저에서의 선형사상이고 변환행렬 $\bf{P}= \begin{bmatrix} \bf{u}_1 & \cdots & \bf{u}_n\end{bmatrix}$ 가 정의되며 아래 표와 같이 변환된다.
...
행렬 $\bf{A}\in \mathbb{F}^{n\times n}$ 에 대해 불변부분공간이 존재하면 $\bf{A}$ 와 닮은 블럭 상삼각 행렬이 존재한다.
...
$\mathbb{F}$-벡터공간 $\mathbb{F}^n$ 의 어떤 부분공간 $U$ 와 행렬 $\bf{A}\in \mathbb{F}^{n\times n}$ 에 대해 $U$ 가 $\bf{A}$ 에 대한 불변부분공간이라고 하자. $U$ 의 기저 $\{\bf{u}_1,\ldots,\,\bf{u}_m\}$ 를 확장하여 $\mathbb{F}^n$ 의 기저 $\{\bf{u}_1,\ldots,\bf{u}_n\}$ 을 구성했다고 하자. 그렇다면 
...
정사각 행렬 $\bf{A}\in \mathbb{F}^{n\times n}$ 에 대해 다음은 동치이다.

&emsp;($1$) $\bf{A}$ 가 어떤 상삼각행렬과 닮음이다.

&emsp;($2$) $\mathbb{F}^n$ 의 어떤 기저 $(\bf{v}_1,\ldots,\,\bf{v}_n)$ 가 존재하여 $\bf{Av}_i = \text{span}(\bf{v}_1,\ldots,\,\bf{v}_i)$ 이다.
...
복소수체에서 정의된 행렬 $\bf{A}\in \mathbb{C}^{n\times n}$ 는 닮은 상삼각행렬이 존재한다. 
:::

</div>

::: {.proof}
$\bf{A}_n \in \mathbb{C}^{n\times n}$ 이라 하고 $n$ 에 대한 귀납법으로 증명한다. $n=1$ 일 때는 자명하다. $k<n$ 에 대해 위의 명제가 성립한다고 가정하자. 우리는 @thm-LA_existance_of_eigenvector_for_complex_field 를 통해 행렬 $\bf{A}_n$ 에 대한 고유값 $\lambda$ 와 고유벡터 $\bf{v}_1$ 이 존재한다는 것을 안다. $\bf{v}_1$ 을 확장하여 $\mathbb{F}^n$ 의 기저를 구성 할 수 있으며, 이 기저로 변환된  블럭 상삼각행렬을 이룬다 (@prp-LA_invariant_subspace_block_upperdiagonal_matrix). 이 블럭 상삼각 행렬의 1 행과 1 열을 제거한 $(n-1)\times (n-1)$ 행렬은 가정에 의해 어떤 상 삼각 행렬과 닮았다. 따라서 $\bf{A}_n$ 은 어떤 상 삼각행렬과 닮았다. $\square$ 
:::

</br>

연산자의 행렬표현을 생각하면 아래의 결론을 얻는다

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-LA_uppertriangular_matrix_representation_over_complex_field}
유한차원 $\C$-벡터공간 $V$ 에서 정의된 연산자 $L\in \mathcal{L}(V)$ 는 어떤 기저에서 상삼각행렬 표현을 갖는다.
:::
...
### 대각화 가능한 연산자 {#sec-LA_diagonalizable_matrix}
...
연산자 $L$ 이 어떤 기저에서의 행렬표현이 대각행렬일 때 이 연산자를 **대각화 가능하다(be diagonalizable)** 라고 한다.
...
유한차원 벡터공간에서의 연산자 $L\in \mathcal{L}(V)$ 이 서로 다른 고유값 $\lambda_1,\ldots,\,\lambda_m$ 을 가질 때 다음은 동치이다.


&emsp;($1$) $L$ 는 대각화 가능하다.

&emsp;($2$) $L$ 의 고유벡터로 이루어진 $V$ 의 기저가 존재한다.

&emsp;($3$) $V= E(\lambda_1, L) \oplus \cdots \oplus E(\lambda_m, L)$
...
($1\implies 2$) $n=\dim (V)$ 일 때 기저 $\mathcal{B}=(v_1,\ldots,\,v_n)$ 에 대해 $[L]_{\mathcal{B}}$ 가 대각행렬이라고하자. $[L]_\mathcal{B}\hat{\bf{e}}_i=\lambda_i \hat{\bf{e}}_i$ 이므로 $\mathcal{B}$ 는 $L$ 의 고유벡터로 이루어진 $V$ 의 기저이다.

($2\implies 3$) trivial.

($3\implies 1$) 각각의 $E(\lambda_i,\,L)$ 의 기저를 $\mathcal{B}_i=\left(v^{(i)}_j\right)$ 라고 하자. $\mathcal{B}=\bigcup_{i=1}^m \mathcal{B}_j$ 는 $V$ 의 기저이며 $L(v^{(i)}_j) = \lambda_jv^{(i)}_j$ 이므로 $[L]_\mathcal{B}$ 는 대각행렬이다. $\square$
...
$L\in \mathcal{L}(V)$ 이고 $\dim (V) = n$ 일 때 $L$ 에 대해 서로 다른 $n$ 개의 고유값이 존재하면 $L$ 는 대각화 가능하다.
...
@prp-LA_eigenvector_linear_independence 에서 보였듯이 서로 다른 고유값을 갖는 고유벡터는 선형독립이므로 $L$ 은 대각화 가능하다. $\square$
:::
...
$n$ 차원 $\F$-벡터공간에서 $V$ 에서 정의된 $L \in \mathcal{L}(V)$ 의 특성다항식 $p_{L}$ 이 $n$ 개의 서로 다른 해를 $\mathbb{F}$ 에서 가지면 $L$ 는 대각화 가능 행렬이다. 
:::
...
::: {#prp-LA_diagoalizability_to_invariant_subspace}

유한차원 벡터공간 $V$ 에서 정의된 연산자 $L$ 이 대각화 가능하며 $U\le V$ 가 $L$-불변 부분공간일 때 $L$ 의 $U$ 에의 제한인 $L|_U$ 는 $U$ 에서 대각화 가능하다.
...
$L$ 을 대각화 하는 기저 $\mathcal{B}=(v_1,\ldots,v_n)$ 을 생각하자. 우선 모든 $v_i$ 에 대한 고유값이 다르다면 @prp-LA_invariant_subspace_and_eigenvector 에 의해 $U$ 는 $U$ 에 속하는 $\mathcal{B}$ 의 부분집합을 기저로 가지며 이 기저에서 $L|_U$ 는 대각화된다. 

이제 $\mathcal{B}$ 에 같은 고유값을 갖는 다른 고유벡터가 존재하는 경우를 보자. $U$ 역시 벡터공간이므로 같은 고유값을 같은 $U$ 의 벡터의 집합은 $U$ 의 부분공간이며, 이를 $E_U(c_i,\,L|_U)$ 라고 할 수 있으며 $U=\bigoplus E_U(c_i,L|_U)$ 이다. 즉 $L|_U$ 는 $U$ 에서 대각화 가능하다. $\square$
:::
...
#### Axler 5.B.1
$L\in \mathcal{L}(V)$ 가 어떤 자연수 $n$ 에 대해 $L^n=0$ 이라고 하자. $I-L$ 는 가역행렬이며 다음이 성립함을 보여라.

$$
(I-L)^{-1} = I +  L + \cdots + L^{n-1}.
$$
...
$P\in \mathcal{L}(V)$ 가 $P^2=P$ 일 때 $P$ 를 **사영연산자(projection)** 이라고 한다. $P$ 가 projection 일 때 $V=(\ker (P)) \oplus (\text{im}(P))$ 임을 보여라.
...
$u\in \ker (P)\cap \text{im}(P)$ 라 하자. $P(u)=0_V$ 이며 $P(v)=u$ 인 $u$ 가 존재해야 한다. 그런데 $P^2(v) = P(u)=0_V$ 이며 $P^2(v) = P(v)$ 이므로 $u=0_V$ 이다. 즉 $\ker (P)\cap \text{im}(P) = \{0_V\}$ 이다. @thm-LA_rank_nullity 에 의해 $V=(\ker (P)) \oplus (\text{im}(P))$ 이다.
...
$L\in \mathcal{L}(V)$ 가 대각화 가능하면 $V=\ker (L)\oplus \text{im}(L)$ 임을 보여라.
...
@thm-LA_diagonalizability 로부터 $\ker (L) = E(0,\,L)$ 이며 $\text{im}(L)= \bigoplus_{i=1,\, \lambda_i \ne 0}^n E(\lambda_i,\,L)$ 임을 안다.
...
유한차원 벡터공간 $V$ 에서 정의된 연산자 $L$ 에 대해 다음 세 명제는 동치임을 보여라.

&emsp;($1$) $V = \ker (L)\oplus \text{im}(L)$.

&emsp;($2$) $V = \ker (L) + \text{im}(L)$.

&emsp;($3$) $\ker (L) \cap \text{im}(L) = \{0_V\}$.
...
($1\implies 2$) $\oplus$ 의 정의에 의해 $\ker (L)\cap \text{im}(A)= \{0_V\}$ 이다. @prp-LA_condition_for_direct_sum 의해 $\ker (L) + \text{im}(L) = \ker (L)\oplus \text{im}(L) = V$ 이다.

($2\implies 3$) @thm-LA_rank_nullity 에 의해 $\ker (L) \cap \text{im}(L) = \{0_V\}$ 이다. 

($3 \implies 1$) $\ker (L) \le V$ 과 $\text{im}(L) \le V$ 이며 $\ker (L) + \text{im}(L) = \ker (L) \oplus \text{im}(L)$ 도 $V$ 의 부분공간이다. @thm-LA_rank_nullity 에 의해 $V = \ker (L)\oplus \text{im}(L)$ 이다.
...
$\bf{A}\in \mathbb{C}^{n\times n}$ 일 때 다음이 동치임을 보이시오.

&emsp;($1$) $\bf{A}$ 는 대각화 가능 행렬이다.

&emsp;($2$) 모든 $\lambda \in \mathbb{C}$ 에 대해 $\mathbb{C}^n = \ker (\bf{A}-\lambda \bf{I}_n) \oplus \text{im}(\bf{A}-\lambda \bf{I}_n)$
...
::: {.solution}
($1 \implies 2$) $\bf{A}\in \mathbb{C}^{n\times n}$ 일 때
...
이므로 $\bf{u}$ 가 $\bf{A}$ 의 고유벡터인것과 $\bf{A}-\lambda\bf{I}_n$ 의 고유벡터인것은 동치임을 안다. 따라서 $\bf{A}$ 가 대각화 가능인것과 임의의 $\lambda\in \mathbb{C}$ 에 대해 $\bf{A}-\lambda \bf{I}_n$ 이 대각화 가능인것은 동치이다. $\bf{A}-\lambda\bf{I}_n$ 이 대각화 가능이므로 @exr-axler_5_c_1 에서 보였듯이 $\mathbb{C}^n = \ker (\bf{A}-\lambda\bf{I}_n) \oplus \text{im} (\bf{A}-\lambda\bf{I}_n)$ 이다. 
...
&emsp;($3$) $\bf{F}$ 의 고유벡터로 이루어진 $\mathbb{R}^2$ 의 기저를 구하라.
...
($3$) $\bf{Fu}_i = \lambda_i \bf{u}_i$ 를 통해, $\bf{u}_1 = \begin{bmatrix}1 \\ \frac{1+\sqrt{5}}2\end{bmatrix}$, $\bf{u}_2 = \begin{bmatrix}1 \\ \frac{1-\sqrt{5}}{2}\end{bmatrix}$ 를 얻으며 둘은 선형독립이므로 $\mathbb{R}^2$ 의 기저이다.
...
::: {#def-LA_commuting_operator}

#### 교환가능한 연산자와 행렬

두 연산자 $L,\,K\in \mathcal{L}(V)$ 가 $LK=KL$ 일 때 $L$ 과 $K$ 는 **교환가능하다(be commutable)** 고 한다. 두 행렬 $\bf{L},\, \bf{K} \in \F^{n\times n}$ 가 $\bf{LK}=\bf{KL}$ 일 때 두 행렬이 교환 가능하다고 한다.
...
::: {#prp-LA_matrix_representation_of_compatible_matrix}

두 연산자 $L,\,K\in \mathcal{L}(V)$ 가 교환가능하면 두 연산자의 행렬 표현도 교환가능하다.
...
$0$ 연산자의 행렬표현은 $\bf{0}$ 행렬이므로 $LK-KL=0$ 이면 $[L][K]-[K][L]=\bf{0}$ 이다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_compatible_matrices_and_invariant_subspace}

교환 가능한 두 연산자 $L,\,K\in \mathcal{L}(V)$ 에 대해 $L$ 의 고유공간은 $K$ 에 대한 불변 부분공간이다.
...
$v\in E(c,\, L)$ 이라고 하자. $LKv = KLv = cKv$ 이므로 $Kv\in E(c,\, L)$ 이다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-LA_simultaneous_diagonalizability_and_commutativity}

유한차원 벡터공간에서의 두 연산자에 대해 두 연산자가 같은 기저에서 대각화 가능할 필요충분조건은 두 연산자는 교환가능한 연산자인 것이다.
:::
...
우선 $L,\,K\in \mathcal{L}(V)$ 가 $\mathcal{B}=(v_1,\ldots,\,v_n)$ 에서 동시에 대각화 가능하다고 하자. 즉 $Lv_i = l_i v_i,\, Kv_i = k_iv_i$ 라고 하자. $(LK-KL)v_i = 0$ 이므로 $LK-KL=0$ 이다.

두 연산자가 교환가능함을 가정하자. $K$ 가 대각화 가능 연산자이므로 $K$ 에 대한 고유값이 $k_1,\ldots,\,k_m$ 일 때 $V=\bigoplus_{k=1}^m E(k_i,\, K)$ 이다. @prp-LA_compatible_matrices_and_invariant_subspace 에 의해 $E(k_i,\,K)$ 는 $L$ 에 대한 불변부분공간이며 @prp-LA_diagoalizability_to_invariant_subspace 에 따라 각각의 $E(k_i,\, K)$ 에서 $L|_{E(k_i,\,K)}$ 은 대각화 가능하다. 각각의 $E(k_i,\,K)$ 에서 $L|_{E(k_i,\,K)}$ 를 대각화 하는 부분공간의 기저를 모으고 이을 $\mathcal{B}=(v_1,\ldots,\,v_n)$ 이라고 하면 이 이 기저는 $L$ 을 대각화하므로 어떤 $li\in \F$ 에 대해 $Lv_i = l_i v_i$ 이며 $K$ 의 고유벡터이므로 역시 어떤 $k_i\in \F$ 에 대해 $Kv_i = k_i v_i$ 이다. 즉 $[L]_\mathcal{B}$ 와 $[K]_\mathcal{B}$ 는 대각행렬이다. $\square$

:::

</br>

위의 정리는 대각화 가능한 두 연산자가 교환가능한 것과 공통으로 대각화하는 기저가 존재하는 것이 동치임을 보인다. 아래 명제는 대각화 가능하지 않더라도 유한차원 $\C$-벡터공간에서는 두 연산자가 교환가능할 경우 공통의 고유벡터를 가진다는 것을 말한다.

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-LA_common_eigenvectors_for_commuting_operators}

유한차원 $\C$-벡터공간에서 정의된 두 연산자 $L,\,K$ 에 대해 $LK=KL$ 이면 두 연산자에 공통인 고유벡터가 존재한다.
:::
...
@thm-LA_existance_of_eigenvector_for_complex_field 에 의해 $L$ 과 $K$ 는 각각 고유벡터가 존재하며 따라서 $E(k_i,\,K) \ne \varnothing$ 인 $k_i$ 가 존재한다. 두 연산자가 교환가능하므로 @prp-LA_compatible_matrices_and_invariant_subspace 에 의해 $E(k_i,\,K)$ 는 $L$ 에 대한 불변부분공간이며 @prp-LA_diagoalizability_to_invariant_subspace 에 따라 각각의 $E(k_i,\, K)$ 에서 $L|_{E(k_i,\,K)}$ 은 대각화 가능하다. $\square$
