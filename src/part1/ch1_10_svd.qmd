...
($2$) $u\in \ker(A^\ast A) \implies \langle A^\ast Au,\, v\rangle = \|Au\|^2=0$ 이므로 $u\in \ker (A)$. 역으로 $u\in \ker A$ 이면 $Au=0$ 이므로 $A^\ast A u=0$ 이며, 따라서 $u\in \ker (A^\ast A)$. 따라서 $\ker (A^\ast A) = \ker (A)$.
...
$L=0$ 이면 $L^\ast L=0$ 이므로 $L$ 의 모든 특이값이 $0$ 이다. $L$ 의 모든 특이값이 $0$ 이면 @prp-properties_of_singular_value ($2$) 에 의해 $\text{rank}(L)=0$ 이므로 $L=0$ 이다.
...
$s$ 가 $L$ 의 특이값이라고 가정하자. @prp-for_svd 에서 $v=e_j,\, w=f_j$ 라고 잡으면 $Lv = sw$ 이다. 또한 $s^2 v = L^\ast L v = sL^\ast w$ 이므로 $L^\ast w = sv$ 이다.
...
&emsp;($c$) $s_1,\ldots,\,s_m$ 은 $T$ 의 양수인 특이값임을 보여라.
...
($a$) $Lv$ 가 정규직교벡터 $\{f_1,\ldots,\,f_n\}$ 의 선형결합이므로 $f_1,\ldots,\,f_m$ 은 $\text{im}$ 의 정규직교기저이다.

($b$) $\{e_1,\ldots,\,e_m,\,e_{m+1},\ldots,\,e_M\}$ 이 $V$ 의 정규직교기저라고 하면 $k>m$ 에 대해 $Le_k = 0$ 이므로 $\{e_{m+1},\ldots,\,e_M\}$ 이 $\ker(L)$ 의 정규직교기저이다. 따라서 $\{e_1,\ldots,\,e_m\}$ 이 $(\ker(L))^\perp$ 의 정규직교기저이다.
