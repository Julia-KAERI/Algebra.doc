...
$\text{nullity}(L^5)$ 는 8 혹은 9이며 @prp-equality_in_the_sequence_of_kernel 에 따라 8 이라면 $\text{nullity}{L^9}=8$ 이어야 하므로 $\text{nullity}(L^5)=9$ 이고 $m\ge 5$ 에 대해 성립한다.
...
$\ker (L)= \ker (L^2)$ 이면 @prp-equality_in_the_sequence_of_kernel 에 따라 모든 $m\ge 1$ 에 대해 $\ker (L^m) = \ker (L)$ 이다. 
