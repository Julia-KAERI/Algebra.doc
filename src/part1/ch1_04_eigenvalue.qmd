...
$\boldsymbol{D}\in \mathcal{M}_{n\times n}(\mathbb{F})$ 가 대각행렬, 즉 대각성분을 제외한 모든 성분이 $0$ 인 행렬이라고 하자. 표준기저 $\hat{\boldsymbol{e}}_i$ 에 대해 $\boldsymbol{D}\hat{\boldsymbol{e}}_i = D_{ii} \hat{\boldsymbol{e}}_i$ 이므로 $\hat{\bf{e}}_i$ 는 $D_{ii}$ 를 고유값으로 갖는 $\boldsymbol{D}$ 의 고유벡터이다. 
