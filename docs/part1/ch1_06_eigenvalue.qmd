...
&emsp; ($2$) $\mathcal{M}_{n}(\mathbb{F})$ 의 어떤 기저 $(\boldsymbol{v}_1,\ldots,\,\boldsymbol{v}_n)$ 에 대해 $\boldsymbol{Av}_i = \text{span}(\boldsymbol{v}_i,\ldots,\,\boldsymbol{v}_n)$ 이다.

:::

::: {.proof}

(1 $\implies$ 2) $\boldsymbol{B}=\boldsymbol{P}^{-1}\boldsymbol{AP}$ 가 상삼각행렬이라고 하자. $\boldsymbol{e}_1,\ldots,\,\boldsymbol{e}_n$ 에 대해 
$$
\boldsymbol{Be}_i \in \text{span} ( \boldsymbol{e}_i,\ldots,\,\boldsymbol{e}_n)
$$
이다. $\boldsymbol{v}_i = \boldsymbol{P}\boldsymbol{e}_i$ 라 하자.  $\boldsymbol{A}= \boldsymbol{PBP}^{-1}$ 이므로,
$$
\boldsymbol{Av}_i = \boldsymbol{PBP}^{-1}\boldsymbol{Pe}_i = \boldsymbol{PB}\boldsymbol{e}_i
$$
이다. 따라서 $\boldsymbol{Av}_i \in \text{span}(\boldsymbol{v}_i,\ldots,\,\boldsymbol{v}_n)$ 이다. 
...
$$
\boldsymbol{Be}_i = \boldsymbol{P}^{-1}\boldsymbol{AP}\boldsymbol{e}_i = \boldsymbol{P}^{-1} \boldsymbol{Av}_i 
$$
이다. $\boldsymbol{Av}_i \in \text{span}(\boldsymbol{v}_i,\ldots,\,\boldsymbol{v}_n)$ 이므로 $\boldsymbol{Be}_i \in \text{span}(\boldsymbol{e}_i,\ldots,\,\boldsymbol{e}_n)$ 이다. 즉 $\boldsymbol{B}$ 는 상삼각행렬이다. $\square$ 
...
이제 우리는 복소수체에서 정의된 정사각 행렬은 어떤 상 삼각 행렬과 닮았음을 안다. 그렇다면 상삼각 행렬에 대해 몇가지를 알아보자.

</br>

### 상삼각행렬의 역행렬

::: {#thm-inverse_of_uppertriangular}
상삼각행렬은 대각성분에 $0$ 이 없다면 가역행렬이다.
:::

::: {.proof}
상삼각행렬의 행렬식은 대각성분의 곱이며, 대각성분에 $0$ 이 있다면 행렬식이 $0$ 이다. 행렬식이 $0$ 인 행렬은 가역행렬이 아니며, 행렬식이 $0$ 이 아닌 행렬은 가역행렬이다. $\square$
:::
</br>

### 닮은 행렬의 고유값과 행렬식

$\boldsymbol{P}$ 가 닮음변환 행렬이라면, $\det (\boldsymbol{P})\ne 0$ 이며,

$$
\det(\boldsymbol{P}^{-1}\boldsymbol{AP}-\lambda \boldsymbol{I}) = \det(\boldsymbol{P}^{-1}(\boldsymbol{A}-\lambda \boldsymbol{I})\boldsymbol{P}) = \det(\boldsymbol{A}-\lambda \boldsymbol{I})
$$

이므로 닮음변환에 의해 행렬의 고유값이 변화하지 않는다. 상삼각 행렬 $\boldsymbol{A}$ 에서

$$
\det (\boldsymbol{A}-\lambda \boldsymbol{I}) = \prod _i (A_{ii}-\lambda)
$$

이므로 상삼각 행렬의 행렬식은 대각성분들의 곱이다. 

::: {#prp-eigenvalues_of_uppertriangular}
상삼각행렬의 행력식은 대각성분의 곱이다.
:::
...
::: {.solution}
$U=\{\boldsymbol{0}\}$ 이면 자명하다. $\dim U < \dim V$ 일 때 $U
