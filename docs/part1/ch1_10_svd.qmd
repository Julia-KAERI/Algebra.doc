...
$\boldsymbol{A} \in \mathbb{C}^{n \times n}$ 에 대해 다음은 동치이다.

1. $\boldsymbol{A}$ 는 positive semidefinite 이다.

2. $\boldsymbol{A}$ 는 에르미트행렬이며 모든 고유값은 non-negative 하다

3. $\boldsymbol{A}$ 의 positive semidifinite 제곱근 행렬이 존재한다.

4. $\boldsymbol{A}$ 의 에르미트 제곱근 행렬이 존재한다.

5. $\boldsymbol{A} = \boldsymbol{R}^\ast \boldsymbol{R}$ 인 행렬 $\boldsymbol{R}$ 이 존재한다.
...
($5 \implies 6$) $\boldsymbol{A}=\boldsymbol{R}^\ast \boldsymbol{R}$ 이면 $\boldsymbol{A}^\ast = (\boldsymbol{R}^\ast\boldsymbol{R})^\ast = \boldsymbol{R}^\ast \boldsymbol{R}=\boldsymbol{A}$ 이므로 $\boldsymbol{A}$ 는 에르미트 행렬이다. 또한 임의의 $\boldsymbol{v}\in \mathbb{C}^n$ 에 대해 $\langle \boldsymbol{v},\, \boldsymbol{A}\boldsymbol{v}\rangle = \langle \boldsymbol{R}^\ast \boldsymbol{R}\boldsymbol{v},\, \boldsymbol{v} \rangle = \|\boldsymbol{Rv}\|^2\ge 0$ 이므로 $\boldsymbol{A}$ 는 positive definite 이다. $\square$
...
임의의 $\boldsymbol{B}\in \mathbb{F}^{m \times n}$ 에 대해 $\boldsymbol{B}^\ast \boldsymbol{B}$ 를 생각 할 수 있다. 이 때 $\boldsymbol{B}^\ast \in \mathbb{C}^{n \times m}$ 이다. $\boldsymbol{B}^\ast \boldsymbol{B}$ 는

1. 에르미트이다.

2. positive semidefinite 이다.

3. $\sqrt{\boldsymbol{B}^\ast \boldsymbol{B}}$ 를 생각 할 수 있다.
...
$\mathbb{F}^{n\times 1}$ 에서의 두 정규직교기저벡터 $\mathcal{B}_1=\{\boldsymbol{u}_1,\ldots,\,\boldsymbol{u}_n\}$ 과 $\mathcal{B}_2=\{\boldsymbol{v}_1,\ldots,\,\boldsymbol{v}_n\}$ 을 생각하자. 그리고 $\mathcal{B}_1$ 과 $\mathcal{B}_2$ 의 벡터를 열벡터로 갖는 행렬 $\boldsymbol{U},\,\boldsymbol{V}$ 를 생각하자. 이 때,
$$
...
#### 특이값 (singular value)
$\boldsymbol{A} \in \mathbb{C}^{n \times n}$ 에 대해 $\sqrt{\boldsymbol{A}^\ast \boldsymbol{A}}$ 의 고유값을 **특이값(singular value)** 라 한다. $\sqrt{\boldsymbol{A}^\ast \boldsymbol{A}}$ 가 positive semidefinite 이므로 당연히 모든 특이값은 $0$ 이 아닌 실수이다.
:::
:::
...
$\boldsymbol{A}\in \mathbb{F}^{n\times n}$ 가 특이값으로 $s_1,\ldots,\,s_n$ 을 가질 때 어떤 isometry $\boldsymbol{U},\,\boldsymbol{V} \in \mathbb{F}^{n\times n}$ 와 대각행렬 $\boldsymbol{D}^\mathbb{n \times n}$ 에 대해
$$
