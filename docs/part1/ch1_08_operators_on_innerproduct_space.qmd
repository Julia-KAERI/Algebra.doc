...
### Adjoint matrix {#sec-adjoint_matrix}
...
($2$) ($1$) 의 결과에 $\boldsymbol{A}$ 대신 $\boldsymbol{A}^\ast$ 를 넣으면 $\ker(\boldsymbol{A}) = (\text{im}(\boldsymbol{A}^\ast))^\perp$ 이다. @cor-dimension_relation_of_orthogonal_complement 에서 부분공간 $U$ 에 대해 $(U^\perp)^\perp = U$ 임을 이용하면 $2$ 를 얻는다. 
...
$\boldsymbol{A}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 에 대해 $\boldsymbol{A}^\ast = \boldsymbol{A}$ 이면 이 행렬을 **에르미트 행렬(Hermitian matrix)** 이라 한다. 특별히 $\mathbb{F}=\mathbb{R}$ 일 경우에는 **대칭행렬 (Symmetric matrix)** 이라 한다.  
...
($2$) 고유벡터 $\boldsymbol{u}\in \mathcal{M}_{n}(\mathbb{F})$ 에 대한 고유값을 $\lambda$ 라 하자. 
...
($4$) 내적벡터공간의 경우 내적을 노름의 합으로 표현할 수 있다. 예를 들어, $\boldsymbol{A}\in \mathcal{M}_{m \times n}(\mathbb{R})$ 이면 임의의 $\boldsymbol{u},\,\boldsymbol{u}'\in \mathcal{M}_{n \times n}(\mathbb{R})$ 에 대해

$$
\langle \boldsymbol{Au},\,\boldsymbol{u}'\rangle =\dfrac{\langle \boldsymbol{A}(\boldsymbol{u}+\boldsymbol{u}'),\, \boldsymbol{u}+\boldsymbol{u}'\rangle -\langle \boldsymbol{A}(\boldsymbol{u}-\boldsymbol{u}'),\, \boldsymbol{u}-\boldsymbol{u}'\rangle}{4}
$$

이다. 모든 $\boldsymbol{u}\in \mathcal{M}_{n }(\mathbb{F})$ 에 대해 $\langle \boldsymbol{Au},\,\boldsymbol{u}\rangle =0$ 이면 우변이 $0$ 이므로 $\langle \boldsymbol{Au},\,\boldsymbol{u}'\rangle = 0$ 이므로 $\boldsymbol{A}=\boldsymbol{0}$ 이다.


이다. $\boldsymbol{A}\in \mathcal{M}_{n \times n}(\mathbb{C})$ 이라면,

$$
\begin{aligned}
 \langle \boldsymbol{Au},\, \boldsymbol{u}'\rangle & = \dfrac{1}{4} \left(\langle \boldsymbol{A}(\boldsymbol{u}+\boldsymbol{u}'),\, \boldsymbol{u}+\boldsymbol{u}' \rangle - \langle \boldsymbol{A}(\boldsymbol{u}-\boldsymbol{u}'),\, \boldsymbol{u} - \boldsymbol{u}'\rangle \right) \\
&\qquad + \dfrac{i}{4} \left( \langle \boldsymbol{A}(\boldsymbol{u}+i\boldsymbol{u}'),\, \boldsymbol{v}+i\boldsymbol{u}'\rangle  - \langle \boldsymbol{A}(\boldsymbol{u}-i\boldsymbol{u}'),\, \boldsymbol{u}-i\boldsymbol{u}'\rangle \right) \\
\end{aligned}
$$

임을 보일 수 있으며, 임의의 $\boldsymbol{u},\, \boldsymbol{u}'\in V$ 에 대해 $\langle \boldsymbol{Au},\,\boldsymbol{u}'\rangle =0$ 이므로 $\boldsymbol{A}=0$ 이다. $\square$
:::
...
&emsp; ($2$) $\boldsymbol{A}$ 는 에르미트 행렬이다.
...
임을 안다. @prp-peroperties_hermitian_operator 의 ($4$) 로 부터 $\boldsymbol{A}=\boldsymbol{A}^\ast$ 임을 알 수 있다. 즉 $\boldsymbol{A}$ 는 에르미트 행렬이다.
