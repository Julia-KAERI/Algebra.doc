...
이면  $L$ 을 $U$ 에서 $V$ 로의 **선형 사상(linear map)** 혹은 **선형 변환 (linear transformation)** 이라 한다. 그리고 $\mathbb{F}$-벡터공간 $U,\,V$ 에 대해 $U\mapsto V$ 인 선형사상의 집합을 $\mathcal{L}(U,\,V)$ 라고 쓴다.
:::
:::
...
함수 $L:U \to V$ 가 모든 $u\in U$ 에 대해 $L(u)=0_V$ 일 때 이 $L$ 를 **영사상(zero map)** 이라 부르며 $0(u)$ 와 같이 표기한다. 영사상이 선형사상이다.
...
::: {#exm-LA_T_zero}
$L\in \mathcal{L}(U,\,V)$ 일 때 $L(0_U) = 0_V$ 이다.
:::
...
#### 항등사상
$I_U\in \mathcal{L}(U)$ 일 때 모든 $u\in u$ 에 대해 $I_U(u) = u$ 이면 $I_U$ 를 벡터공간 $U$ 에서의 **항등 사상 (identity operator)** 이라고 한다. 항등사상은 선형사상임은 자명하다. 또한 $L\in \mathcal{L}(U)$ 일 때 다음 두 명제가 동치이다.
...
벡터공간 $V$ 에 대해 $I\in \mathcal{L}(V)$ 가 $I(v)=v$ 일 때, 즉 $I$ 가 항등사상일 때 이에 대한 행렬표현 $\bf{I}=[I]$ 를 항등행렬이라 한다. 어떤 기저 $\mathcal{B}_V = \{v_i\}$ 에 대해서도 $I(v_i) = v_i$ 이므로 $(\bf{I})_{ij} = \delta_{ij}$ 이다. 
:::
:::
...
::: {.proof}
$\ker (L)$ 는 $U$ 의 부분공간이다(@lem-LA_ker_range_subspace). $\text{nullity}(L)=m$ 이라 하고 $\ker (L)$ 의 기저를 $(u_1,\ldots,u_m )$ 이라 하자. 이 기저를 확장하여 $U$ 의 기저를 이루는 벡터 $(u_1,\ldots,\,u_m,\,w_1,\ldots,w_n)$ 를 구할 수 있다(@lem-LA_basis_expansion). 그렇다면 $\dim (U) = m + n$ 이다. 이제 $\text{rank}(L)=n$ 임을 보이면 된다.
...
### 가역사상과 역행렬 {#sec-LA_invertible_map_and_inverse_matrix}

$L\in \mathcal{L}(U,\,V)$ 가 가역사상이라고 하자. 이 때 임의의 $u\in U,\, v\in V$ 에 대해 $(L^{-1}\circ T )(u)=u$ 이고 $(L \circ L^{-1}) (v)= v$ 이다. $\mathcal{B}_U,\,\mathcal{B}_V$ 가 각각 $U,\,V$ 의 기저라고 하자. $\bf{L}=[L]_{\mathcal{B}_U, \mathcal{B}_V}$ 에 대해 $[L^{-1}]_{\mathcal{B}_V,\mathcal{B}_U} =\bf{L}^{-1}$ 이라고 쓰는 것은 자연스럽다. 또한 $\bf{LL}^{-1} = \bf{L}^{-1}\bf{L} = \bf{I}$ 라는 것을 알 수 있다. 이 때 $\bf{L}^{-1}$ 을 $\bf{L}$ 의 **역행렬 (inverse matrix)** 라 한다.
...
$U$ 가 $n$ 차원 $\mathbb{F}$-벡터공간이며 $\mathcal{B}_1 = (u_1,\ldots,u_n)$ 이 $U$ 의 기저라고 하자. $P\in \mathcal{L}(U)$ 가 가역이면 @cor-LA_basis_transformation 에서 보았듯이 $\mathcal{B}_2= (P(u_1),\ldots, P(u_n) )$ 도 $U$ 의 기저이다. $v_i = T(u_i)$ 라고 할 때 $P$ 가 다음과 같이 정의된다고 하자.

$$
P(u_i) = \sum_{j=1}^n P_{ji}u_j = v_i
$$

그렇다면 $u_i = P^{-1}(P(u_i)) = P^{-1}(v_i)$ 이다. $\bf{u}_i=[u_i]_{\mathcal{B}_1}$, $\bf{v}_i = [v_i]_{\mathcal{B}_1}$, $\bf{P}=[P]_{\mathcal{B}_1}$ 라면 $\bf{v}_i = \bf{Pu}_i$ 이다. 그런데 $\bf{u}_i =[u_i]_{\mathcal{B}_1}$ 은 표준기저 $\bf{e}_i$ 이므로 $\bf{v}_i$ 는 $\bf{P}$ 의 $i$ 번 째 열 $\bf{P}_{:i}$ 이다. 이것은 앞으로도 매우 중요하므로 뒤에 별도로 다시 언급하겠다.
...
이제 연산자 $L\in \mathcal{L}(U)$ 이 $L(u_i) = \sum_j A_{ij}u_j$ 로 정의되었으며 이 연산자의 $\mathcal{B}_1$ 기저에서의 행렬표현을 $\bf{A}$ 라 하자. 즉 $\bf{A} = [L]_{\mathcal{B}_1}$ 이다. 이 때,
...
이다. 이로부터 $[L]_{\mathcal{B}_2} = \bf{P}^{-1}\bf{AP}$ 임을 알 수 있다. 즉 두 기저 $\mathcal{B}_1 =\{u_i\}$, $\mathcal{B}_2 = \{P(u_i)\}$ 사이에,

$$
[L]_{\mathcal{B_1}} = \bf{A} \iff [L]_{\mathcal{B}_2} = \bf{P}^{-1}\bf{AP}
$$
...
두 정사각 행렬 $\bf{A},\,\bf{B}$ 가 닮았을 때 $\bf{A} \sim \bf{B}$ 라 표현한다고 하자. 이 때 다음이 성립함을 보여라.

&emsp;($1$) $\bf{A} \sim \bf{A}$ 이다.

&emsp;($2$) $\bf{A} \sim \bf{B}$ 이면 $\bf{B} \sim \bf{A}$ 이다.

&emsp;($3$) $\bf{A} \sim \bf{B}$ 이고 $\bf{B} \sim \bf{C}$ 이면 $\bf{C} \sim \bf{A}$ 이다.
