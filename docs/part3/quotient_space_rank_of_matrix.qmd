...
이제 행렬의 행공간의 차원과 열공간의 차원이 같다는 것은 쉽게 보일 수 있다. $[T^\ast] = [T]^T$ 이며 @lem-rank_Tast 로부터 $\text{rank}(T^\ast) = \text{rank}(T)$ 임을 알았다. $\text{rank}(T^{\ast})$ 는 $[T]$ 의 행공간의 차원이며 $\text{rank}(T)$ 는 열공간의 차원이므로 행렬의 행공간과 열공간의 차원은 같다는 것을 다시 한번 확인할 수 있었다.
