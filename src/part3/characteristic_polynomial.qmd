...
행렬의 trace $\text{tr} : \F^{n\times n} \to \mathbb{F}$ 는 다음과 같이 정의된다.

$$
\text{tr}(\bf{\bf{A}}) = \sum_{i=1}^n A_{ii}.
$$

Trace 역시 닮음 변환에 대해 불변이므로(@prp-ALA_trace_is_invariant_under_similar_transformation) 연산자 $L\in \mathcal{L}(V)$ 에 대해서도 정의 할 수 있다.
...
$\bf{A},\,\bf{P}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 이며 $\bf{P}$ 가 가역이라고 하자. 

$$
\begin{aligned}
\text{tr}(\bf{P}^{-1}\bf{AP}) &= \sum_{i=1}^n\left(\bf{P}^{-1}\bf{AP}\right)_{ii} = \sum_{i, j, k=1}^n (P^{-1})_{ij}A_{jk}P_{ki} = \sum_{j, k=1}^n A_{jk} (\bf{PP}^{-1})_{kj} = \sum_{j=1}^n A_{jj} \\
...
$\bf{B}_i\in \mathcal{M}_{n \times n}(\mathbb{F})$ 는 상삼각행렬이며 $i$ 번째 대각성분이 $0$ 이라고 하자. 이 때 $\bf{B}_1 \bf{B}_2 \cdots \bf{B}_n = \bf{0}$ 임을 보여라.
