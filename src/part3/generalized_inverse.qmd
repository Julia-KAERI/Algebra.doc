...
::: {#prp-part3_generalized_inverse_of_invertible_matrix}
...
@thm-part3_gim_abcd 의 notation 에 대해 $G_0 = \begin{bmatrix} R^{-1} & 0 \\ 0 & 0 \end{bmatrix} \in \mathcal{M}_{n \times m}(\mathbb{F})$ 를 생각하자. 이 때 $G=P_2G_0 {P_1}$ 이 $A$ 의 일반화된 역행렬이다.
...
@thm-part3_gim 은 일반적인 행렬에 대해 최소한 하나의 일반화된 역행렬이 존재한다는 것을 보장한다. 
...
::: {#prp-part3_uniqueness_of_penrose_inverse}
...
::: {#prp-part3_penrose_inverse_for_diagonal_matrix}
...
@def-part3_penrose_inverse 의 각 조건을 만족함을 보이자.
...
::: {#exr-part3_penrose_inverse_and_adjoint}
...
::: {#exr-part3_basic_properties_of_penrose_inverse_1}
...
::: {#prp-part3_basic_properties_of_penrose_inverse_2}
...
($1$) 우선 $A^+ = A^\ast (AA^\ast)^+$ 를 보인다. $D = A^\ast(AA^\ast)^+$ 라 하자. $D$ 가  @def-part3_penrose_inverse 를 따르는 $A$ 의 펜로즈 역행렬임을 보인다.
...
\end{aligned}
$$ {#eq-part3_prp_basic_properties_of_penrose_inverse_2_1}
...
이므로 $XA=0$ 이다. 즉 @eq-part3_prp_basic_properties_of_penrose_inverse_2_1 에 대해 $(I-AD)A=0 \implies ADA=A$.
...
::: {#prp-part3_product_of_penrose_inverse}
