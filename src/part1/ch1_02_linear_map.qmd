...
#### 항등연산자
$I_U\in \mathcal{L}(U)$ 일 때 모든 $u\in u$ 에 대해 $I_U(u) = u$ 이면 $I_U$ 를 벡터공간 $U$ 에서의 **항등 연산자 (identity operator)** 이라고 한다. 항등사상은 선형사상임은 자명하다. 또한 $L\in \mathcal{L}(U)$ 일 때 다음 두 명제가 동치이다.
...
$\mathcal{M}_{1\times n}(\mathbb{F})$ 과 $\mathcal{M}_{n \times 1}(\mathbb{F})=\mathcal{M}_n(\mathbb{F})$ 이 벡터공간임을 안다. 행렬 $\bf{A}$ 의 $i,\,j$ 성분을 $A_{ij}$ 혹은 $A_{i,j}$  로 표현할 수 있듯이 $\bf{A}$ 의 $i$ 번째 행을 $\bf{A}_{i,:}$ 혹은 $\bf{A}_{i:}$ 라 쓰고 $j$ 번째 열을 $\bf{A}_{:,j}$ 혹은 $\bf{A}_{:j}$ 라 쓰기로 하자. 각각의 $\bf{A}_{:i}$ 와 $\bf{A}_{j:}$ 를 행벡터와 열벡터로 간주한다. 즉 $m\times n$ 행렬의 경우 $\bf{A}_{:i}\in \mathcal{M}_m(\mathbb{F})$ 이며 $\bf{A}_{i:}\in \mathcal{M}_{1 \times n}(\mathbb{F})$ 이다. 많은 경우 계산을 하는데 도움이 된다.

**1.** $m \times n$ 행렬 $\bf{A}$ 과 $n \times p$ 행렬 $\bf{B}$ 를 생각하자. 이 때,
...
**2.** $m\times n$ 행렬 $\bf{A}$ 와 $n \times 1$ 행렬 $\bf{c}$ 를 생각하자. $c_i$ 를 $\bf{c}$ 의 $i$ 번째 성분이라 하면,
...
::: {.proof}
$U,\,V$ 의 기저를 각각 $\mathcal{B}_U = (u_i),\,\mathcal{B}_V = (v_j)$ 라 할 때 $\Phi: \mathcal{L}(U,\,V) \to \mathcal{M}_{m \times n}(\mathbb{F})$ 를 $\Phi(L) = [L]_{\mathcal{B}_U, \mathcal{B}_V}$ 로 정의한다. @lem-LA_MM_linearmap 으로부터 $\Phi$ 는 선형사상임을 안다. $\ker (\Phi) = \{0_U\}$ 이므로 단사(injective)이다. 모든 $\mathcal{M}_{m \times n}(\mathbb{F})$ 에 대해 상응하는 $L$ 가 존재하므로 전사(surjective)이다. 즉 $\Phi$ 는 동형사상이다. $\square$
