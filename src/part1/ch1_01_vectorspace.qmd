...
실수 성분을 갖는 $m \times n$ 행렬 전체의 집합을 $\mathbb{R}^{m\times n}$ 으로 표기한다. 마찬가지로 복소수 성분을 갖는 $m \times n$ 행렬 전체의 집합을 $\mathbb{C}^{m \times n}$ 으로 표기하도록 한다.
...
일반적으로 $\{y_1,\,y_2,\ldots\}$ 처럼 기술되는 집합은 집합 내의 원소를 나열하는 순서가 중요하지 않다. 하지만 그 순서가 중요한 경우가 있는데 이 경우 집합에 순서를 부여하여 $(x_1,\,x_2,\,x_3,\ldots)$ 와 같이 $(\quad)$ 안에 나열하여 표현하는 것을 **튜플(tuple)** 이라고 한다. 즉, $\{a,\,b\} = \{b,\,a\}$ 이지만 $(a,\,b) \ne (b,\,a)$ 이다. 순서집합의 대표적인 경우가 수열이다.
...
실계수를 갖는 $m \times n$ 행렬의 집합을 $\mathbb{R}^{m\times n}$ 이이라고 하기로 하였다. $\bf{A},\,\bf{B}\in \mathbb{R}^{m \times n}$ 일 때 두 행렬을 더하면 그 결과는 같은 크기의 행렬이며 더하여 나온 행렬의 성분은 같은 위치에서의 $\bf{A}$ 와 $\bf{B}$ 의 성분의 합으로 정의하자. 즉,
...
이라 정의하자. 그렇다면 $\mathbb{R}^{m \times n}$ 은 $\mathbb{R}$-벡터공간이다. $\mathbb{C}^{m\times n}$ 이 $\mathbb{C}$-벡터공간임은 쉽게 보일 수 있다. 즉 체 $\mathbb{F}$ 를 원소로 갖는 $m\times n$ 행렬의 집합을 $\mathbb{F}^{m\times n}$ 이라 하면 이 집합은 $\mathbb{F}$-벡터공간이다.
:::
...
$\mathbb{F}^{n \times 1}$ 의 원소를 **열벡터**라고 하며 $\mathbb{F}^n$ 라고 쓰기로 하자. 데카르트곱과 같은 표기법을 사용하지만 이는 문맥에서 구별 할 수 있거나 명시하기로 하자. $\mathbb{F}^{1\times n}$ 의 원소를 **행벡터** 라고 한다.
...
$U,\,V$ 가 벡터공간일 때 $U \cap V$ 도 벡터공간이며 따라서 $U,\,V$ 각각에 대한 부분공간이다.
:::
</div></br>
...
::: {#prp-LA_vector_independency_and_span_2}
벡터공간 $V$ 와 영벡터가 아닌 벡터의 집합 $V_0 = \{v_1,\ldots,\,v_n\} \subset V$ 에 대해 다음은 동치이다.
...
유한개의 벡터의 집합 $\{u_1,\ldots,u_m\}$ 이 벡터공간 $V$ 를 span 하면 $V$ 는 유한차원 벡터공간이다. 만약 이 $\{u_i\}$ 가 선형종속이면 적절한 벡터를 제거하여 $V$ 를 span 하게 할 수 있다(@prp-LA_vector_dependency_and_span). 더 이상 제거할 수 없을 때 이 집합은 선형 독립(@prp-LA_vector_independency_and_span_2)이며 $V$ 의 기저이다. 우리는 이로부터 다음을 알 수 있다.
...
>$$
>\text{span} (\{u_1,\ldots,u_k,v_1,\ldots,v_{k+1}\}-\{u_m\})=V
>$$
>이다.
...
유한차원 벡터공간 $V$ 의 기저에 포함된 벡터의 개수를 $V$ 의 **차원(dimension)** 이라고 하며 $\dim (V)$ 로 표기한다.
...
우리는 @exm-LA_vectorspace_matrix 에서 $\mathbb{F}$ 의 의 원소를 성분으로 갖는 행렬의 집합 $\mathbb{F}^{m\times n}$ 이 벡터공간이라는 것을 알았다. $1 \le i \le m$, $1 \le j \le n$ 에 대해 $\bf{E}^{(i,j)}$ 을 $i$ 행 $j$ 열 성분은 $1$ 이며 나머지 성분은 모두 $0$ 인 $m \times n$ 행렬이라 하자. 즉 $\mathbb{F}^{2 \times 3}$ 에서 $\bf{E}^{(2, 1)} = \begin{bmatrix} 0 & 0 & 0 \\ 1 & 0 & 0\end{bmatrix}$ 이다. $\mathbb{F}^{m \times n}$ 에서 $\mathcal{B}=\left\{\bf{E}^{(i, j)} : 1\le i \le m,\, 1 \le j \le n\right\}$ 는 선형독립이다. 또한 $\mathcal{B}$ 가 $\mathbb{F}^{m\times n}$ 을 span 한다. $\mathcal{B}$ 의 원소의 갯수는 $mn$ 이므로 $\dim (\mathbb{F}^{m\times n})=mn$ 이다.
...
열벡터의 벡터공간 $\mathbb{F}^n$ 의 표준기저벡터 $\bf{e}_i$ 는 $i$ 번째 성분만 $1$ 이며 나머지 성분은 $0$ 인 $n \times 1$ 행렬이다. $\mathbb{F}^n$ 에서와 마찬가지로 $\{\bf{e}_1,\ldots,\bf{e}_n\}$ 이 $\mathbb{F}^n$ 의 기저이며 이를 $\mathbb{F}^n$ 의 표준기저라고 한다.
