...
@prp-relatively_prime 에 의해 $ma+nb=1$ 을 만족하는 $m,\,n\in \mathbb{Z}$ 가 존재한다. $mac+nbc=c$ 이며 $a\mid mac$ 이고 가정에 의해 $a\mid mbc$ 이므로 $a \mid c$ 이다. $\square$
...
$p$ 가 소수이므로 $p \nmid a$ 라면 @prp-prime_number_equivalence 에 의해 $\gcd (p,\,a)=1$ 이다. $mp+na=1$ 을 만족하는 $m,\,n\in \mathbb{Z}$ 가 존재한다. $mpb + nab = b$ 이며 $p \mid nab$ 이므로 $p\mid b$ 이다. $\square$ 
...
($4$) $n \mid a(b-c)$ 이고 $n$ 과 $a$ 가 서로소 이므로 @prp-relatively_prime 에 의해 $n \mid (b-c)$ 이다.  $\square$
...
우리는 @thm-part1_permutation_even_odd 에서 대칭군이 호환(transposition) 의 합성으로 표현될 때 그 갯수는 변하더라도 갯수가 짝수인지 홀수인지는 변하지 않는다는 것을 보였으며 그것이 짝수일 때 **짝순열**, 홀수일 때 **홀순열** 이라고 부른다는 것을 안다. 특히 짝순열의 집합은 군을 이루며 그것을 **교대군 (Alternating group)** 이라고 하고 $A_n$ 이라고 쓴다. 
...
&emsp; ($1$) @def-part1_elementary_row_operation 의 $E^{(2)}$ 와 $E^{(3)}$ 가 $GL_n(\mathbb{R})$ 을 생성함을 보이시오.
