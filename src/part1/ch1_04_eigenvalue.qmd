...
정사각행렬 $\bf{A}\in \mathcal{M}_{n \times n}(\mathbb{F})$ 에 대해 $p_{\bf{A}}(\lambda) := \det (\lambda \bf{I}-\bf{A})$ 를 $\bf{A}$ 의 **특성 다항식(characteristic polynomial)** 이라 한다.
...
즉 $\bf{A}=\mathcal{M}_{m \times m}(\F), \bf{B} = \mathcal{M}_{n\times n}(\F),\, \bf{C}=\mathcal{M}_{m\times n}(\F),\, \bf{D}= \mathcal{M}_{n \times m}(\F)$ 에 대해
...
&emsp;($2$) $\mathcal{M}_{n}(\mathbb{F})$ 의 어떤 기저 $(\bf{v}_1,\ldots,\,\bf{v}_n)$ 가 존재하여 $\bf{Av}_i = \text{span}(\bf{v}_1,\ldots,\,\bf{v}_i)$ 이다.
...
복소수체에서 정의된 행렬 $\bf{A}\in \mathcal{M}_{n\times n}(\mathbb{C})$ 는 닮은 상삼각행렬이 존재한다. 
:::
...
두 연산자 $L,\,K\in \mathcal{L}(V)$ 가 $LK=KL$ 일 때 $L$ 과 $K$ 는 **교환가능하다(be commutable)** 고 한다. 두 행렬 $\bf{L},\, \bf{K} \in \mathcal{M}_{n \times n}(\F)$ 가 $\bf{LK}=\bf{KL}$ 일 때 두 행렬이 교환 가능하다고 한다.
