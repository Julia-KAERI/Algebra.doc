...
@prp-part3_linear_functional_4 로부터 $\varphi(v_i) \in V^\ast$ 이다. 이제 $\dim V^\ast = \dim V = n$ 이므로 $\{\varphi_1,\ldots,\varphi_n\}$ 이 선형독립임을 보이면 된다.
...
@prp-part3_dual_basis 은 $n$ 차원 $\mathbb{F}$-벡터공간 $V$ 의 모든 선형 범함수는 $(\varphi_i,\ldots,\,\varphi_n)$ 의 선형결합으로 표현할 수 있다는 것을 의미한다.
...
::: {.callout-note appearance="simple" icon="false"}
::: {#def-part3_dual_basis}

#### 쌍대 기저 (Dual basis)

@prp-part3_dual_basis 의 $(\varphi_1,\ldots,\,\varphi_n)$ 을 $V$ 의 기저 $(v_1,\ldots,\,v_n)$ 에 대한 **쌍대 기저(dual basis)** 라 한다. 
...
::: {#prp-part3_dual_basis_and_vector}
...
::: {#prp-part3_dual_map}
@def-part3_dual_map 에서 정의된 $L^T$ 은 선형사상이다.
:::
...
우리는 이미 전치행렬과 행렬의 rank 에 대해 [행간소 사다리꼴 행렬을 통한 선형사상의 이해](../part1/ch1_03_determinant.qmd#sec-linear_map_understood_by_RREM) 알아보았다. 여기서는 그 의미를 다른 방법으로 알아보자.
...
::: {#thm-part3_dimension_of_matrix_by_transpose}
...
@prp-part3_transpose_of_matrix 의 표기법을 그대로 사용한다. $\boldsymbol{A}$ 의 열공간의 차원은 $\text{rank}(L)$ 와 같다. @lem-part3_rank_Tprime 에 따라 $\text{rank}(L^T)$ 과도 같다. 이것은 $\boldsymbol{A}^T$ 의 열공간의 차원과 같고 또한 $\boldsymbol{A}$ 의 행공간의 차원과 같다. $\square$ 
:::
...
::: {#thm-part3_isomorphsm_to_dual_space}
...
유한차원 벡터공간 $V$ 에 대해 $V$ 와 $V'$ 은 동형지만 동형사상은 보통 $V$ 의 기저를 정하고 난후 찾을 수 있다. 그러나 $V$ 와 $V^\ast{}^\ast$ 사이의 동형사상은 @thm-part3_isomorphsm_to_dual_space 의 ($3$) 에서 보였듯이 매우 직접적이고 자연스럽다. 
...
@prp-part3_dual_basis 에 의해 $\psi$ 는 $\{\varphi_1,\ldots,\,\varphi_n\}$ 의 선형 결합이다. $\psi = \sum_i c_i \varphi_i$ 라고 하면
...
이므로 $\Phi$ 는 동형사상이다. $\mathcal{L}(U, V)$ 와 $\mathcal{L}(V^\ast, U^\ast)$ 가 모두 유한차원 이므로 $\ker (\Phi)=0$ 임을 보이면 되는데 이미 @exr-part3_axler_3_F_15 에서 보였다. 

:::

</br><div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-part3_axler_3_F_18}
...
trivial. see @exr-part3_axler_3_F_18

:::

</br><div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-part3_axler_3_F_20}
...
@exr-part3_axler_3_F_22 로 부터 $\dim(U^0 \cap W^0) = \dim (U+W)^0$ 임을 안다. 따라서,
