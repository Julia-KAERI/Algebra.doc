...
벡터공간에서의 내적과 마찬가지로 노름 역시 공간에 부여된 특정 조건을 만족하는 함수이다. 내적벡터공간은 그 자체로 거리공간인데 이것은 내적에 의해 노름과 거리함수가 자연스럽게 정의되기 때문이다. 참고로 노름공간은 거리함수가 부여된 벡터공간이지만 엄밀한 의미에서의 거리공간은 벡터공간이 아닐 수도 있으며, 단지 적절한 거리함수가 정의된 집합이면 거리공간이다. 여기서는 거리함수가 부여된 벡터공간인 노름공간만을 
...
우리는 @prp-orthogonal_linearity 와 @cor-number_of_orthogonal_vectors 를 통해 유한차원 내적벡터공간의 서로 직교하는 벡터의 갯수는 벡터공간의 차원보다 작거나 같다는 것을 안다. 정규직교기저가 되기 위해서는 정규직교벡터의 숫자가 차원과 같아야 하는데 이것은 뒤에 [그람-슈미트 과정과 정규직교기저](ch1_07_innerproduct.qmd#sec-gram_schmidt) 에서 자연스럽게 알게 된다. 
...
<div class="border" style="background-color:#FFF0F5 ;padding:5px;">
::: {#prp-properties_of_orthonormal_vectors}
...
## 그람-슈미트 과정과 정규직교기저 {#sec-gram_schmidt}
...
::: {#thm-part1_reisz_representation_theorem_1}

#### Riesz 의 표현 정리 1 (Reisz Representation theorem 1)
...
::: {.callout-note appearance="simple" icon="false"}
::: {#def-part1_orthogonal_complement}
...
이므로 $w\in U^\perp$ 이다. @prp-orthogonal_complement_is_a_vector_space 의 ($3$) 에 따라 $U\cap U^\perp = \{0_V\}$ 이므로 @prp-condition_for_direct_sum 에 따라 $V=U \oplus U^\perp$ 이다.
...
::: {#exm-part1_orthogonal_space_in_infinite_dimensional_vector_space}

#### 무한차원 벡터공간에서의 직교여공간

위의 @prp-generated_subspace_and_orthogonal_compliment 는 $U$ 가 무한차원 벡터공간일 때 성립하지 않는 수 있다는 것을 보일 수 있다. $[-1,\,1]$ 구간에서 연속인 함수의 집합 $C_{[-1, 1]}$ 는 벡터공간이며 내적을
...
::: {.proof}
@prp-generated_subspace_and_orthogonal_compliment 에 따라 $V=U\oplus U^\perp$ 이다. 이제 $U$ 의 정규직교기저 $\{e_1,\ldots,\,e_n\}$ 과 $U^\perp$ 의 정규직교 기저 $\{f_1,\ldots,\,f_m\}$ 에 대해 $u = \sum_i a_i e_i + \sum_j b_j f_j$ 를 만족하는 $a_i,\, b_j$ 가 존재한다. $u= \sum_i a_i e_i$ 이며 $w=\sum_j b_j f_j$ 이다. 이 때 $u,\,w$ 가 유일하게 정해진다는 것은 자명하다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-dimension_relation_of_orthogonal_complement}
...
### Orthogonal projection {#sec-part1_orthogonal_projection}
...
벡터공간 $V$ 와 projection $P\in \mathcal{L}(V)$ 에 대해 $U = \text{im}(P)$ 라 하자. $U$ 가 유한차원 부분공간이라면 $V=U \oplus U^\perp$ 이라는 것을 안다. 또한 @prp-uniqueness_of_orthogonal_decomposition 에서 보인 것처럼 $V$ 가 유한차원이라면 $v\in V$ 에 대해 $v=u+u',\, u\in U,\, u'\in U^{\perp}$ 가 되도록 하는 $u,\,u'$ 은 유일하다. 이제 반대로 $U \le V$ 에 대해 $v\in V$ 는 어떤 $u\in U$ 와 $u'\in U^\perp$ 의 합으로 유일하게 표현 될 수 있다는 것을 안다. 이렇게 어떤 부분공간 $U$ 를 치역으로 갖는 projection 다음과 같이 따로 정의할 수 있다.
...
Reisz 표현 정리(@thm-part1_reisz_representation_theorem_1) 에서는 유한차원 벡터공간 $V$ 에서의 모든 선형 범함수는 각각 유일하게 정해진 어떤 벡터에 대한 내적과 같다는 것을 보였다. 


</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part1_reisz_representation_theorem_2}
...
$\Phi : V \to V^\ast$ 를 $\Phi(v) = \varphi_v$ 로 정의하자. $\Phi$ 가 단사임은 @thm-part1_reisz_representation_theorem_1 에서 보였다. 이제 $\Phi$ 가 전사임을 보이자. $\varphi \in V^\ast$ 일 때 $\varphi=0$ 이면 $\varphi = \varphi_{0_V}$ 이다. 이제 $\varphi \ne \varphi_{0_V}$ 에서만 생각해도 된다. $\ker \varphi \ne V$ 이므로 $(\ker \varphi)^\perp \ne \{0_V\}$ 이다.

$w\in (\ker \varphi)^\perp,\, w\ne 0_V$ 일 때

$$
v = \dfrac{\overline{\varphi (w)}}{\|w\|^2} w
$$ {#eq-part1_reisz_representation_theorem_2_1}

라면 $v\in (\ker \varphi)^\perp$ 이다. 양변에 노름을 취하면

$$
\|v\| = \dfrac{|\varphi (w)|}{\|w\|}
$$ {#eq-part1_reisz_representation_theorem_2_2}
...
### 최소화 문제 {#sec-part1_minimization_problems}


::: {.callout-note appearance="simple" icon="false"}
::: {#def-part1_distance_to_subspace}
...
::: {#exm-part1_approximation_of_sin_function}
...
를 최소화하도록 $p(x)$ 의 계수를 정하는 것도 한가지 방법이다. 이제 $C[-\pi,\, \pi]$ 를 $[-\pi,\,\pi]$ 구간에서의 연속인 실함수의 집합으로 정의하면 $C:=C[-\pi,\, \pi]$ 는 벡터공간이며 $f,\,g\in \in C$ 일 때 $C$ 에서의 내적을 아래와 같이 정의 할 수 있다는 것을 안다. (@exm-innerproduct_definded_by_intergration_in_R)
...
### 의사 역사상 {#sec-part1_pseudoinverse}
...
::: {#lem-part1_pseudoinverse_2}

위의 @def-part1_pseudo_inverse 에서 
...
($2$) $w\in \text{im}(L)$ 이면 @lem-part1_pseudoinverse_2 ($2$) 에서 보였듯이 $LL^+w = w$ 이다. $w\in (\text{im}(L))^\perp$ 이면 역시 @lem-part1_pseudoinverse_2 ($2$) 에서 보였듯이 $LL^+w=0_W$ 이다. 즉 $LL^+ = P_{\text{im}(L)}$ 이다.
...
($1$) $x\in V$ 에 대해 $Lx-b = (Lx - LL^+b) + (LL^+b -b)$ 이다. @lem-part1_pseudoinverse_3 ($2$) 에 따라 $(Lx-LL^+b) \in \text{im} (L)$ 이며 $(LL^+b - b)\in (\text{im}L)^\perp$ 이다. 따라서 
...
원래의 우리는 주어진 $L \in \mathcal{L}(V,\,W)$ 과 $b\in W$ 에 대해 $\|Lx-b\|$ 를 최소화 하는 $x$ 를 찾고자 했다. @thm-part1_pseudoinverse_2 로부터 우리는 $x=L^+b$ 일 때 최소가 됨을 알았다. 
...
라고 정의한다. Reisz 표현정리 (@thm-part1_reisz_representation_theorem_1) 를 내적벡터공간 $U$ 에 적용하면 
...
$(e_1,\ldots,\,e_n)$ 이 $V$ 의 정규직교기저라고 하자. 이 정규직교기저의 쌍대기저는 Reisz 의 표현정리 2 (@thm-part1_reisz_representation_theorem_2) 에서의 범함수의 그것과 같음을 보여랴.
...
$L$ 이 projection 이므로 $L^2=L$ 이며 $L = P_{\text{im}(L)}$ 이다. 또한 $(\ker (L))^\perp = \text{im}(L)$ 이므로 $L|_{(\ker L)^\perp} = I|_{\text{im}(L)}$ 이다. @def-part1_pseudo_inverse 로 부터 $L^+w = L$ 이다.
...
($1$) $\ker (L^+) = (\text{im}(L))^\perp$ 를 보이자. @def-part1_pseudo_inverse 로부터 $L^+w = 0_V \iff P_{\text{im}(L)}= 0_V$ 따라서 $\ker (L^+) = (\text{im}(L))^\perp$
...
::: {.solution}
@lem-part1_pseudoinverse_3 ($2$) 로부터 $LL^+=P_{\text{im}(L)}$ 이므로

$$
LL^+L = P_{\text{im}(L)}L = L.
$$

@lem-part1_pseudoinverse_3 ($3$) 으로부터 $L^+L=P_{(\ker L)^\perp}$ 이므로
...
이제 $v\in (\ker L)^\perp$ 인 경우를 생각해보자. 앞서 보았듯이 $P_{\text{im}(M)}=P_{(\ker L)^\perp}$ 이므로 $M^+v = \left(M|_{(\ker M)^\perp} \right)^{-1}v$ 이다. 역시 @exr-axler4_6_C_20 에서 보였듯이 $(\ker L^+)=(\text{im}(L))^\perp$ 이며 @cor-dimension_relation_of_orthogonal_complement ($2$) 로부터
...
$\ker (P)$ 와 $\text{im}(P)$ 가 직교하므로 @thm-rank_nullity 에 의해 $\ker (P) \cap \text{im}(P) =\{0_V\}$ 이다. 또한 $\text{im}(P_U)=U$ 이므로 $U=\text{im}(P)$ 라 놓으며 조건을 만족한다.
